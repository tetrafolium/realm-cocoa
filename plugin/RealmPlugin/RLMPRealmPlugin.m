...
- (void)didApplicationFinishLaunchingNotification:
  ...
  NSString *urlString =
      [[NSWorkspace sharedWorkspace] fullPathForApplication:@"Realm Browser"];
  if (urlString) {
    ...
    if (menuItem) {
      ...
      NSMenuItem *actionMenuItem =
          [[NSMenuItem alloc] initWithTitle:@"Open Realm..."
    ...
    NSLog(@"Realm Plugin: Couldn't find Realm Browser. Will not show 'Open "
          @"Realm...' menu item.");
  }
}
...
- (void)openBrowser {
  ...
  if (!self.browserUrl) {
    ...
    NSString *message =
        @"You need to install the Realm Browser in order to use it from this "
        @"plugin. Please visit realm.io for more information.";
  ...
  if (realmFileURLs.count == 0) {
    ...
    NSString *message =
        @"You must launch iOS Simulator with app that uses Realm";
  ...
  if (arguments.count > 1) {
    ...
    [alert setInformativeText:
               [NSString stringWithFormat:@"%ld Realm files are detected. Are "
                                          @"you sure to open them at once?",
                                          arguments.count]];
