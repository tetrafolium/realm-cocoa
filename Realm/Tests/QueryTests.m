...
- (void)testPredicateNotSupported {
  ...
  XCTAssertThrows(([LinkToAllTypesObject
      objectsWhere:@"%@ = allTypesCol.binaryCol", [NSData data]]));
  XCTAssertThrows(([LinkToAllTypesObject
      objectsWhere:@"allTypesCol.binaryCol CONTAINS %@", [NSData data]]));
...
- (void)testPredicateMisuse {
  ...
  RLMAssertThrowsWithReasonMatching([realm objects:className
                                             where:@"height > 72"],
                                    @"'height' not found in .* 'PersonObject'");
  ...
  RLMAssertThrowsWithReasonMatching(
      [realm objects:className where:@"name == 3"],
      @"type string .* property 'name' .* 'PersonObject'.*: 3");
  RLMAssertThrowsWithReasonMatching(
      [realm objects:className where:@"age IN {'xyz'}"],
      @"type int .* property 'age' .* 'PersonObject'.*: xyz");
  XCTAssertThrows([realm objects:className where:@"name IN {3}"],
                  @"invalid type");

  className = AllTypesObject.className;

  XCTAssertThrows([realm objects:className where:@"boolCol == Foo"],
                  @"invalid type");
  XCTAssertThrows([realm objects:className where:@"boolCol == 2"],
                  @"invalid type");
  XCTAssertThrows([realm objects:className where:@"dateCol == 7"],
                  @"invalid type");
  XCTAssertThrows([realm objects:className where:@"doubleCol == The"],
                  @"invalid type");
  XCTAssertThrows([realm objects:className where:@"floatCol == Bar"],
                  @"invalid type");
  XCTAssertThrows([realm objects:className where:@"intCol == Baz"],
                  @"invalid type");

  className = PersonObject.className;

  // compare two constants
  XCTAssertThrows([realm objects:className where:@"3 == 3"],
                  @"comparing 2 constants");

  // invalid strings
  RLMAssertThrowsWithReasonMatching([realm objects:className where:@""],
                                    @"Unable to parse");
  ...
  XCTAssertThrows([realm objects:className where:@"age * 25"],
                  @"invalid operator");
  XCTAssertThrows([realm objects:className where:@"age === 25"],
                  @"invalid operator");
  ...
  RLMAssertThrowsWithReasonMatching(
      [realm objects:className where:@"age BETWEEN Foo"],
      @"BETWEEN operator must compare a KeyPath with an aggregate");
  RLMAssertThrowsWithReasonMatching([realm objects:className
                                             where:@"age BETWEEN {age, age}"],
                                    @"must be constant values");
  RLMAssertThrowsWithReasonMatching([realm objects:className
                                             where:@"age BETWEEN {age, 0}"],
                                    @"must be constant values");
  RLMAssertThrowsWithReasonMatching([realm objects:className
                                             where:@"age BETWEEN {0, age}"],
                                    @"must be constant values");
  RLMAssertThrowsWithReasonMatching([realm objects:className
                                             where:@"age BETWEEN {0, {1, 10}}"],
                                    @"must be constant values");

  NSPredicate *pred =
      [NSPredicate predicateWithFormat:@"age BETWEEN %@", @[ @1 ]];
  RLMAssertThrowsWithReasonMatching(
      [realm objects:className withPredicate:pred], @"exactly two objects");
  ...
  RLMAssertThrowsWithReasonMatching(
      [realm objects:className withPredicate:pred], @"type int for BETWEEN");
  ...
  // bad type in link IN
  XCTAssertThrows([PersonLinkObject objectsInRealm:realm
                                             where:@"person.age IN {'Tim'}"]);
}
...
- (void)testLinkQueryInvalid {
  ...
  RLMAssertThrowsWithReasonMatching(
      ...
      @"Key paths.*array property.*aggregate operations");
  RLMAssertThrowsWithReasonMatching(
...
- (void)testQueryBetween {
  ...
  [realm beginWriteTransaction];
  id a = [AllTypesObject createInRealm:realm
                             withValue:@[
                               @YES, @1, @1.0f, @1.0, @"a",
                               [@"a" dataUsingEncoding:NSUTF8StringEncoding],
                               date1, @YES, @((long)1), stringObj
                             ]];
  id b = [AllTypesObject createInRealm:realm
                             withValue:@[
                               @YES, @2, @2.0f, @2.0, @"b",
                               [@"b" dataUsingEncoding:NSUTF8StringEncoding],
                               date2, @YES, @((long)2), stringObj
                             ]];
  id c = [AllTypesObject createInRealm:realm
                             withValue:@[
                               @NO, @3, @3.0f, @3.0, @"c",
                               [@"c" dataUsingEncoding:NSUTF8StringEncoding],
                               date3, @YES, @((long)3), stringObj
                             ]];
  id d = [AllTypesObject createInRealm:realm
                             withValue:@[
                               @NO, @33, @3.3f, @3.3, @"cc",
                               [@"cc" dataUsingEncoding:NSUTF8StringEncoding],
                               date33, @NO, @((long)3.3), stringObj
                             ]];
  ...
  RLMAssertCount(AllTypesObject.allObjects, 2U,
                 @"doubleCol BETWEEN {3.0, 7.0}");
...
- (void)testQueryWithDates {
  ...
  [AllTypesObject createInRealm:realm
                      withValue:@[
                        @YES, @1, @1.0f, @1.0, @"a",
                        [@"a" dataUsingEncoding:NSUTF8StringEncoding], date1,
                        @YES, @((long)1), stringObj
                      ]];
  [AllTypesObject createInRealm:realm
                      withValue:@[
                        @YES, @2, @2.0f, @2.0, @"b",
                        [@"b" dataUsingEncoding:NSUTF8StringEncoding], date2,
                        @YES, @((long)2), stringObj
                      ]];
  [AllTypesObject createInRealm:realm
                      withValue:@[
                        @NO, @3, @3.0f, @3.0, @"c",
                        [@"c" dataUsingEncoding:NSUTF8StringEncoding], date3,
                        @YES, @((long)3), stringObj
                      ]];
  [realm commitWriteTransaction];
...
- (void)verifySortWithAccuracy:(RLMRealm *)realm
                        column:(NSString *)column
                     ascending:(BOOL)ascending
                        getter:(double (^)(id))getter
                      expected:(double)val
                      accuracy:(double)accuracy {
  // test TableView query
  RLMResults<AllTypesObject *> *results = [[AllTypesObject
      allObjectsInRealm:realm] sortedResultsUsingKeyPath:column
                                               ascending:ascending];
  XCTAssertEqualWithAccuracy(getter(results[0][column]), val, accuracy,
                             @"Array not sorted as expected");
...
- (void)testQuerySorting {
  ...
  [self verifySortWithAccuracy:realm
                        column:@"dateCol"
                     ascending:YES
                        getter:dateGetter
                      expected:date1.timeIntervalSince1970
                      accuracy:1];
  [self verifySortWithAccuracy:realm
                        column:@"dateCol"
                     ascending:NO
                        getter:dateGetter
                      expected:date33.timeIntervalSince1970
                      accuracy:1];
...
- (void)testTwoColumnComparison {
  ...
  [self.queryObjectClass
      createInRealm:realm
          withValue:@[
            @YES, @YES, @1, @2, @23.0f, @1.7f, @0.0, @5.55, @"a", @"a"
          ]];
  [self.queryObjectClass
      createInRealm:realm
          withValue:@[
            @YES, @NO, @1, @3, @-5.3f, @4.21f, @1.0, @4.44, @"a", @"A"
          ]];
  [self.queryObjectClass
      createInRealm:realm
          withValue:@[
            @NO, @NO, @2, @2, @1.0f, @3.55f, @99.9, @6.66, @"a", @"ab"
          ]];
  [self.queryObjectClass
      createInRealm:realm
          withValue:@[
            @NO, @YES, @3, @6, @4.21f, @1.0f, @1.0, @7.77, @"a", @"AB"
          ]];
  [self.queryObjectClass
      createInRealm:realm
          withValue:@[
            @YES, @YES, @4, @5, @23.0f, @23.0f, @7.4, @8.88, @"a", @"b"
          ]];
  [self.queryObjectClass
      createInRealm:realm
          withValue:@[
            @YES, @NO, @15, @8, @1.0f, @66.0f, @1.01, @9.99, @"a", @"ba"
          ]];
  [self.queryObjectClass
      createInRealm:realm
          withValue:@[
            @NO, @YES, @15, @15, @1.0f, @66.0f, @1.01, @9.99, @"a", @"BA"
          ]];
...
- (void)testStringBeginsWith {
  ...
  RLMAssertCount(AllTypesObject, 0U,
                 @"objectCol.stringCol BEGINSWITH[cd] NULL");
}
...
- (void)testStringLike {
  ...
  RLMAssertCount(StringObject, 1U,
                 @"stringCol LIKE 'a?"
                  "?'");
  RLMAssertCount(StringObject, 1U, @"stringCol LIKE '?b?'");
  RLMAssertCount(StringObject, 1U, @"stringCol LIKE '*?c'");
  ...
  RLMAssertThrowsWithReasonMatching(
      [StringObject objectsWhere:@"stringCol LIKE[d] '*'"],
      @"'LIKE' not supported .* diacritic-insensitive");
  RLMAssertThrowsWithReasonMatching(
      [StringObject objectsWhere:@"stringCol LIKE[cd] '*'"],
      @"'LIKE' not supported .* diacritic-insensitive");
}
...
- (void)testLiveQueriesInsideTransaction {
  ...
  [realm beginWriteTransaction];
  {
    [self.queryObjectClass
        createInRealm:realm
            withValue:@[
              @YES, @YES, @1, @2, @23.0f, @1.7f, @0.0, @5.55, @"", @""
            ]];
    ...
    QueryObject *q1 = [self.queryObjectClass
        createInRealm:realm
            withValue:@[
              @NO, @YES, @1, @2, @23.0f, @1.7f, @0.0, @5.55, @"", @""
            ]];
    XCTAssertEqual(resultsQuery.count, 0U);
    XCTAssertEqual(resultsTableView.count, 0U);
    ...
    [self.queryObjectClass
        createInRealm:realm
            withValue:@[
              @YES, @NO, @1, @3, @-5.3f, @4.21f, @1.0, @4.44, @"", @""
            ]];
...
- (void)testMultiLevelLinkQuery {
  ...
  for (int i = 0; i < 5; ++i) {
    circle = [CircleObject createInRealm:realm
                               withValue:@{
                                 @"data" : [NSString stringWithFormat:@"%d", i],
  ...
  AllTypesObject *obj4 =
      [AllTypesObject createInRealm:realm
                          withValue:@[
                            @NO, @3, @3.0f, @3.0, @"c",
                            [@"c" dataUsingEncoding:NSUTF8StringEncoding],
                            date3, @YES, @34359738368LL, NSNull.null
                          ]];
...
- (void)testClass:(Class)class
  ...
  XCTAssertEqual(
      notCount, [[self evaluate:[class objectsWithPredicate:predicate]] count]);
}
...
- (void)testINPredicate {
  ...
  [AllTypesObject createInRealm:realm
                      withValue:@[
                        @YES, @1, @1.0f, @1.0, @"abc",
                        [@"a" dataUsingEncoding:NSUTF8StringEncoding],
                        [NSDate dateWithTimeIntervalSince1970:1], @YES, @1LL, so
                      ]];
  [realm commitWriteTransaction];
  ...
  [self testClass:[AllTypesObject class]
      withNormalCount:1
             notCount:0
                where:@"doubleCol IN {1}"];
  [self testClass:[AllTypesObject class]
      withNormalCount:1
             notCount:0
                where:@"doubleCol IN {1, 2}"];

  // NSString
  [self testClass:[AllTypesObject class]
      withNormalCount:1
             notCount:0
                where:@"stringCol IN {'abc'}"];
  [self testClass:[AllTypesObject class]
      withNormalCount:0
             notCount:1
                where:@"stringCol IN {'def'}"];
  [self testClass:[AllTypesObject class]
      withNormalCount:0
             notCount:1
                where:@"stringCol IN {'ABC'}"];
...
- (void)testComparisonsWithKeyPathOnRHS {
  ...
  RLMAssertThrowsWithReasonMatching(
      [self.queryObjectClass objectsWhere:@"'Realm' CONTAINS string1"].count,
...
- (void)testSubqueries {
  ...
  [realm beginWriteTransaction];
  CompanyObject *first = [CompanyObject
      createInRealm:realm
          withValue:@[
            @"first company",
            @[
...
- (void)testSortingColumnsWithNull {
  ...
  {
    NumberObject *no1 = [NumberObject createInRealm:realm
                                          withValue:@[ @1, @1.1f, @1.1, @YES ]];
    NumberObject *noNull = [NumberObject
        createInRealm:realm
            withValue:@[ NSNull.null, NSNull.null, NSNull.null, NSNull.null ]];
    NumberObject *no0 = [NumberObject createInRealm:realm
    ...
    for (RLMProperty *property in [[NumberObject alloc] init]
      ...
      RLMResults *descending =
          [[NumberObject allObjectsInRealm:realm] sortedResultsUsingKeyPath:name
                                                                  ascending:NO];
...

  {
    ...
    RLMResults *ascending = [[DateObject allObjectsInRealm:realm]
        sortedResultsUsingKeyPath:@"dateCol"
                        ascending:YES];
    XCTAssertEqualObjects(
        [ascending valueForKey:@"dateCol"],
        ([@[ doNull, doNegative, doZero, doPositive ] valueForKey:@"dateCol"]));

    RLMResults *descending = [[DateObject allObjectsInRealm:realm]
        sortedResultsUsingKeyPath:@"dateCol"
                        ascending:NO];
    XCTAssertEqualObjects(
        [descending valueForKey:@"dateCol"],
        ([@[ doPositive, doZero, doNegative, doNull ] valueForKey:@"dateCol"]));
  }
...
#define RLMAssertOperator(op, matchingCount, notMatchingCount, nilCount)       \
  ...
  [AllOptionalTypes
      createInRealm:realm
          withValue:@[ @NO, @0, @0, @0, @"", matchingData, matchingDate ]];
  [realm commitWriteTransaction];
