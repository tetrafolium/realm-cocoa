...
- (void)testSystemExceptionWithNonPOSIXSystemException {
  ...
  XCTAssertEqualObjects(RLMMakeError(exception),
                        [NSError errorWithDomain:RLMUnknownSystemErrorDomain
                                            code:code
                                        userInfo:expectedUserInfo]);
}

- (void)testRealmFileException {
  realm::RealmFileException exception(
      realm::RealmFileException::Kind::NotFound, "/some/path",
      "don't do that to your files", "lp0 on fire");
  RLMError dummyCode = RLMErrorFail;
  NSDictionary *expectedUserInfo = @{
    NSLocalizedDescriptionKey : @"don't do that to your files",
