...
static void RLMAssertRealmSchemaMatchesTable(id self, RLMRealm *realm) {
	for (RLMObjectSchema *objectSchema in realm.schema.objectSchema) {
		...
		TableRef table = ObjectStore::table_for_object_type(
			realm.group, objectSchema.objectName.UTF8String);
		for (RLMProperty *property in objectSchema.properties) {
			auto column = info.tableColumn(property);
			XCTAssertEqual(column, table->get_column_index(RLMStringDataWithNSString(
									       property.columnName)));
			XCTAssertEqual(property.indexed || property.isPrimary,
			               table->has_search_index(column));
		}
	}
}
...
- (void)testGetSchemaVersion {
	XCTAssertThrows([RLMRealm schemaVersionAtURL:RLMDefaultRealmURL()
	                 encryptionKey:nil
	                 error:nil]);
	NSError *error;
	XCTAssertEqual(RLMNotVersioned,
	               [RLMRealm schemaVersionAtURL:RLMDefaultRealmURL()
	                encryptionKey:nil
	                error:&error]);
	RLMValidateRealmError(error, RLMErrorFail,
...
- (void)testEnumeratedObjectsDuringMigration {
	[self createTestRealmWithClasses:
	 @[ StringObject.class, ArrayPropertyObject.class, IntObject.class ]
	 block:^(RLMRealm *realm) {
	         [StringObject createInRealm:realm
	          withValue:@[ @"string" ]];
	         [ArrayPropertyObject
...
- (void)testDateTimeFormatAutoMigration {
		...
		distantPast, distantFuture, beforeEpoch, epoch, afterEpoch, referenceDate
        ];
        ...
        @autoreleasepool {
                ...
                for (NSDate *date in expectedDates) {
                        [DateMigrationObject
                         createInRealm:realm
                         withValue:@[ date, date, date, date, @(cookieValue) ]];
                        [DateMigrationObject
                         createInRealm:realm
                         withValue:@[
				 date, NSNull.null, date, NSNull.null, @(cookieValue)
                        ]];
		}
                [realm commitWriteTransaction];

                NSURL *url = [config.fileURL.URLByDeletingLastPathComponent
                              URLByAppendingPathComponent:@"fileformat-old-date.realm"];
                [realm writeCopyToURL:url encryptionKey:nil error:nil];
                NSLog(@"wrote pre-migration realm to %@", url);
#else
                NSURL *bundledRealmURL =
			[[NSBundle bundleForClass:[DateMigrationObject class]]
			 ...
			 withExtension:@"realm"];
                ...
                for (NSDate *date in expectedDates) {
                        RLMResults *results = [DateMigrationObject
                                               objectsInRealm:realm
                                               where:@"nonNullIndexed = %@ AND nullIndexed = %@", date,
                                               date];
...
- (void)testMigratingFromMixed {
        ...
        RLMRealmConfiguration *config = [RLMRealmConfiguration defaultConfiguration];
        config.objectClasses = @[
		[AllTypesObject class], [LinkToAllTypesObject class], [StringObject class]
        ];
        ...
        NSLog(@"wrote pre-migration realm to %@", url);
#else
        NSURL *bundledRealmURL = [[NSBundle
                                   bundleForClass:[DateMigrationObject class]] URLForResource:@"mixed-column"
                                  withExtension:@"realm"];
        ...
        config.migrationBlock = ^(RLMMigration *migration, uint64_t) {
                ...
                [migration
                 enumerateObjects:@"AllTypesObject"
                 block:^(RLMObject *oldObject, RLMObject *newObject) {
                         XCTAssertEqualObjects(values[--i], oldObject[@"mixedCol"]);
                         RLMAssertThrowsWithReasonMatching(
				 newObject[@"mixedCol"],
				 @"Invalid property name 'mixedCol' for class "
				 @"'AllTypesObject'.");
		 }];
                migrationCalled = true;
	};
...
- (void)testMigrationRenameProperty {
        ...
        id inputValue = @[
		@YES, @1, @1.1f, @1.11, @"string", [NSData dataWithBytes:"a" length:1], now,
		@YES, @11, @[ @"a" ]
        ];

        [self createTestRealmWithSchema:@[
		 objectSchema, stringObjectSchema, linkingObjectsSchema
         ]
         block:^(RLMRealm *realm) {
