...
- (void)testValueForCollectionOperationKeyPath {
	...
	[CompanyObject
	 createInRealm:realm
	 withValue:@{@"name" : @"ABC AG", @"employees" : @[ c3e1 ]}];
	[realm commitWriteTransaction];
...
- (void)testResultsDependingOnDeletedLinkView {
        ...
        [realm transactionWithBlock:^{
                 ...
                 object = [IntegerArrayPropertyObject
                           createInDefaultRealmWithValue:@[ @0, @[ intObject ] ]];
	 }];

        RLMResults *results = [object.array sortedResultsUsingKeyPath:@"intCol"
                               ascending:YES];
        [results firstObject];

        RLMResults *unevaluatedResults =
		[object.array sortedResultsUsingKeyPath:@"intCol" ascending:YES];
