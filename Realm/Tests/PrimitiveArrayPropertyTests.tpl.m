...
- (void)testAddObject {
	RLMAssertThrowsWithReason(
		[$array addObject:$wrong],
		^n @"Invalid value '$wdesc' of type '$wtype' for expected type '$type'");
...
- (void)testInsertObject {
	RLMAssertThrowsWithReason(
		[$array insertObject:$wrong atIndex:0],
		^n @"Invalid value '$wdesc' of type '$wtype' for expected type '$type'");
	% r RLMAssertThrowsWithReason(
...
- (void)testReplace {
	...
	RLMAssertThrowsWithReason(
		[$array replaceObjectAtIndex:0 withObject:$wrong],
		^n @"Invalid value '$wdesc' of type '$wtype' for expected type '$type'");
	% r RLMAssertThrowsWithReason(
...
- (void)testIndexOfObjectWhere {
	...
	% unman XCTAssertEqual(NSNotFound,
...
- (void)testMin {
	...
	man RLMAssertThrowsWithReason(
		[$array minOfProperty:@"self"],
		^n @"minOfProperty: is not supported for $type array '$class.$prop'");
...
- (void)testMax {
	...
	man RLMAssertThrowsWithReason(
		[$array maxOfProperty:@"self"],
		^n @"maxOfProperty: is not supported for $type array '$class.$prop'");
...
- (void)testSum {
	...
	man RLMAssertThrowsWithReason(
		[$array sumOfProperty:@"self"],
		^n @"sumOfProperty: is not supported for $type array '$class.$prop'");
...
- (void)testAverage {
	...
	% noavg %
	man RLMAssertThrowsWithReason([$array averageOfProperty:@"self"],
	                              ^n @"averageOfProperty: is not supported "
	                              @"for $type array '$class.$prop'");
	...
	% avg XCTAssertEqualWithAccuracy(
		[$array averageOfProperty:@"self"].doubleValue, ($s0 + $s1) / 2.0,
		.001);
}
...
- (void)testValueForKeyNumericAggregates {
	...
	% sum XCTAssertEqualObjects([$array valueForKeyPath:@"@sum.self"],
	                            @($s0 + $s1));
	% avg XCTAssertEqualWithAccuracy(
		[[$array valueForKeyPath:@"@avg.self"] doubleValue], ($s0 + $s1) / 2.0,
		.001);
}
...
- (void)testUnionOfObjects {
	...
	XCTAssertEqualObjects([$array valueForKeyPath:@"@unionOfObjects.self"],
	                      ^n($values2));
	XCTAssertEqualObjects(sortedDistinctUnion($array, @"Objects", @"self"),
	                      ^n($values));
}
...
- (void)testSetValueForKey {
	...
	RLMAssertThrowsWithReason(
		[$array setValue:$wrong forKey:@"self"],
		^n @"Invalid value '$wdesc' of type '$wtype' for expected type '$type'");
	% r RLMAssertThrowsWithReason(
