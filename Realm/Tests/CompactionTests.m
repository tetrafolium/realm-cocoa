...
- (void)testCachedRealmOtherThreadCompactOnLaunch {
  // Test that the compaction block never gets called if the Realm is open on a
  // different thread Access Realm before opening it with a compaction block
  RLMRealmConfiguration *configuration =
      [RLMRealmConfiguration defaultConfiguration];
  configuration.fileURL = RLMTestRealmURL();
  ...
  XCTestExpectation *realmOpenedExpectation = [self
      expectationWithDescription:@"Realm was opened on background thread"];
  [self dispatchAsync:^{
    @autoreleasepool {
  ...
  configurationWithCompactBlock.shouldCompactOnLaunch =
      ^BOOL(NSUInteger totalBytes, NSUInteger usedBytes) {
        ...
        NSUInteger fiveHundredKB = 500 * 1024;
        BOOL shouldCompact =
            (totalBytes > fiveHundredKB) && (usedBytes / totalBytes) < 0.2;
        compactBlockInvoked = YES;
...
- (void)testAccessDeniedOnTemporaryFile {
  ...
  [NSFileManager.defaultManager setAttributes:@{NSFileImmutable : @YES}
                                 ofItemAtPath:tmpURL.path
                                        error:nil];
  RLMAssertThrowsWithReason([RLMRealm realmWithConfiguration:configuration
                                                       error:nil],
