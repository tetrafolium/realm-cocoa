...
- (void)testErrorHandling {
  ...
  auto token = [IntObject.allObjects
      addNotificationBlock:^(RLMResults *results, RLMCollectionChange *change,
        ...
        RLMValidateRealmError(error, RLMErrorFileAccess, @"Too many open files",
                              nil);
        called = true;
        [exp fulfill];
      }];
...
- (void)testMultipleSourceVersionsForAsyncQueries {
  ...
  // Each Realm should see a different number of objects as they're on different
  // versions
...
- (void)testMultipleSourceVersionsWithNotifiersRemovedBeforeRunning {
  ...
  @autoreleasepool {
    ...
    // Each Realm should see a different number of objects as they're on
    // different versions
    for (NSUInteger i = 0; i < 10; ++i) {
      XCTAssertEqual(i + 1, [IntObject allObjectsInRealm:realms[i]].count);
    }
  ...
  auto coord = realm::_impl::RealmCoordinator::get_existing_coordinator(
      config.config.path);
  coord->on_change();
