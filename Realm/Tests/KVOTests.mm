...
- (id)observableForObject:(id)value {
  if (RLMObject *obj = RLMDynamicCast<RLMObject>(value)) {
    RLMObject *copy =
        RLMCreateManagedAccessor(obj.objectSchema.accessorClass, obj->_info);
    copy->_row = obj->_row;
    return copy;
...
- (id)observableForObject:(id)value {
  ...
  if (RLMObject *obj = RLMDynamicCast<RLMObject>(value)) {
    RLMObject *copy = RLMCreateManagedAccessor(
        obj.objectSchema.accessorClass,
...
- (void)testInsertNewColumns {
  ...
  // Add a column before the observed one so that the observed one's index
  // changes
  obj.boolCol = YES;
  ...
  obj->_row.set_int(ndx + 2,
                    3); // can't use the accessor after a local schema change
...
- (void)testShiftObservedColumnBeforeChange {
  ...
  obj->_row.set_bool(
      ndx + 1, true); // can't use the accessor after a local schema change
  AssertChanged(r, @NO, @YES);
}
