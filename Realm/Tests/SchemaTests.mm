...
- (void)testObjectSchema {
	...
	NSArray *expectedTypes = @[
		@"AllTypesObject", @"LinkToAllTypesObject", @"StringObject", @"IntObject"
	];
...
- (void)testClassWithDuplicateProperties {
	...
	RLMObjectSchema *secondSchema = [RLMObjectSchema
...
- (void)testInsertingColumnsInBackgroundProcess {
        ...
        if (!self.isParent) {
                ...
                RLMRealm *realm = [RLMRealm realmWithConfiguration:config error:nil];
                [realm beginWriteTransaction];
                realm->_info[@"IntObject"].table()->insert_column(0, realm::type_String,
                                                                  "col");
                [realm commitWriteTransaction];
                return;
	}
