...
- (void)testOpeningInvalidPathThrows {
  ...
  RLMAssertThrowsWithCodeMatching(
      [RLMRealm realmWithConfiguration:config error:nil], RLMErrorFileAccess);
}
...
- (void)testPathCannotBeBothInMemoryAndRegularDurability {
  ...
  config.fileURL = [NSURL
      fileURLWithPath:@(inMemoryRealm.configuration.config.path.c_str())];
...
- (void)testReadOnlyFileInImmutableDirectory {
  ...
  // Delete '*.lock' and '.note' files to simulate opening Realm in an app
  // bundle
...
- (void)testReadOnlyRealmMustExist {
  RLMAssertThrowsWithCodeMatching([self readOnlyRealmWithURL:RLMTestRealmURL()
                                                       error:nil],
...
- (void)testFilePermissionDenied {
  ...
  RLMAssertThrowsWithCodeMatching([self realmWithTestPath],
...
#define AssertFileUnmodified(oldURL, newURL)                                   \
  do {                                                                         \
    ...
    if (oldData.length < realm::util::page_size()) {                           \
      ...
      XCTAssertNotEqual(([newData rangeOfData:oldData                          \
                                      options:0                                \
                                        range:{0, oldData.length}])            \
                            .location,                                         \
                        NSNotFound);                                           \
    } else                                                                     \
...
- (void)testFileFormatUpgradeRequiredDeleteRealmIfNeeded {
  ...
  bundledRealmURL = [[NSBundle bundleForClass:[RealmTests class]]
      URLForResource:@"fileformat-old-date"
       withExtension:@"realm"];
...
- (void)testAddOrUpdateObjectsFromArray {
  ...
  XCTAssertEqual([(PrimaryStringObject *)objects[0] intCol], 1,
                 @"Value should be 1");
  XCTAssertEqual([(PrimaryStringObject *)objects[1] intCol], 2,
                 @"Value should be 2");
  XCTAssertEqual([(PrimaryStringObject *)objects[2] intCol], 3,
                 @"Value should be 3");
  ...
  XCTAssertEqual([(PrimaryStringObject *)objects[0] intCol], 1,
                 @"Value should be 1");
  XCTAssertEqual([(PrimaryStringObject *)objects[1] intCol], 4,
                 @"Value should be 4");
  XCTAssertEqual([(PrimaryStringObject *)objects[2] intCol], 3,
                 @"Value should be 3");
  XCTAssertEqual([(PrimaryStringObject *)objects[3] intCol], 5,
                 @"Value should be 5");
...
- (void)testCannotOverwriteWithWriteCopy {
  ...
  NSString *expectedError =
      [NSString stringWithFormat:@"File at path '%@' already exists.",
                                 RLMTestRealmURL().path];
  NSString *expectedUnderlying =
      [NSString stringWithFormat:@"open(\"%@\") failed: file exists",
                                 RLMTestRealmURL().path];
  XCTAssertFalse([realm writeCopyToURL:RLMTestRealmURL()
                         encryptionKey:nil
                                 error:&writeError]);
...
- (void)testCannotWriteInNonExistentDirectory {
  ...
  NSString *expectedError = [NSString
      stringWithFormat:@"Directory at path '%@' does not exist.", badPath];
  NSString *expectedUnderlying = [NSString
      stringWithFormat:@"open(\"%@\") failed: no such file or directory",
                       badPath];
  NSError *writeError;
...
- (void)testWriteToReadOnlyDirectory {
  ...
  NSString *expectedError = [NSString
      stringWithFormat:@"Unable to open a Realm at path '%@'. Please use a "
                       @"path where your app has read-write permissions.",
                       RLMTestRealmURL().path];
  NSString *expectedUnderlying =
      [NSString stringWithFormat:@"open(\"%@\") failed: permission denied",
                                 RLMTestRealmURL().path];
  NSError *writeError;
  XCTAssertFalse([realm writeCopyToURL:RLMTestRealmURL()
                         encryptionKey:nil
                                 error:&writeError]);
  RLMValidateRealmError(writeError, RLMErrorFilePermissionDenied, expectedError,
                        expectedUnderlying);
...
- (void)testWriteWithNonSpecialCasedError {
  ...
  NSString *expectedError =
      [NSString stringWithFormat:@"Unable to open a Realm at path '%@': open() "
                                 @"failed: too many open files",
                                 RLMTestRealmURL().path];
  NSString *expectedUnderlying =
      [NSString stringWithFormat:@"open(\"%@\") failed: too many open files",
                                 RLMTestRealmURL().path];
  NSError *writeError;
  XCTAssertFalse([realm writeCopyToURL:RLMTestRealmURL()
                         encryptionKey:nil
                                 error:&writeError]);
  RLMValidateRealmError(writeError, RLMErrorFileAccess, expectedError,
                        expectedUnderlying);
...
- (void)testCoreDebug {
#if DEBUG
  XCTAssertTrue([RLMRealm isCoreDebug],
                @"Debug version of Realm should use librealm{-ios}-dbg");
#else
  XCTAssertFalse([RLMRealm isCoreDebug],
                 @"Release version of Realm should use librealm{-ios}");
#endif
}
#endif
...
- (void)testIsEmpty {
  ...
  [realm beginWriteTransaction];
  [StringObject createInRealm:realm withValue:@[ @"a" ]];
  XCTAssertFalse(realm.isEmpty, @"Realm should not be empty within a write "
                                @"transaction after adding an object.");
  [realm cancelWriteTransaction];

  XCTAssertTrue(realm.isEmpty, @"Realm should be empty after canceling a write "
                               @"transaction that added an object.");

  [realm beginWriteTransaction];
  [StringObject createInRealm:realm withValue:@[ @"a" ]];
  [realm commitWriteTransaction];
  XCTAssertFalse(realm.isEmpty, @"Realm should not be empty after committing a "
...
- (void)testRealmFileAccessInvalidFile {
  ...
  XCTAssertNil([RLMRealm realmWithConfiguration:configuration error:&error],
               @"Invalid database");
  RLMValidateRealmError(error, RLMErrorFileAccess,
...
- (void)testRealmFileAccessFileIsDirectory {
  ...
  XCTAssertNil([RLMRealm realmWithConfiguration:configuration error:&error],
               @"Invalid database");
  RLMValidateRealmError(error, RLMErrorFileAccess,
                        @"Unable to open a realm at path", @"Is a directory");
}
...
- (void)testInvalidLockFile {
  ...
  int fd = open([RLMRealmConfiguration.defaultConfiguration.fileURL.path
                    stringByAppendingString:@".lock"]
                    .UTF8String,
                O_RDWR);
  XCTAssertNotEqual(-1, fd);
  ...
  RLMRealm *realm = [RLMRealm
      realmWithConfiguration:RLMRealmConfiguration.defaultConfiguration
                       error:&error];
  XCTAssertNil(realm);
  RLMValidateRealmError(error, RLMErrorIncompatibleLockFile,
