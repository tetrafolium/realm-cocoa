...
static void verifyInWriteTransaction(__unsafe_unretained RLMRealm *const realm,
                                     SEL sel) {
  if (!realm) {
    @throw RLMException(@"Cannot call %@ on an unmanaged object.",
                        NSStringFromSelector(sel));
  }
  if (!realm.inWriteTransaction) {
    @throw RLMException(@"Cannot call %@ outside of a write transaction.",
                        NSStringFromSelector(sel));
  }
}
...
id RLMPermissionForRole(RLMArray *array, id role) {
  ...
  for (RLMPermission *p in filtered) {
    ...
    // one and then delete it as otherwise revoking permissions won't actually
    // work
    else {
  ...
  if (!permission) {
    // Use the dynamic API to create the appropriate Permission class for the
    // array
    permission = (id)[array.realm createObject:array.objectClassName
                                     withValue:@[ role ]];
...
+ (NSDictionary *)linkingObjectsProperties {
  return @{
    @"roles" :
        [RLMPropertyDescriptor descriptorWithClass:RLMPermissionRole.class
                                      propertyName:@"users"]
  };
}
...
+ (RLMPermission *)permissionForRole:(RLMPermissionRole *)role
  ...
  RLMPermission *permission = [RLMPermission createInRealm:role.realm
                                                 withValue:@[ role ]];
  [array addObject:permission];
  return permission;
}
...
+ (RLMPermission *)permissionForRoleNamed:(NSString *)roleName
                                  inArray:(RLMArray<RLMPermission *>
                                           <RLMPermission> *)array {
  verifyInWriteTransaction(array.realm, _cmd);
  return RLMPermissionForRole(
      array, [RLMPermissionRole createOrUpdateInRealm:array.realm
...
+ (RLMPermission *)permissionForRoleNamed:(NSString *)roleName
  ...
  return [self permissionForRoleNamed:roleName
                              inArray:[RLMRealmPermission objectInRealm:realm]
                                          .permissions];
}
...
+ (RLMPermission *)permissionForRoleNamed:(NSString *)roleName
  ...
  verifyInWriteTransaction(realm, _cmd);
  return [self permissionForRoleNamed:roleName
                              inArray:[RLMClassPermission objectInRealm:realm
                                                               forClass:cls]
                                          .permissions];
}
...
+ (RLMPermission *)permissionForRoleNamed:(NSString *)roleName
  ...
  verifyInWriteTransaction(realm, _cmd);
  return
      [self permissionForRoleNamed:roleName
                           inArray:[RLMClassPermission objectInRealm:realm
...
+ (RLMPermission *)permissionForRoleNamed:(NSString *)roleName
  ...
  @throw RLMException(
      @"Object %@ does not have a RLMArray<RLMPermission *> property.", object);
}
@end
...
+ (instancetype)objectInRealm:(RLMRealm *)realm forClass:(Class)cls {
  return [RLMClassPermission
      objectInRealm:realm
      forPrimaryKey:[cls _realmObjectName] ?: [cls className]];
}
@end
...
- (instancetype)initWithRealmPath:(NSString *)path
  ...
  if (self = [super init]) {
    ...
    if (!identity) {
      @throw RLMException(
          @"A permission value cannot be created without a valid user ID");
    }
...
- (NSUInteger)hash {
  return self.identity.hash ^ self.accessLevel ^ self.path.hash ^
...
- (BOOL)isEqual:(id)object {
  ...
  if (self.identity) {
    ...
    if (that.identity || ![self.key isEqualToString:that.key] ||
...
- (RLMSyncPermission *)tildeExpandedSyncPermissionForUser:(RLMSyncUser *)user {
  ...
  NSString *path = [self.path stringByReplacingCharactersInRange:{1, 1}
                                                      withString:user.identity];
  if (_identity) {
    return [[RLMSyncPermission alloc] initWithRealmPath:path
                                               identity:_identity
...
- (NSString *)description {
  ...
  if (self.identity) {
    typeDescription =
        [NSString stringWithFormat:@"identity: %@", self.identity];
  } else {
    typeDescription =
        [NSString stringWithFormat:@"key: %@, value: %@", self.key, self.value];
  }
  return [NSString
      stringWithFormat:@"<RLMSyncPermission> %@, path: %@, access level: %@",
                       typeDescription, self.path,
                       RLMSyncAccessLevelToString(self.accessLevel)];
}
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<RLMSyncPermissionOffer: %p> path: %@, access level: "
                       @"%@, token: %@, createdAt: %@, expiresAt: %@",
                       self, _realmPath,
                       RLMSyncAccessLevelToString(_accessLevel), _token,
                       _createdAt, _expiresAt];
}
@end
