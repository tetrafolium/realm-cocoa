...
void CocoaSyncUserContext::register_refresh_handle(
	const std::string &path, RLMSyncSessionRefreshHandle *handle) {
	REALM_ASSERT(handle);
...
+ (NSDictionary *)allUsers {
	...
	return
	        [NSDictionary dictionaryWithObjects:allUsers
...
+ (RLMSyncUser *)currentUser {
	NSArray *allUsers = [[RLMSyncManager sharedManager] _allUsers];
	if (allUsers.count > 1 &&
	    [NSSet setWithArray:[allUsers valueForKey:@"identity"]].count > 1) {
		@throw RLMException(@"+currentUser cannot be called if more that one "
		                    @"valid, logged-in user exists.");
	}
...
+ (void)logInWithCredentials:(RLMSyncCredentials *)credentials
	...
	if (!authServerURL) {
		@throw RLMException(@"A user cannot be logged in without specifying an "
	...
	RLMSyncCompletionBlock handler = ^(NSError *error, NSDictionary *json) {
		...
		RLMAuthResponseModel *model =
			[[RLMAuthResponseModel alloc] initWithDictionary:json
		...
		SyncUserIdentifier identity{
			model.refreshToken.tokenData.identity.UTF8String,
			authServerURL.absoluteString.UTF8String};
		auto sync_user = SyncManager::shared().get_user(
			identity, [model.refreshToken.token UTF8String]);
		if (!sync_user) {
			return completion(nil, make_auth_error_client_issue());
		}
		sync_user->set_is_admin(model.refreshToken.tokenData.isAdmin);
		return completion(
			[[RLMSyncUser alloc] initWithSyncUser:std::move(sync_user)], nil);
	};

	[RLMSyncAuthEndpoint
	 sendRequestToServer:authServerURL
	 JSON:json
	 timeout:timeout
...
- (RLMRealmConfiguration *)configurationWithURL:(NSURL *)url
	...
	auto syncConfig = [[RLMSyncConfiguration alloc]
	                   initWithUser:self
	...
	syncConfig.pinnedCertificateURL =
		RLMSyncManager.sharedManager
		.pinnedCertificatePaths[syncConfig.realmURL.host];
...
- (nullable RLMSyncSession *)sessionForURL:(NSURL *)url {
	...
	auto path = SyncManager::shared().path_for_realm(
		*_user, [url.absoluteString UTF8String]);
...
+ (void)requestEmailConfirmationForAuthServer:(NSURL *)serverURL
        userEmail:(NSString *)email
        completion:(RLMPasswordChangeStatusBlock)
        completion {
	[RLMSyncUpdateAccountEndpoint
	 sendRequestToServer:serverURL
	 JSON:@{
...
+ (void)confirmEmailForAuthServer:(NSURL *)serverURL
	...
	[RLMSyncUpdateAccountEndpoint
	 ...
	 JSON:@{
	         @"data" :
	         @{@"action" : @"confirm_email", @"token" : token}
	 }
	 completion:completion];
}
...
- (void)retrieveInfoForUser:(NSString *)providerUserIdentity
	...
	[RLMSyncGetUserInfoEndpoint
	 ...
	 completion:^(NSError *error, NSDictionary *json) {
	         ...
	         RLMUserResponseModel *model =
			 [[RLMUserResponseModel alloc] initWithDictionary:json];
	         if (!model) {
	                 return completion(nil, make_auth_error_bad_response(json));
		 }
...
namespace {
	NSError *checkUser(std::shared_ptr<SyncUser> const &user, NSString *msg) {
		...
		msg = [NSString
		       stringWithFormat:@"Permissions cannot be %@ using an invalid user.", msg];
		return [NSError errorWithDomain:RLMSyncPermissionErrorDomain
		        code:RLMSyncAuthErrorInvalidParameters
...
- (void)retrievePermissionsWithCallback:(RLMPermissionResultsBlock)callback {
        ...
        [RLMSyncGetPermissionsEndpoint
         ...
         completion:^(NSError *error, NSDictionary *json) {
                 ...
                 // FIXME: ROS currently gives duplicated results for 'all' due
                 // to an incorrect query
                 NSMutableSet *permissions = [NSMutableSet new];
                 for (NSDictionary *permission in json[@"permissions"]) {
                         // ROS reports the permission for __wildcardpermissions,
                         ...
                         [permissions
                          addObject:
                          [[RLMSyncPermission alloc]
                           initWithRealmPath:permission[@"path"]
                           identity:userId
                           accessLevel:
                           RLMSyncAccessLevelFromString(
...
- (void)applyPermission:(RLMSyncPermission *)permission
        ...
        if (permission.identity) {
                ...
                condition =
			@{@"metadataKey" : permission.key, @"metadataValue" : permission.value};
	}
        [RLMSyncApplyPermissionsEndpoint
         ...
         JSON:@{
                 ...
                 @"accessLevel" :
                 RLMSyncAccessLevelToString(permission.accessLevel)
	 }
         completion:callback];
}
        ...
        [RLMSyncGetPermissionOffersEndpoint
         ...
         completion:^(NSError *error, NSDictionary *json) {
                 ...
                 for (NSDictionary *offer in json[@"offers"]) {
                         ...
                         [offers
                          addObject:
                          [[RLMSyncPermissionOffer alloc]
                           initWithRealmPath:offer[@"realmPath"]
                           token:offer[@"token"]
                           expiresAt:expiresAt ?[formatter
                                                 dateFromString:
                                                 expiresAt]
                                                           : nil
                           createdAt:createdAt ?[formatter
                                                 dateFromString:
                                                 createdAt]
                                                           : nil
                           accessLevel:RLMSyncAccessLevelFromString(
				   offer[@"accessLevel"])]];
		 }
                 callback(offers, nil);
	 }];
...
- (NSURL *)urlForPath:(nullable NSString *)path {
		...
		[NSURLComponents componentsWithURL:self.authenticationServer
		...
		@throw RLMException(
			       @"The provided user's authentication server URL (%@) was not valid.",
			       self.authenticationServer);
...
+ (void)_performLoginForDirectAccessTokenCredentials:
        (RLMSyncCredentials *)credentials
        authServerURL:(NSURL *)serverURL
        completionBlock:
        (nonnull RLMUserCompletionBlock)
        completion {
        ...
        if (serverURL) {
                ...
                if (![scheme isEqualToString:@"http"] &&
                    ![scheme isEqualToString:@"https"]) {
                        @throw RLMException(
				       @"The Realm Object Server authentication URL provided for this user, "
				       @"\"%@\", "
				       @" is invalid. It must begin with http:// or https://.",
				       serverURL);
		}
                ...
                sync_user = SyncManager::shared().get_admin_token_user(
			[serverURL absoluteString].UTF8String, credentials.token.UTF8String,
			std::move(identity_string));
	} else {
                // Retrieve the user based on the identity.
                if (!identity) {
                        @throw RLMException(@"A direct access credential must specify either an "
                                            @"identity, a server URL, or both.");
		}
                sync_user = SyncManager::shared().get_admin_token_user_from_identity(
			identity.UTF8String, none, credentials.token.UTF8String);
	}
        if (!sync_user) {
...
+ (void)_performLoginForCustomRefreshTokenCredentials:
        (RLMSyncCredentials *)credentials
        authServerURL:(NSURL *)serverURL
        completionBlock:
        (nonnull RLMUserCompletionBlock)
        ...
        if (![scheme isEqualToString:@"http"] && ![scheme isEqualToString:@"https"]) {
                @throw RLMException(@"The Realm Object Server authentication URL provided "
                                    @"for this user, \"%@\", "
                                    @" is invalid. It must begin with http:// or https://.",
                                    serverURL);
	}
        ...
        NSString *identity = credentials.userInfo[kRLMSyncIdentityKey];
        SyncUserIdentifier identifier{identity.UTF8String,
                                      serverURL.absoluteString.UTF8String};

        std::shared_ptr<SyncUser> sync_user = SyncManager::shared().get_user(
		std::move(identifier), credentials.token.UTF8String);
        if (!sync_user) {
                completion(nil, make_auth_error_client_issue());
                return;
	}
