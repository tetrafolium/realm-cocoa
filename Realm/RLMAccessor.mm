...
T get(__unsafe_unretained RLMObjectBase *const obj, NSUInteger index) {
  ...
  return obj->_row.get<T>(
      obj->_info->objectSchema->persisted_properties[index].table_column);
}
...
template <typename Fn> auto translateError(Fn &&fn) {
  try {
    return fn();
...
[[gnu::noinline]] void setNull(realm::Row &row, size_t col) {
...
void setValue(__unsafe_unretained RLMObjectBase *const obj, NSUInteger colIndex,
  ...
  if (val->_row.get_table() !=
      obj->_row.get_table()->get_link_target(colIndex)) {
    @throw RLMException(
...
// array getter/setter
RLMArray *getArray(__unsafe_unretained RLMObjectBase *const obj,
...
void setValue(__unsafe_unretained RLMObjectBase *const obj, NSUInteger colIndex,
  ...
  realm::List list(obj->_realm->_realm, *obj->_row.get_table(), colIndex,
                   obj->_row.get_index());
...
getLinkingObjects(__unsafe_unretained RLMObjectBase *const obj,
  ...
  auto backlinkView = obj->_row.get_table()->get_backlink_view(
      obj->_row.get_index(), objectInfo.table(), linkingProperty->table_column);
...
id makeSetter(__unsafe_unretained RLMProperty *const prop) {
  ...
  return ^(__unsafe_unretained RLMObjectBase *const obj, ArgType val) {
    ...
    if (RLMObservationInfo *info = RLMGetObservationInfo(
            obj->_observationInfo, obj->_row.get_index(), *obj->_info)) {
...
id unmanagedSetter(RLMProperty *prop, const char *) {
  ...
  return ^(RLMObjectBase *obj, id<NSFastEnumeration> values) {
    ...
    if (prop.type == RLMPropertyTypeObject)
      ...
      ar = [[RLMArray alloc] initWithObjectType:prop.type
...
id RLMDynamicGet(__unsafe_unretained RLMObjectBase *const obj,
  ...
  return translateError([&] {
    return RLMCoerceToNil(
        o.get_property_value<id>(c, prop.columnName.UTF8String));
  });
}
...
RLMAccessorContext::RLMAccessorContext(RLMAccessorContext &parent,
    ...
    : _realm(parent._realm), _info(property.type == realm::PropertyType::Object
...
id RLMAccessorContext::propertyValue(
    __unsafe_unretained id const obj, size_t propIndex,
    __unsafe_unretained RLMProperty *const prop) {
  ...
  if ([obj isKindOfClass:_info.rlmObjectSchema.objectClass] && prop.swiftIvar) {
    if (prop.array) {
      return static_cast<RLMListBase *>(object_getIvar(obj, prop.swiftIvar))
          ._rlmArray;
    } else { // optional
      value = RLMGetOptional(
          static_cast<RLMOptionalBase *>(object_getIvar(obj, prop.swiftIvar)));
    }
  } else {
    // Property value from some object that's KVC-compatible
    value = RLMValidatedValueForProperty(
        obj,
...
template <>
realm::Timestamp RLMAccessorContext::unbox(__unsafe_unretained id const value,
                                           realm::CreatePolicy, size_t) {
  id v = RLMCoerceToNil(value);
  return RLMTimestampForNSDate(v);
}

template <>
bool RLMAccessorContext::unbox(__unsafe_unretained id const v,
                               realm::CreatePolicy, size_t) {
  return [v boolValue];
}
template <>
double RLMAccessorContext::unbox(__unsafe_unretained id const v,
                                 realm::CreatePolicy, size_t) {
  return [v doubleValue];
}
template <>
float RLMAccessorContext::unbox(__unsafe_unretained id const v,
                                realm::CreatePolicy, size_t) {
  return [v floatValue];
}
template <>
long long RLMAccessorContext::unbox(__unsafe_unretained id const v,
                                    realm::CreatePolicy, size_t) {
...
static auto to_optional(__unsafe_unretained id const value, Fn &&fn) {
  id v = RLMCoerceToNil(value);
  return v && v != NSNull.null ? realm::util::make_optional(fn(v))
                               : realm::util::none;
}

template <>
realm::util::Optional<bool>
RLMAccessorContext::unbox(__unsafe_unretained id const v, realm::CreatePolicy,
                          size_t) {
  return to_optional(
      v, [&](__unsafe_unretained id v) { return (bool)[v boolValue]; });
}
template <>
realm::util::Optional<double>
RLMAccessorContext::unbox(__unsafe_unretained id const v, realm::CreatePolicy,
                          size_t) {
  return to_optional(v,
                     [&](__unsafe_unretained id v) { return [v doubleValue]; });
}
template <>
realm::util::Optional<float>
RLMAccessorContext::unbox(__unsafe_unretained id const v, realm::CreatePolicy,
                          size_t) {
  return to_optional(v,
                     [&](__unsafe_unretained id v) { return [v floatValue]; });
}
template <>
realm::util::Optional<int64_t>
RLMAccessorContext::unbox(__unsafe_unretained id const v, realm::CreatePolicy,
                          size_t) {
  return to_optional(
      v, [&](__unsafe_unretained id v) { return [v longLongValue]; });
}

template <>
realm::RowExpr RLMAccessorContext::unbox(__unsafe_unretained id const v,
  ...
  if (!link) {
    ...
    return RLMCreateObjectInRealmWithValue(
               _realm, _info.rlmObjectSchema.className, v, policy)
        ->_row;
  }

  if (link.isInvalidated) {
    if (create) {
      @throw RLMException(@"Adding a deleted or invalidated object to a Realm "
                          @"is not permitted");
    } else {
  ...
  if (![link->_objectSchema.className
          isEqualToString:_info.rlmObjectSchema.className]) {
    if (create && !_promote_existing)
      return RLMCreateObjectInRealmWithValue(
                 _realm, _info.rlmObjectSchema.className, link, policy)
          ->_row;
    return link->_row;
  }
  ...
  if (!link->_realm) {
    ...
    if (_promote_existing)
      @throw RLMException(@"Object is already managed by another Realm. Use "
                          @"create instead to copy it into this Realm.");
    return RLMCreateObjectInRealmWithValue(
               _realm, _info.rlmObjectSchema.className, v, policy)
        ->_row;
  }
...
void RLMAccessorContext::will_change(realm::Row const &row,
...
RLMOptionalId
RLMAccessorContext::default_value_for_property(realm::ObjectSchema const &,
                                               realm::Property const &prop) {
  return RLMOptionalId { defaultValue(@(prop.name.c_str())) };
}

bool RLMAccessorContext::is_same_list(realm::List const &list,
                                      __unsafe_unretained id const v) const
    noexcept {
