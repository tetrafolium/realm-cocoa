...
typedef void (^RLMSyncCompletionBlock)(NSError *_Nullable,
...
FOUNDATION_EXTERN uint8_t RLMGetComputedPermissions(RLMRealm *realm,
                                                    id _Nullable object);
...
#define RLM_SYNC_PARSE_STRING_OR_ABORT(json_macro_val, key_macro_val,          \
                                       prop_macro_val)                         \
  {                                                                            \
...
#define RLM_SYNC_PARSE_OPTIONAL_STRING(json_macro_val, key_macro_val,          \
                                       prop_macro_val)                         \
  {                                                                            \
    id data = json_macro_val[key_macro_val];                                   \
    if (![data isKindOfClass:[NSString class]]) {                              \
...
#define RLM_SYNC_PARSE_OPTIONAL_BOOL(json_macro_val, key_macro_val,            \
                                     prop_macro_val)                           \
  {                                                                            \
    id data = json_macro_val[key_macro_val];                                   \
...
#define RLM_SYNC_PARSE_DOUBLE_OR_ABORT(json_macro_val, key_macro_val,          \
                                       prop_macro_val)                         \
  {                                                                            \
    id data = json_macro_val[key_macro_val];                                   \
    if (![data isKindOfClass:[NSNumber class]]) {                              \
...
#define RLM_SYNC_PARSE_MODEL_OR_ABORT(json_macro_val, key_macro_val,           \
                                      class_macro_val, prop_macro_val)         \
  {                                                                            \
...
/// A macro to build an array of sub-models out of a JSON dictionary, or return
/// nil.
#define RLM_SYNC_PARSE_MODEL_ARRAY_OR_ABORT(json_macro_val, key_macro_val,     \
                                            class_macro_val, prop_macro_val)   \
  {                                                                            \
    ...
    for (id value in jsonArray) {                                              \
      ...
      if ([value isKindOfClass:[NSDictionary class]]) {                        \
        next = [[class_macro_val alloc] initWithDictionary:value];             \
      }                                                                        \
      if (!next) {                                                             \
        return nil;                                                            \
      }                                                                        \
...
#define RLM_SYNC_PARSE_OPTIONAL_MODEL(json_macro_val, key_macro_val,           \
                                      class_macro_val, prop_macro_val)         \
  {                                                                            \
    id model;                                                                  \
    id raw = json_macro_val[key_macro_val];                                    \
    if (![raw isKindOfClass:[NSDictionary class]]) {                           \
      ...
      model = [[class_macro_val alloc] initWithDictionary:raw];                \
    }                                                                          \
    self.prop_macro_val = model;                                               \
  }\
