...
- (NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)state
	...
	return [self._rlmArray countByEnumeratingWithState:state
	        objects:buffer
	        count:len];
}
...
- (void)addObserver:(id)observer
	...
	RLMEnsureArrayObservationInfo(_observationInfo, keyPath, self._rlmArray,
	                              self);
...
- (RLMResults *)results {
	...
	auto backlinkView = _row.get_table()->get_backlink_view(
		_row.get_index(), objectInfo.table(), linkingProperty->table_column);
...
- (RLMObjectBase *)parent {
	RLMObjectBase *obj =
		RLMCreateManagedAccessor(_info->rlmObjectSchema.accessorClass, _info);
	obj->_row = _row;
	return obj;
}
@end
