...
/// A type representing the unique identifier of a Realm Object Server identity
/// provider.
typedef NSString *RLMIdentityProvider RLM_EXTENSIBLE_STRING_ENUM;

/// The debug identity provider, which accepts any token string and creates a
/// user associated with that token if one does not yet exist. Not enabled for
/// Realm Object Server configured for production.
extern RLMIdentityProvider const RLMIdentityProviderDebug;

/// The username/password identity provider. User accounts are handled by the
/// Realm Object Server directly without the involvement of a third-party
/// identity provider.
...
extern RLMIdentityProvider const RLMIdentityProviderNickname
    __deprecated_msg("Use RLMIdentityProviderUsernamePassword instead");
...
/// An opaque credentials token containing information that uniquely identifies
/// a Realm Object Server user.
...
/// A dictionary containing additional pertinent information. In most cases this
...
/**
   Construct and return credentials from a Realm Object Server username and
   password.
 */
...
+ (instancetype)credentialsWithNickname:(NSString *)nickname
    ...
    __deprecated_msg("Use +credentialsWithUsername instead");
...
/**
   Construct and return special credentials representing a token issued by an
   external entity that will be used instead of a ROS refresh token.

   @discussion Unlike other types of credentials, CustomRefreshToken credentials
   do not participate in the regular login flow, because they take place of the
   refresh tokens login produces. Instead, the token will be used to authorize
   each server operation such as opening a Realm or editing permissions. When
   the token expires the SyncUser object will still be valid, but server
   operations will fail until the token is updated.

   @remark ROS must be configured to accept the external issuing identity as a
   refresh token validator.

   @warning The values of @c identity and @c isAdmin are used for client-side
   validation only. The server will compute their values based on the token
   string and the token validator configuration, but it's important for  correct
   functioning that the values here match the server.
 */
...
/**
   Construct and return credentials with a custom token string, identity
   provider string, and optional user info. In most cases, the convenience
   initializers should be used instead.
 */
...
- (instancetype)init __attribute__((
    unavailable("RLMSyncCredentials cannot be created directly")));
