...
realm::Table *RLMClassInfo::table() const {
  if (!m_table) {
    m_table =
        ObjectStore::table_for_object_type(realm.group, objectSchema->name)
            .get();
  }
  return m_table;
}

RLMProperty *RLMClassInfo::propertyForTableColumn(NSUInteger col) const
    noexcept {
...
RLMClassInfo &RLMClassInfo::linkTargetType(size_t propertyIndex) {
  ...
  m_linkTargets[propertyIndex] =
      &realm->_info[rlmObjectSchema.properties[propertyIndex].objectClassName];
  return *m_linkTargets[propertyIndex];
}
...
RLMClassInfo &RLMSchemaInfo::operator[](NSString *name) {
  ...
  if (it == m_objects.end()) {
    @throw RLMException(
        @"Object type '%@' is not managed by the Realm. "
