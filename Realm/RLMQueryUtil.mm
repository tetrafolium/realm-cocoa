...
template <CFStringCompareFlags options> struct Equal {
  ...
  bool operator()(StringData v1, StringData v2, bool v1_null,
                  bool v2_null) const {
...
Table &get_table(Group &group, RLMObjectSchema *objectSchema) {
  return *ObjectStore::table_for_object_type(
      group, objectSchema.objectName.UTF8String);
}
...
public:
  ColumnReference(Query &query, Group &group, RLMSchema *schema,
      ...
      : m_links(links), m_property(property), m_schema(schema), m_group(&group),
        m_query(&query), m_table(query.get_table().get()) {
  ...
  auto resolve(SubQuery &&... subquery) const {
    ...
    if (type() != RLMPropertyTypeLinkingObjects) {
      return m_table->template column<T>(index(),
                                         std::forward<SubQuery>(subquery)...);
    } else {
...
private:
  ...
  auto resolve_backlink(SubQuery &&... subquery) const {
    ...
    return do_resolve_backlink<T>(std::is_same<T, Link>(),
                                  std::forward<SubQuery>(subquery)...);
  }
  ...
  auto do_resolve_backlink(std::true_type, SubQuery &&... subquery) const {
    return with_link_origin(m_property, [&](Table &table, size_t col) {
      return m_table->template column<T>(table, col,
                                         std::forward<SubQuery>(subquery)...);
    });
  }
  ...
  template <typename Func> Table &walk_link_chain(Func &&func) const {
    ...
    for (const auto &link : m_links) {
      if (link.type != RLMPropertyTypeLinkingObjects) {
        ...
        with_link_origin(
            link, [&](Table &link_origin_table, size_t link_origin_column) {
  ...
  auto with_link_origin(RLMProperty *prop, Func &&func) const {
    ...
    size_t link_origin_column =
        link_origin_table.get_column_index(column_name.UTF8String);
    return func(link_origin_table, link_origin_column);
  }

  void set_link_chain_on_table() const {
    walk_link_chain(
        [&](Table &current_table, size_t column, RLMPropertyType type) {
          if (type == RLMPropertyTypeLinkingObjects) {
...
public:
  ...
  CollectionOperation(Type type, ColumnReference link_column,
    ...
    RLMPrecondition(m_link_column.property().array, @"Invalid predicate",
                    @"Collection operation can only be applied to a property "
                    @"of type RLMArray.");

    switch (m_type) {
    case Count:
      RLMPrecondition(!m_column, @"Invalid predicate",
    ...
    case Sum:
    case Average:
      RLMPrecondition(m_column && RLMPropertyTypeIsNumeric(m_column->type()),
                      @"Invalid predicate",
  ...
  CollectionOperation(NSString *operationName, ColumnReference link_column,
      ...
      : CollectionOperation(type_for_name(operationName),
                            std::move(link_column), std::move(column)) {}
  ...
  void validate_comparison(id value) const {
    ...
    case Sum:
      RLMPrecondition(
          RLMIsObjectValidForProperty(value, m_column->property()),
          @"Invalid operand",
  ...
  void validate_comparison(const ColumnReference &column) const {
    switch (m_type) {
    case Count:
      RLMPrecondition(RLMPropertyTypeIsNumeric(column.type()),
                      @"Invalid operand",
                      @"%@ can only be compared with a numeric value.",
                      name_for_type(m_type));
      break;
...
private:
  static Type type_for_name(NSString *name) {
    ...
    @throw RLMPredicateException(
        @"Invalid predicate", @"Unsupported collection operation '%@'", name);
  }
...
public:
  ...
  void apply_collection_operator_expression(RLMObjectSchema *desc,
                                            NSString *keyPath, id value,
                                            NSComparisonPredicate *pred);
  void apply_value_expression(RLMObjectSchema *desc, NSString *keyPath,
                              id value, NSComparisonPredicate *pred);
  void apply_column_expression(RLMObjectSchema *desc, NSString *leftKeyPath,
                               NSString *rightKeyPath,
                               NSComparisonPredicate *predicate);
  void apply_subquery_count_expression(RLMObjectSchema *objectSchema,
                                       NSExpression *subqueryExpression,
                                       NSPredicateOperatorType operatorType,
                                       NSExpression *right);
  void apply_function_subquery_expression(RLMObjectSchema *objectSchema,
  ...
  void add_string_constraint(NSPredicateOperatorType operatorType,
                             ...
                             Columns<String> &&column, T value);

  void add_string_constraint(NSPredicateOperatorType operatorType,
                             NSComparisonPredicateOptions predicateOptions,
                             StringData value, Columns<String> &&column);

  template <typename L, typename R>
  void
  add_constraint(RLMPropertyType type, NSPredicateOperatorType operatorType,
                 NSComparisonPredicateOptions predicateOptions, L lhs, R rhs);
  template <typename... T>
  void
  do_add_constraint(RLMPropertyType type, NSPredicateOperatorType operatorType,
                    NSComparisonPredicateOptions predicateOptions, T... values);
  void do_add_constraint(RLMPropertyType, NSPredicateOperatorType,
                         NSComparisonPredicateOptions, id, realm::null);

  void add_between_constraint(const ColumnReference &column, id value);

  void add_binary_constraint(NSPredicateOperatorType operatorType,
                             const ColumnReference &column, BinaryData value);
  void add_binary_constraint(NSPredicateOperatorType operatorType,
                             const ColumnReference &column, id value);
  void add_binary_constraint(NSPredicateOperatorType operatorType,
                             const ColumnReference &column, null);
  void add_binary_constraint(NSPredicateOperatorType operatorType, id value,
                             const ColumnReference &column);
  void add_binary_constraint(NSPredicateOperatorType, const ColumnReference &,
  ...
  template <typename T>
  void add_link_constraint(NSPredicateOperatorType operatorType, T obj,
                           const ColumnReference &column);
  ...
  void add_collection_operation_constraint(RLMPropertyType propertyType,
                                           NSPredicateOperatorType operatorType,
                                           T... values);
  template <typename... T>
  void
  add_collection_operation_constraint(NSPredicateOperatorType operatorType,
                                      CollectionOperation collectionOperation,
                                      T... values);

  CollectionOperation collection_operation_from_key_path(RLMObjectSchema *desc,
                                                         NSString *keyPath);
  ColumnReference column_reference_from_key_path(RLMObjectSchema *objectSchema,
                                                 NSString *keyPath,
                                                 bool isAggregate);
...
void QueryBuilder::add_numeric_constraint(RLMPropertyType datatype,
  ...
  default:
    @throw RLMPredicateException(
        @"Invalid operator type", @"Operator '%@' not supported for type %@",
        operatorName(operatorType), RLMTypeToString(datatype));
  }
}

template <typename A, typename B>
void QueryBuilder::add_bool_constraint(NSPredicateOperatorType operatorType,
                                       A lhs, B rhs) {
  switch (operatorType) {
  case NSEqualToPredicateOperatorType:
    m_query.and_query(lhs == rhs);
...
void QueryBuilder::add_substring_constraint(const Columns<T> &value,
  ...
  m_query.and_query(const_cast<Columns<String> &>(value).size() != 0 &&
                    std::move(condition));
}
...
void QueryBuilder::add_string_constraint(
    NSPredicateOperatorType operatorType,
    NSComparisonPredicateOptions predicateOptions, Columns<String> &&column,
    T value) {
  bool caseSensitive = !(predicateOptions & NSCaseInsensitivePredicateOption);
  bool diacriticSensitive =
      !(predicateOptions & NSDiacriticInsensitivePredicateOption);
  ...
  case NSBeginsWithPredicateOperatorType: {
    using C =
        ContainsSubstring<kCFCompareDiacriticInsensitive | kCFCompareAnchored>;
...
void QueryBuilder::add_string_constraint(
    NSPredicateOperatorType operatorType,
    NSComparisonPredicateOptions predicateOptions, StringData value,
  ...
  default:
    @throw RLMPredicateException(
        @"Invalid operator type",
        @"Operator '%@' is not supported for string type with key path on "
...
void validate_and_extract_between_range(id value, RLMProperty *prop, id *from,
  ...
  *to = value_from_constant_expression_or_value(array.lastObject);
  RLMPrecondition(RLMIsObjectValidForProperty(*from, prop) &&
                      RLMIsObjectValidForProperty(*to, prop),
                  @"Invalid value",
...
void QueryBuilder::add_between_constraint(const ColumnReference &column,
  ...
  m_query.group();
  add_constraint(type, NSGreaterThanOrEqualToPredicateOperatorType, 0, column,
                 from);
...
void QueryBuilder::add_binary_constraint(NSPredicateOperatorType operatorType,
                                         const ColumnReference &column,
                                         BinaryData value) {
  RLMPrecondition(!column.has_links(), @"Unsupported operator",
...
void QueryBuilder::add_binary_constraint(NSPredicateOperatorType operatorType,
  ...
  default:
    @throw RLMPredicateException(
        @"Invalid operator type",
        @"Operator '%@' is not supported for binary type with key path on "
        @"right side of operator",
        operatorName(operatorType));
  }
}

void QueryBuilder::add_binary_constraint(NSPredicateOperatorType,
                                         const ColumnReference &,
                                         const ColumnReference &) {
  @throw RLMPredicateException(
      @"Invalid predicate",
...
void QueryBuilder::add_link_constraint(NSPredicateOperatorType operatorType,
  ...
  RLMPrecondition(operatorType == NSEqualToPredicateOperatorType ||
                  ...
                  @"Invalid operator type",
                  @"Only 'Equal' and 'Not Equal' operators supported for "
                  @"object comparison");
...
template <typename T>
void QueryBuilder::add_link_constraint(NSPredicateOperatorType operatorType,
                                       T obj, const ColumnReference &column) {
  // Link constraints only support the equal-to and not-equal-to operators. The
  // order of operands is not important for those comparisons so we can delegate
  // to the other implementation.
  add_link_constraint(operatorType, column, obj);
}

void QueryBuilder::add_link_constraint(NSPredicateOperatorType,
                                       const ColumnReference &,
                                       const ColumnReference &) {
...
template <typename RequestedType> RequestedType convert(id value);

template <> Timestamp convert<Timestamp>(id value) {
  return RLMTimestampForNSDate(value);
}

template <> bool convert<bool>(id value) { return [value boolValue]; }

template <> Double convert<Double>(id value) { return [value doubleValue]; }

template <> Float convert<Float>(id value) { return [value floatValue]; }

template <> Int convert<Int>(id value) { return [value longLongValue]; }

template <> String convert<String>(id value) {
...
template <typename RequestedType> auto value_of_type(id value) {
...
void QueryBuilder::do_add_constraint(
  ...
  static_assert(sizeof...(T) == 2,
                "do_add_constraint accepts only two values as arguments");
  ...
  case RLMPropertyTypeDate:
    add_numeric_constraint(type, operatorType,
                           value_of_type<realm::Timestamp>(values)...);
    break;
  case RLMPropertyTypeDouble:
    add_numeric_constraint(type, operatorType,
                           value_of_type<Double>(values)...);
    break;
  case RLMPropertyTypeFloat:
...
void QueryBuilder::do_add_constraint(RLMPropertyType, NSPredicateOperatorType,
                                     NSComparisonPredicateOptions, id,
                                     realm::null) {
  // This is not actually reachable as this case is caught earlier, but this
  // overload is needed for the code to compile
  @throw RLMPredicateException(@"Invalid predicate expressions",
                               @"Predicate expressions must compare a keypath "
                               @"and another keypath or a constant value");
}

bool is_nsnull(id value) { return !value || value == NSNull.null; }

template <typename T> bool is_nsnull(T) { return false; }

template <typename L, typename R>
void QueryBuilder::add_constraint(RLMPropertyType type,
                                  NSPredicateOperatorType operatorType,
                                  NSComparisonPredicateOptions predicateOptions,
                                  L lhs, R rhs) {
...
KeyPath key_path_from_string(RLMSchema *schema, RLMObjectSchema *objectSchema,
  ...
  do {
    ...
    if (end != NSNotFound) {
      RLMPrecondition(property.type == RLMPropertyTypeObject ||
...
ColumnReference QueryBuilder::column_reference_from_key_path(
    RLMObjectSchema *objectSchema, NSString *keyPathString, bool isAggregate) {
  ...
  if (isAggregate && !keyPath.containsToManyRelationship) {
    @throw RLMPredicateException(@"Invalid predicate",
                                 @"Aggregate operations can only be used on "
                                 @"key paths that include an array property");
  } else if (!isAggregate && keyPath.containsToManyRelationship) {
    @throw RLMPredicateException(@"Invalid predicate",
                                 @"Key paths that include an array property "
                                 @"must use aggregate operations");
  }

  return ColumnReference(m_query, m_group, m_schema, keyPath.property,
                         std::move(keyPath.links));
}

void validate_property_value(
    const ColumnReference &column, __unsafe_unretained id const value,
  ...
  if (prop.array) {
    RLMPrecondition(
        [RLMObjectBaseObjectSchema(RLMDynamicCast<RLMObjectBase>(value))
                .className isEqualToString:prop.objectClassName],
        @"Invalid value", err, prop.objectClassName, keyPath,
        objectSchema.className, value);
  } else {
    RLMPrecondition(RLMIsObjectValidForProperty(value, prop), @"Invalid value",
                    err, RLMTypeToString(prop.type), keyPath,
                    objectSchema.className, value);
  }
  if (RLMObjectBase *obj = RLMDynamicCast<RLMObjectBase>(value)) {
    RLMPrecondition(!obj->_row.is_attached() ||
                        &column.group() == &obj->_realm.group,
...
template <>
struct ValueOfTypeWithCollectionOperationHelper<Int,
...
#define VALUE_OF_TYPE_WITH_COLLECTION_OPERATOR_HELPER(OperationType, function) \
  ...
  struct ValueOfTypeWithCollectionOperationHelper<T, OperationType> {          \
    static auto convert(const CollectionOperation &operation) {                \
      ...
      auto targetColumn =                                                      \
          operation.link_column().resolve<Link>().template column<T>(          \
              operation.column().index());                                     \
      return targetColumn.function();                                          \
    }                                                                          \
  }

VALUE_OF_TYPE_WITH_COLLECTION_OPERATOR_HELPER(CollectionOperation::Minimum,
                                              min);
VALUE_OF_TYPE_WITH_COLLECTION_OPERATOR_HELPER(CollectionOperation::Maximum,
                                              max);
VALUE_OF_TYPE_WITH_COLLECTION_OPERATOR_HELPER(CollectionOperation::Sum, sum);
VALUE_OF_TYPE_WITH_COLLECTION_OPERATOR_HELPER(CollectionOperation::Average,
                                              average);
...
auto value_of_type_with_collection_operation(CollectionOperation operation) {
  using helper =
      ValueOfTypeWithCollectionOperationHelper<Requested, OperationType>;
  return helper::convert(operation);
}

template <CollectionOperation::Type Operation, typename... T>
void QueryBuilder::add_collection_operation_constraint(
    RLMPropertyType propertyType, NSPredicateOperatorType operatorType,
    T... values) {
  switch (propertyType) {
  case RLMPropertyTypeInt:
    add_numeric_constraint(
        propertyType, operatorType,
        value_of_type_with_collection_operation<Int, Operation>(values)...);
    break;
  case RLMPropertyTypeFloat:
    add_numeric_constraint(
        propertyType, operatorType,
        value_of_type_with_collection_operation<Float, Operation>(values)...);
    break;
  case RLMPropertyTypeDouble:
    add_numeric_constraint(
        propertyType, operatorType,
        value_of_type_with_collection_operation<Double, Operation>(values)...);
    break;
  default:
...
void QueryBuilder::add_collection_operation_constraint(
  ...
  static_assert(sizeof...(T) == 2, "add_collection_operation_constraint "
                                   "accepts only two values as arguments");

  switch (collectionOperation.type()) {
  case CollectionOperation::Count:
    add_numeric_constraint(RLMPropertyTypeInt, operatorType,
                           value_of_type_with_collection_operation<
                               Int, CollectionOperation::Count>(values)...);
    break;
  case CollectionOperation::Minimum:
    add_collection_operation_constraint<CollectionOperation::Minimum>(
        collectionOperation.column().type(), operatorType, values...);
    break;
...
NSString *get_collection_operation_name_from_key_path(NSString *keyPath,
  ...
  if (at.location == NSNotFound || at.location >= keyPath.length - 1) {
    @throw RLMPredicateException(@"Invalid key path",
                                 @"'%@' is not a valid key path'", keyPath);
  }
  ...
  NSRange trailingKeyRange =
      [keyPath rangeOfString:@"."
                     options:0
                       range:{at.location, keyPath.length - at.location}
                      locale:nil];
  ...
  *leadingKeyPath = [keyPath substringToIndex:at.location - 1];
  if (trailingKeyRange.location == NSNotFound) {
    ...
    *trailingKey = [keyPath substringFromIndex:trailingKeyRange.location + 1];
    return [keyPath substringWithRange:{at.location, trailingKeyRange.location -
                                                         at.location}];
  }
}

CollectionOperation
QueryBuilder::collection_operation_from_key_path(RLMObjectSchema *desc,
                                                 NSString *keyPath) {
  NSString *leadingKeyPath;
  NSString *trailingKey;
  NSString *collectionOperationName =
      get_collection_operation_name_from_key_path(keyPath, &leadingKeyPath,
                                                  &trailingKey);

  ColumnReference linkColumn =
      column_reference_from_key_path(desc, leadingKeyPath, true);
  util::Optional<ColumnReference> column;
  if (trailingKey) {
    RLMPrecondition(
        [trailingKey rangeOfString:@"."].location == NSNotFound,
        @"Invalid key path",
        @"Right side of collection operator may only have a single level key");
    NSString *fullKeyPath =
        [leadingKeyPath stringByAppendingFormat:@".%@", trailingKey];
...
void QueryBuilder::apply_collection_operator_expression(
    RLMObjectSchema *desc, NSString *keyPath, id value,
    NSComparisonPredicate *pred) {
  CollectionOperation operation =
      collection_operation_from_key_path(desc, keyPath);
  operation.validate_comparison(value);

  if (pred.leftExpression.expressionType == NSKeyPathExpressionType) {
    add_collection_operation_constraint(pred.predicateOperatorType, operation,
                                        operation, value);
  } else {
    add_collection_operation_constraint(pred.predicateOperatorType, operation,
                                        value, operation);
  ...
  if (pred.predicateOperatorType == NSInPredicateOperatorType) {
    process_or_group(m_query, value, [&](id item) {
      ...
      validate_property_value(
          column, normalized,
          @"Expected object of type %@ in IN clause for property '%@' on "
          @"object of type '%@', but received: %@",
          desc, keyPath);
      add_constraint(column.type(), NSEqualToPredicateOperatorType,
                     pred.options, column, normalized);
    });
    return;
  }

  validate_property_value(column, value,
                          @"Expected object of type %@ for property '%@' on "
                          @"object of type '%@', but received: %@",
                          desc, keyPath);
  if (pred.leftExpression.expressionType == NSKeyPathExpressionType) {
    add_constraint(column.type(), pred.predicateOperatorType, pred.options,
...
void QueryBuilder::apply_column_expression(RLMObjectSchema *desc,
  ...
  if (left_key_path_contains_collection_operator &&
    ...
    @throw RLMPredicateException(
        ...
        @"Key paths including aggregate operations cannot be compared with "
        @"other aggregate operations.");
  }

  if (left_key_path_contains_collection_operator) {
    CollectionOperation left =
        collection_operation_from_key_path(desc, leftKeyPath);
    ColumnReference right =
        column_reference_from_key_path(desc, rightKeyPath, false);
    left.validate_comparison(right);
    add_collection_operation_constraint(predicate.predicateOperatorType, left,
                                        left, std::move(right));
    return;
  }
  if (right_key_path_contains_collection_operator) {
    ColumnReference left =
        column_reference_from_key_path(desc, leftKeyPath, false);
    CollectionOperation right =
        collection_operation_from_key_path(desc, rightKeyPath);
    right.validate_comparison(left);
    add_collection_operation_constraint(predicate.predicateOperatorType, right,
                                        std::move(left), right);
    return;
  }
  ...
  bool isAny = false;
  ColumnReference left =
      column_reference_from_key_path(desc, leftKeyPath, isAny);
  ColumnReference right =
      column_reference_from_key_path(desc, rightKeyPath, isAny);
  ...
  // (tautology)
  add_constraint(left.type(), predicate.predicateOperatorType,
                 predicate.options, std::move(left), std::move(right));
}
...
void QueryBuilder::apply_subquery_count_expression(
  ...
  if (right.expressionType != NSConstantValueExpressionType ||
    ...
    @throw RLMPredicateException(@"Invalid predicate expression",
                                 @"SUBQUERY(…).@count is only supported when "
                                 @"compared with a constant number.");
  }
  int64_t value = [right.constantValue integerValue];

  ColumnReference collectionColumn = column_reference_from_key_path(
      objectSchema, [subqueryExpression.collection keyPath], true);
...
void QueryBuilder::apply_function_subquery_expression(
    RLMObjectSchema *objectSchema, NSExpression *functionExpression,
    NSPredicateOperatorType operatorType, NSExpression *right) {
  if (![functionExpression.function isEqualToString:@"valueForKeyPath:"] ||
  ...
  if ([keyPathExpression.keyPath isEqualToString:@"@count"]) {
    apply_subquery_count_expression(objectSchema, functionExpression.operand,
                                    operatorType, right);
  } else {
    @throw RLMPredicateException(
        @"Invalid predicate",
        @"SUBQUERY is only supported when immediately followed by .@count that "
        @"is compared with a constant number.");
  }
}
...
void QueryBuilder::apply_function_expression(
  ...
  if (functionExpression.operand.expressionType == NSSubqueryExpressionType) {
    apply_function_subquery_expression(objectSchema, functionExpression,
                                       operatorType, right);
  } else {
    @throw RLMPredicateException(@"Invalid predicate",
...
void QueryBuilder::apply_predicate(NSPredicate *predicate,
  ...
  if ([predicate isMemberOfClass:[NSCompoundPredicate class]]) {
    ...
    if (compp.comparisonPredicateModifier == NSAnyPredicateModifier) {
      // for ANY queries
      RLMPrecondition(exp1Type == NSKeyPathExpressionType &&
                          exp2Type == NSConstantValueExpressionType,
                      @"Invalid predicate",
                      @"Predicate with ANY modifier must compare a KeyPath "
                      @"with RLMArray with a value");
    }
    ...
    if (compp.predicateOperatorType == NSBetweenPredicateOperatorType ||
      ...
      if (exp1Type == NSKeyPathExpressionType &&
        ...
        if (compp.predicateOperatorType == NSBetweenPredicateOperatorType) {
          @throw RLMPredicateException(
              @"Invalid predicate",
              @"Predicate with BETWEEN operator must compare a KeyPath with an "
              @"aggregate with two values");
        } else if (compp.predicateOperatorType == NSInPredicateOperatorType) {
          @throw RLMPredicateException(@"Invalid predicate",
                                       @"Predicate with IN operator must "
                                       @"compare a KeyPath with an aggregate");
        }
      }
    }
    ...
    if (exp1Type == NSKeyPathExpressionType &&
      ...
      @throw RLMPredicateException(
          ...
          @"Predicate expressions must compare a keypath and another keypath "
          @"or a constant value");
    }
