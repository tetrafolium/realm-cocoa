...
- (RLMManagedArray *)
	...
	return [self initWithList:realm::List(realm->_realm, *row.get_table(), col,
	                                      row.get_index())
...
void RLMEnsureArrayObservationInfo(std::unique_ptr<RLMObservationInfo> &info,
	...
	if (!info && array.class == [RLMManagedArray class]) {
		...
		info = std::make_unique<RLMObservationInfo>(
			*lv->_ownerInfo, lv->_backingList.get_origin_row_index(), observed);
	}
}
...
// validation helpers
//
[[gnu::noinline]] [[noreturn]] static void
throwError(__unsafe_unretained RLMManagedArray *const ar,
           NSString *aggregateMethod) {
	try {
		throw;
		...
		@throw RLMException(@"RLMArray has been invalidated or the containing "
		...
		if (ar->_backingList.get_type() == realm::PropertyType::Object) {
			@throw RLMException(
				       @"%@: is not supported for %s%s property '%s'.", aggregateMethod,
				       string_for_property_type(e.property_type),
				       is_nullable(e.property_type) ? "?" : "", e.column_name.data());
		}
...
static void changeArray(__unsafe_unretained RLMManagedArray *const ar,
                        ...
                        IndexSetFactory &&is) {
	translateErrors([&] {
		ar->_backingList.verify_in_transaction();
	});
	RLMObservationInfo *info = RLMGetObservationInfo(
		ar->_observationInfo.get(), ar->_backingList.get_origin_row_index(),
		*ar->_ownerInfo);
	if (info) {
...
static void RLMInsertObject(RLMManagedArray *ar, id object, NSUInteger index) {
	...
	if (index == NSUIntegerMax) {
		index = translateErrors([&] {
			return ar->_backingList.size();
		});
	}
...
- (void)setValue:(id)value forKey:(NSString *)key {
	if ([key isEqualToString:@"self"]) {
		...
		RLMArrayValidateMatchingObjectType(self, value);
		translateErrors([&] {
			_backingList.verify_in_transaction();
		});
...
- (size_t)columnForProperty:(NSString *)propertyName {
	...
	if (![propertyName isEqualToString:@"self"]) {
		@throw RLMException(@"Arrays of '%@' can only be aggregated on \"self\"",
...
- (id)minOfProperty:(NSString *)property {
	...
	auto value = translateErrors(
		self, [&] {
		return _backingList.min(column);
	}, @"minOfProperty");
...
- (id)maxOfProperty:(NSString *)property {
	...
	auto value = translateErrors(
		self, [&] {
		return _backingList.max(column);
	}, @"maxOfProperty");
	return value ? RLMMixedToObjc(*value) : nil;
}
...
- (id)sumOfProperty:(NSString *)property {
	size_t column = [self columnForProperty:property];
	return RLMMixedToObjc(translateErrors(
				      self, [&] {
		return _backingList.sum(column);
	}, @"sumOfProperty"));
}
...
- (id)averageOfProperty:(NSString *)property {
	...
	auto value = translateErrors(
		self, [&] {
		return _backingList.average(column);
	}, @"averageOfProperty");
...
- (RLMResults *)objectsWithPredicate:(NSPredicate *)predicate {
	...
	auto results =
		translateErrors([&] {
		return _backingList.filter(std::move(query));
	});
...
- (realm::TableView)tableView {
	return translateErrors([&] {
		return _backingList.get_query();
	}).find_all();
}

- (RLMFastEnumerator *)fastEnumerator {
	return translateErrors([&] {
		return [[RLMFastEnumerator alloc] initWithList:_backingList
		        collection:self
...
- (std::unique_ptr<realm::ThreadSafeReferenceBase>)makeThreadSafeReference {
	auto list_reference =
		_realm->_realm->obtain_thread_safe_reference(_backingList);
	return std::make_unique<realm::ThreadSafeReference<realm::List> >(
		std::move(list_reference));
}
...
+ (instancetype)
        objectWithThreadSafeReference:
        (std::unique_ptr<realm::ThreadSafeReferenceBase>)reference
        metadata:(RLMManagedArrayHandoverMetadata *)metadata
        realm:(RLMRealm *)realm {
	REALM_ASSERT_DEBUG(
		dynamic_cast<realm::ThreadSafeReference<realm::List> *>(reference.get()));
	auto list_reference =
		static_cast<realm::ThreadSafeReference<realm::List> *>(reference.get());

	auto list =
		realm->_realm->resolve_thread_safe_reference(std::move(*list_reference));
	if (!list.is_valid()) {
		return nil;
	}
	RLMClassInfo *parentInfo = &realm->_info[metadata.parentClassName];
	return [[RLMManagedArray alloc]
	        initWithList:std::move(list)
