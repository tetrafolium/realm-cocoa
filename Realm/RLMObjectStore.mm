...
static_assert(RLMUpdatePolicyError ==
                  static_cast<int>(CreatePolicy::ForceCreate),
              "");
static_assert(RLMUpdatePolicyUpdateAll ==
                  static_cast<int>(CreatePolicy::UpdateAll),
              "");
static_assert(RLMUpdatePolicyUpdateChanged ==
                  static_cast<int>(CreatePolicy::UpdateModified),
              "");
...
static inline void
RLMVerifyInWriteTransaction(__unsafe_unretained RLMRealm *const realm) {
  RLMVerifyRealmRead(realm);
  // if realm is not writable throw
  if (!realm.inWriteTransaction) {
    @throw RLMException(@"Can only add, remove, or create objects in a Realm "
                        @"in a write transaction - call beginWriteTransaction "
                        @"on an RLMRealm instance first.");
  }
}

void RLMInitializeSwiftAccessorGenerics(
    __unsafe_unretained RLMObjectBase *const object) {
  ...
  for (RLMProperty *prop in object->_objectSchema.swiftGenericProperties) {
    if (prop.type == RLMPropertyTypeLinkingObjects) {
      [prop.swiftAccessor initializeObject:(char *)(__bridge void *)object +
                                           ivar_getOffset(prop.swiftIvar)
                                    parent:object
                                  property:prop];
    } else if (prop.array) {
      id ivar = object_getIvar(object, prop.swiftIvar);
      RLMArray *array = [[RLMManagedArray alloc] initWithParent:object
                                                       property:prop];
...
void RLMAddObjectToRealm(__unsafe_unretained RLMObjectBase *const object,
  ...
  if (object->_realm) {
    ...
    // for differing realms users must explicitly create the object in the
    // second realm
  ...
  try {
    realm::Object::create(c, realm->_realm, *info.objectSchema, (id)object,
                          static_cast<CreatePolicy>(updatePolicy), -1,
                          &object->_row);
...
RLMObjectBase *RLMCreateObjectInRealmWithValue(RLMRealm *realm,
  ...
  if (updatePolicy != RLMUpdatePolicyError &&
      RLMIsObjectSubclass([value class])) {
    RLMObjectBase *obj = value;
    if (obj->_realm == realm &&
        [obj->_objectSchema.className isEqualToString:className]) {
      // This is a no-op if value is an RLMObject of the same type already
      // backed by the target realm.
      return value;
    }
  }
  ...
  if ([value isKindOfClass:[NSArray class]] &&
      [value count] > info.objectSchema->persisted_properties.size()) {
    @throw RLMException(
  ...
  try {
    object->_row =
        realm::Object::create(c, realm->_realm, *info.objectSchema, (id)value,
                              static_cast<realm::CreatePolicy>(updatePolicy))
            .row();
  } catch (std::exception const &e) {
    @throw RLMException(e);
  }
...
RLMResults *RLMGetObjects(__unsafe_unretained RLMRealm *const realm,
  ...
  if (predicate) {
    realm::Query query = RLMPredicateToQuery(predicate, info.rlmObjectSchema,
                                             realm.schema, realm.group);
    return [RLMResults
