...
/**
   `RLMArray` is the container type in Realm used to define to-many
 relationships.
 ...
 the array.
   ...
   When declaring an `RLMArray` property, the type must be marked as conforming
 to a protocol by the same name as the objects it should contain (see the
   `RLM_ARRAY_TYPE` macro). In addition, the property can be declared using
 ...
 are lazily created when accessed, or can be obtained by querying a Realm.
...
/**
   ...
   @param objects     An enumerable object such as `NSArray` or `RLMResults`
...
/**
   ...
   @param index1 The index of the object which should replace the object at
   index `index2`.
   @param index2 The index of the object which should replace the object at
   index `index1`.
 */
...
/**
   ...
   the array, which objects are in the results, or the order of the objects in
   the array.

   The `changes` parameter will be `nil` the first time the block is called.
   For each call after that, it will contain information about
   which rows in the array were added, removed or modified. If a write
   transaction did not modify any objects in the array, the block is not called
   at all. See the `RLMCollectionChange` documentation for information on how
   the changes are reported and an example of updating a `UITableView`.
...
- (RLMNotificationToken *)addNotificationBlock:
    (void (^)(RLMArray<RLMObjectType> *__nullable array,
