...
/**
   `RLMPropertyType` is an enumeration describing all property types supported
   in Realm models.
...
#pragma mark - Object types
    ...
    /**
       ...
       files with any-typed properties are still supported for migration
       purposes.
     */
...
typedef RLM_ERROR_ENUM(NSInteger, RLMError, RLMErrorDomain){
    ...
    /**
       ...
       This error can occur if the user does not have permission to open or
       create the specified file in the specified access mode when opening a
       Realm.
     */
    ...
    /** Denotes an error that occurs when attempting to open an incompatible
       ...
       of Realm and an automatic migration to the current version is not
       possible. When such an error occurs, the original file is moved to a
       backup location, and future attempts to open the synchronized Realm will
       result in a new file being created. If you wish to migrate any data from
       the backup Realm, you can open it using the provided Realm configuration.
     */
...
/**
   ...
   `-[RLMRealm refresh]` after doing some work. Refreshing the Realm is
   optional, but not refreshing the Realm may lead to large Realm files. This is
   because an extra copy of the data must be kept for the stale Realm.
 */
