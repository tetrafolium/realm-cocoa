...
void RLMObservationInfo::recordObserver(
	realm::Row &objectRow, RLMClassInfo *objectInfo,
	...
	if (prop && prop.array) {
		...
		RLMArray *array =
			[value isKindOfClass:[RLMListBase class]] ?[value _rlmArray] : value;
		array->_key = key;
		array->_parentObject = object;
	} else if (auto swiftIvar = prop.swiftIvar) {
		if (auto optional = RLMDynamicCast<RLMOptionalBase>(
			    object_getIvar(object, swiftIvar))) {
...
void RLMTrackDeletions(__unsafe_unretained RLMRealm *const realm,
	...
	// resulting link nullifications immediately before things are deleted and
	// nullified
	realm.group.set_cascade_notification_handler(
		...
		for (auto const &link : cs.links) {
		        ...
		        for (auto observer : *observers[table_ndx]) {
					    ...
					    link.origin_col_ndx) != type_LinkList) {
					...
					return c.info == observer && c.property == name;
		                ...
		                // are going away
		                auto linkview =
					observer->getRow().get_linklist(link.origin_col_ndx);
		                size_t start = 0, index;
		                while ((index = linkview->find(link.old_target_row_ndx, start)) !=
		                       realm::not_found) {
...
void forEach(realm::BindingContext::ObserverState const &state, Func &&func) {
	for (size_t i = 0, size = state.changes.size(); i < size; ++i) {
		if (state.changes[i].kind !=
		    realm::BindingContext::ColumnInfo::Kind::None) {
	...
	for (auto &table : schema) {
		for (auto info : table.second.observedObjects) {
			...
			if (!row.is_attached())
				continue;
			observers.push_back(
				{row.get_table()->get_index_in_group(), row.get_index(), info});
		}
	}
	...
	if (!observed.empty()) {
		...
		for (auto const &o : reverse(observed)) {
			forEach(o, [&](size_t i, auto const &change, RLMObservationInfo *info) {
				info->didChange(info->columnName(i), convert(change.kind),
				                convert(change.indices, indexes));
			});
		}
	}
