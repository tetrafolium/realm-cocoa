...
public:
  ...
  realm::Row const &getRow() const { return row; }
  ...
  bool isForRow(size_t ndx) const { return row && row.get_index() == ndx; }

  void recordObserver(realm::Row &row, RLMClassInfo *objectInfo,
...
// invoke the block, sending notifications for cascading deletes/link
// nullifications
