...
/**
   `RLMSyncSubscriptionState` is an enumeration representing the possible state
   of a sync subscription.
 */
typedef RLM_CLOSED_ENUM(NSInteger, RLMSyncSubscriptionState){
    ...
    /**
       The subscription has been processed by the server, and objects matching
       the subscription are now being synchronized to this client.
     */
...
/**
   `RLMSyncSubscription` represents a subscription to a set of objects in a
   synced Realm.
...
/**
   ...
   This value will be `nil` for subscriptions created with older versions of
   Realm which did not store the update date. Newly created subscriptions should
   always have a non-nil update date.

   The update date is the time when the subscription was last updated by a call
   to `-[RLMResults subscribeWithOptions:]`, and not when the set of objects
   which match the subscription last changed.
 */
...
/**
   ...
   subscription will be automatically removed the first time that any
   subscription is created, modified, or deleted after that time has elapsed.
...
/**
   ...
   Removing a subscription will delete all objects from the local Realm that
   were matched only by that subscription and not any remaining subscriptions.
   The deletion is performed by the server, and so has no immediate impact on
   the contents of the local Realm. If the device is currently offline, the
   removal will not be processed until the device returns online.

   Unsubscribing is an asynchronous operation and will not immediately remove
   the subscription from the Realm's list of subscriptions. Observe the state
   property to be notified of when the subscription has actually been removed.
 */
- (void)unsubscribe;
...
/**
   ...
   If this is set to YES, instead the existing subscription will be updated
   using the query and options from the new subscription. This only works if the
   new subscription is for the same type of objects as the existing
   subscription. Trying to overwrite a subscription with a subscription of a
   different type of objects will fail.
...
/**
   ...
   removed by calling `unsubscribe()`. Subscriptions can instead be made
   temporary by setting the time to live to how long the subscription should
   remain. After that time has elapsed the subscription will be automatically
   removed.
...
/**
   ...
   `limit` objects will be included. This respects the sort and distinct order
   of the query being subscribed to for the determination of what the "first"
   objects are.

   The limit does not count or apply to objects which are added indirectly due
   to being linked to by the objects in the subscription or due to being listed
   in `includeLinkingObjectProperties`. If the limit is larger than the number
   of objects which match the query, all objects will be included. A limit of
...
/**
   ...
   matches one object, every object which is reachable via links from that
   object are also included in the subscription.
...
/**
   ...
   `state` has transitioned from `RLMSyncSubscriptionStateCreating` to any of
   the other states.
...
/**
   ...
   still in the `Creating` state are not included, and calling this immediately
   after calling
   `-[RLMResults subscribe]` will typically not include that subscription.
   Similarly, because unsubscription happens asynchronously, this may continue
   to include subscriptions after `-[RLMSyncSubscription unsubscribe]` is called
   on them.
