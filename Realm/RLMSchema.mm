...
static RLMObjectSchema *RLMRegisterClass(Class cls) {
	...
	schema.unmanagedClass =
		RLMUnmanagedAccessorClassForObjectClass(schema.objectClass, schema);
...
static void RLMRegisterClassLocalNames(Class *classes, NSUInteger count) {
	for (NSUInteger i = 0; i < count; i++) {
		...
		// indicates it's a type, and C indicates it's a class.
			...
			@throw RLMException(@"RLMObject subclasses cannot be nested within other "
			                    @"declarations. Please move %@ to global scope.",
			                    className);
		}
...
- (RLMObjectSchema *)objectForKeyedSubscript:
	...
	if (!schema) {
		@throw RLMException(@"Object type '%@' not managed by the Realm",
		                    className);
	}
	return schema;
}
...
+ (instancetype)sharedSchema {
	@synchronized(s_localNameToClass) {
		...
		if (s_sharedSchemaState == SharedSchemaState::Initializing) {
			@throw RLMException(@"Illegal recursive call of +[%@ %@]. Note: "
			                    @"Properties of Swift `Object` classes must not be "
			                    @"prepopulated with queried results from a Realm.",
