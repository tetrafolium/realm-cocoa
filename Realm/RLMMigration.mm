...
- (void)enumerateObjects:(NSString *)className
  ...
  for (auto i = count; i > 0; --i) {
    auto index = i - 1;
    if (deletedObjects != _deletedObjectIndices.end() &&
        deletedObjects->second.contains(index)) {
      continue;
    }
    @autoreleasepool {
...
- (void)deleteObject:(RLMObject *)object {
  _deletedObjectIndices[object.objectSchema.className].add(
      object->_row.get_index());
}
...
- (void)deleteObjectsMarkedForDeletion {
  for (auto &objectType : _deletedObjectIndices) {
    TableRef table = ObjectStore::table_for_object_type(
        _realm.group, objectType.first.UTF8String);
    if (!table) {
      continue;
    }
    ...
    // not-yet-deleted indices
      ...
      for (auto it = std::make_reverse_iterator(indices.end()),
                end = std::make_reverse_iterator(indices.begin());
           it != end; ++it) {
...
- (void)renamePropertyForClass:(NSString *)className
  ...
  realm::ObjectStore::rename_property(_realm.group, *_schema,
                                      className.UTF8String, oldName.UTF8String,
                                      newName.UTF8String);
}
