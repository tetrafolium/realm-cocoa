...
/**
   ...
   queries. It represents the results of the query in the form of a collection
   of objects.
   ...
   `RLMResults` always reflect the current state of the Realm on the current
   thread, including during write transactions on the current thread. The one
   exception to this is when using `for...in` fast enumeration, which will
   always enumerate over the objects which matched the query when the
   enumeration is begun, even if some of them are deleted or modified to be
...
/**
   ...
   containing `realm`. An invalidated results collection can be accessed, but
...
/**
   ...
   @return    The index of the object, or `NSNotFound` if the object is not
   found in the results collection.
 */
...
/**
   Returns all the objects matching the given predicate in the results
   collection.
...
/**
   ...
   the block is not called at all. See the `RLMCollectionChange` documentation
   for information on how the changes are reported and an example of updating a
   `UITableView`.
   ...
   transaction on the same thread or explicitly call `-[RLMRealm refresh]`,
   ...
   You must retain the returned token for as long as you want updates to
   continue to be sent to the block. To stop receiving updates, call
   `-invalidate` on the token.
...
- (RLMNotificationToken *)addNotificationBlock:
    (void (^)(RLMResults<RLMObjectType> *__nullable results,
...
/**
   Returns the minimum (lowest) value of the given property among all the
   objects represented by the results collection.
   ...
   @param property The property whose minimum value is desired. Only properties
   of types `int`, `float`, `double`, and `NSDate` are supported.
...
/**
   Returns the maximum (highest) value of the given property among all the
   objects represented by the results collection.
   ...
   @param property The property whose maximum value is desired. Only properties
   of types `int`, `float`, `double`, and `NSDate` are supported.
...
/**
   ...
   @param property The property whose values should be summed. Only properties
   of types `int`, `float`, and `double` are supported.
...
/**
   ...
   @return    The average value of the given property, or `nil` if the Results
   are empty.
 */
...
/**
   ...
   directly. `RLMResults` can be obtained by querying a Realm.
 */
- (instancetype)init
