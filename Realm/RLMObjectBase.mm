...
- (id)valueForUndefinedKey:(NSString *)key {
	...
	if (Class accessor = prop.swiftAccessor) {
		return [accessor
		        get:(char *)(__bridge void *)self + ivar_getOffset(prop.swiftIvar)];
	}
...
- (void)setValue:(id)value forUndefinedKey:(NSString *)key {
	...
	if (Ivar ivar = property.swiftIvar) {
		if (property.array) {
			...
			if (value) {
				[array addObjects:validatedObjectForProperty(
					 value, _objectSchema, property,
...
+ (nullable NSArray<RLMProperty *> *)_getPropertiesWithInstance:
        (__unused id)obj {
	return nil;
}
...
+ (BOOL)automaticallyNotifiesObserversForKey:(NSString *)key {
	if (isManagedAccessorClass(self) &&
	    [class_getSuperclass(self.class) sharedSchema][key]) {
		return NO;
	}
...
- (std::unique_ptr<realm::ThreadSafeReferenceBase>)makeThreadSafeReference {
	...
	realm::ThreadSafeReference<Object> reference =
		_realm->_realm->obtain_thread_safe_reference(std::move(object));
	return std::make_unique<realm::ThreadSafeReference<Object> >(
		std::move(reference));
}
...
+ (instancetype)objectWithThreadSafeReference:
	...
	REALM_ASSERT_DEBUG(
		dynamic_cast<realm::ThreadSafeReference<Object> *>(reference.get()));
	auto object_reference =
		static_cast<realm::ThreadSafeReference<Object> *>(reference.get());

	Object object = realm->_realm->resolve_thread_safe_reference(
	...
	return RLMCreateObjectAccessor(realm->_info[objectClassName],
	                               object.row().get_index());
}
