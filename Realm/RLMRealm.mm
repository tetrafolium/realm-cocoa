...
static void RLMAddSkipBackupAttributeToItemAtPath(std::string const &path) {
  [[NSURL fileURLWithPath:@(path.c_str())]
...
static void waitForPartialSyncSubscriptions(Realm::Config const &config) {
  ...
  auto table =
      ObjectStore::table_for_object_type(realm->read_group(), "__ResultSets");
  ...
  CFRunLoopPerformBlock(runLoop, kCFRunLoopDefaultMode, [&]() mutable {
    token = results.add_notification_callback(
        [&](CollectionChangeSet const &, std::exception_ptr) mutable {
          if (table->size() > 1) {
            token = {};
...
+ (RLMAsyncOpenTask *)
  ...
  auto openCompletion = [=](ThreadSafeReference<Realm> ref,
    ...
    @autoreleasepool {
      ...
      bool needsSubscriptions =
          realm->is_partial() && ObjectStore::table_for_object_type(
                                     realm->read_group(), "__ResultSets")
                                         ->size() == 0;
  ...
  dispatch_async(s_async_open_queue, ^{
    @autoreleasepool {
      ...
      if (config.sync_config) {
        ...
        try {
          openCompletion(realm::_impl::RealmCoordinator::get_coordinator(config)
                             ->get_unbound_realm(),
                         nullptr);
        } catch (...) {
...
REALM_NOINLINE void RLMRealmTranslateException(NSError **error) {
  try {
    ...
    case RealmFileException::Kind::IncompatibleLockFile: {
      ...
      RLMSetErrorOrThrow(
          RLMMakeError(RLMErrorIncompatibleLockFile,
                       File::PermissionDenied(err.UTF8String, ex.path())),
          error);
      break;
    }
    ...
    case RealmFileException::Kind::BadHistoryError: {
      NSString *err =
          @"Realm file's history format is incompatible with the "
           ...
           "file may be corrupt.";
      RLMSetErrorOrThrow(
          RLMMakeError(RLMErrorFileAccess,
                       File::AccessError(err.UTF8String, ex.path())),
          error);
      break;
    }
    ...
    case RealmFileException::Kind::FormatUpgradeRequired:
      RLMSetErrorOrThrow(RLMMakeError(RLMErrorFileFormatUpgradeRequired, ex),
                         error);
      break;
    default:
...
REALM_NOINLINE static void
translateSharedGroupOpenException(RLMRealmConfiguration *originalConfiguration,
                                  NSError **error) {
  try {
    throw;
  } catch (RealmFileException const &ex) {
    ...
    case RealmFileException::Kind::IncompatibleSyncedRealm: {
      ...
      NSError *intermediateError =
          RLMMakeError(RLMErrorIncompatibleSyncedFile, ex);
      ...
      NSError *finalError = [NSError errorWithDomain:intermediateError.domain
                                                code:intermediateError.code
                                            userInfo:userInfo];
...
+ (instancetype)realmWithConfiguration:(RLMRealmConfiguration *)configuration
    ...
    if (cache || dynamic) {
      if (RLMRealm *realm = RLMGetThreadLocalCachedRealmForPath(config.path)) {
        ...
        if (old_config.immutable() != config.immutable() ||
            old_config.read_only_alternative() !=
                config.read_only_alternative()) {
          @throw RLMException(@"Realm at path '%s' already opened with "
                              @"different read permissions",
                              config.path.c_str());
        }
  ...
  if (realm->_schema) {
    ...
    try {
      realm->_realm->update_schema(schema.objectStoreCopy,
...
+ (instancetype)uncachedSchemalessRealmWithConfiguration:
                    (RLMRealmConfiguration *)configuration
                                                   error:(NSError **)error {
  RLMRealm *realm = [[RLMRealm alloc] initPrivate];
  try {
...
- (BOOL)commitWriteTransactionWithoutNotifying:
  ...
  for (RLMNotificationToken *token in tokens) {
    if (token.realm != self) {
      @throw RLMException(@"Incorrect Realm: only notifications for the Realm "
...
/**
   Replaces all string columns in this Realm with a string enumeration column
   ...
   While compaction is in progress, attempts by other threads or processes to
   open the database will wait.

   Be warned that resource requirements for compaction is proportional to the
   amount of live data in the database.

   Compaction works by writing the database contents to a temporary database
   file and then replacing the database with the temporary one. The name of the
...
+ (uint64_t)schemaVersionAtURL:(NSURL *)fileURL
  ...
  try {
    ...
    if (version == realm::ObjectStore::NotVersioned) {
      RLMSetErrorOrThrow([NSError errorWithDomain:RLMErrorDomain
                                             code:RLMErrorFail
                                         userInfo:@{
                                           NSLocalizedDescriptionKey :
                                               @"Cannot open an uninitialized "
                                               @"realm in read-only mode"
                                         }],
                         error);
    }
    return version;
...
- (BOOL)writeCopyToURL:(NSURL *)fileURL
  ...
  try {
    _realm->write_copy(path.UTF8String,
                       {static_cast<const char *>(key.bytes), key.length});
    return YES;
  } catch (...) {
...
+ (BOOL)deleteFilesForConfiguration:(RLMRealmConfiguration *)config
  ...
  bool didCall = SharedGroup::call_with_lock(path, [&](auto const &path) {
    ...
    [fm removeItemAtURL:[url URLByAppendingPathExtension:@"management"]
                  error:&localError];
    if (localError && localError.code != NSFileNoSuchFileError) {
      return;
    }

    [fm removeItemAtURL:[url URLByAppendingPathExtension:@"note"]
                  error:&localError];
  });
  if (error && localError && localError.code != NSFileNoSuchFileError) {
    *error = localError;
    ...
    if (error) {
      NSString *msg =
          [NSString stringWithFormat:@"Realm file at path %s cannot be deleted "
                                     @"because it is currently opened.",
                                     path.c_str()];
...
static bool hasPrivilege(realm::ComputedPrivileges actual,
                         realm::ComputedPrivileges expected) {
...
- (RLMClassPrivileges)privilegesForClass:(Class)cls {
  if (![cls respondsToSelector:@selector(_realmObjectName)]) {
    @throw RLMException(@"Cannot get privileges for non-RLMObject class %@",
                        cls);
  }
  return
      [self privilegesForClassNamed:[cls _realmObjectName] ?: [cls className]];
}
