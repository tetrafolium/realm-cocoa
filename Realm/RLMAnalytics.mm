...
// - What version of Realm is being used, and from which language (obj-c or
// Swift).
...
static NSString *RLMMACAddress() {
	...
	std::array<int, 6> mib = {
		{CTL_NET, PF_ROUTE, 0, AF_LINK, NET_RT_IFLIST, en0}};
	size_t bufferSize;
	auto buffer = RLMSysCtl(&mib[0], mib.size(), &bufferSize);
	if (!buffer) {
		return nil;
	...
	auto mac = reinterpret_cast<const unsigned char *>(sockaddr->sdl_data +
	                                                   sockaddr->sdl_nlen);
...
static NSDictionary *RLMAnalyticsPayload() {
	...
	NSString *swiftVersion =
		isSwift ?[swiftObjectUtilClass swiftVersion] : @"N/A";
...
void RLMSendAnalytics() {
	...
	NSData *payload =
		[NSJSONSerialization dataWithJSONObject:RLMAnalyticsPayload()
		 options:0
		 error:nil];
	NSString *url = [NSString
	                 stringWithFormat:@"https://api.mixpanel.com/track/?data=%@&ip=1",
