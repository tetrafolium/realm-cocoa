...
uint8_t RLMGetComputedPermissions(RLMRealm *realm, id _Nullable object) {
  ...
  if ([object isKindOfClass:[NSString class]]) {
    return static_cast<unsigned char>(
        realm->_realm->get_privileges([object UTF8String]));
  }
...
NSError *make_auth_error_http_status(NSInteger status) {
  return [NSError errorWithDomain:RLMSyncAuthErrorDomain
                         ...
                         userInfo:@{
                           kRLMSyncErrorStatusCodeKey : @(status)
                         }];
}
...
NSError *make_auth_error(RLMSyncErrorResponseModel *model) {
  NSMutableDictionary<NSString *, NSString *> *userInfo =
...
NSError *make_sync_error(RLMSyncSystemErrorKind kind, NSString *description,
                         NSInteger code, NSDictionary *custom) {
...
NSError *make_sync_error(NSError *wrapped_auth_error) {
  return [NSError
      errorWithDomain:RLMSyncErrorDomain
                 code:RLMSyncErrorUnderlyingAuthError
             userInfo:@{kRLMSyncUnderlyingErrorKey : wrapped_auth_error}];
}

NSError *make_sync_error(std::error_code sync_error,
                         RLMSyncSystemErrorKind kind) {
  return [NSError
      errorWithDomain:RLMSyncErrorDomain
                 code:kind
             userInfo:@{
