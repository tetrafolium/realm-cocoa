...
void RLMThrowResultsError(NSString *aggregateMethod) {
  try {
    ...
    @throw RLMException(
        @"Object of type '%s' does not match RLMResults type '%s'.",
...
- (id)objectAtIndex:(NSUInteger)index {
  ...
  return translateRLMResultsErrors([&] { return _results.get(ctx, index); });
}
...
- (id)firstObject {
  ...
  RLMAccessorContext ctx(*_info);
  return translateRLMResultsErrors([&] { return _results.first(ctx); });
}
...
- (id)lastObject {
  ...
  RLMAccessorContext ctx(*_info);
  return translateRLMResultsErrors([&] { return _results.last(ctx); });
}
...
- (id)aggregate:(NSString *)property
  ...
  auto value = translateRLMResultsErrors(
      [&] { return (_results.*method)(column); }, methodName);
...
- (id)averageOfProperty:(NSString *)property {
  ...
  auto value = translateRLMResultsErrors(
      [&] { return _results.average(column); }, @"averageOfProperty");
...
- (RLMNotificationToken *)addNotificationBlock:
  ...
  if (!_realm) {
    @throw RLMException(@"Linking objects notifications are only supported on "
                        @"managed objects.");
  }
...
- (std::unique_ptr<realm::ThreadSafeReferenceBase>)makeThreadSafeReference {
  return std::make_unique<realm::ThreadSafeReference<Results>>(
      _realm->_realm->obtain_thread_safe_reference(_results));
}
...
+ (instancetype)objectWithThreadSafeReference:
  ...
  REALM_ASSERT_DEBUG(
      dynamic_cast<realm::ThreadSafeReference<Results> *>(reference.get()));
  auto results_reference =
      static_cast<realm::ThreadSafeReference<Results> *>(reference.get());

  Results results = realm->_realm->resolve_thread_safe_reference(
      std::move(*results_reference));
