...
BOOL isValidRealmURL(NSURL *url) {
  ...
  return
      [scheme isEqualToString:@"realm"] || [scheme isEqualToString:@"realms"];
}
}
...
- (void)setPinnedCertificateURL:(NSURL *)pinnedCertificateURL {
  if (pinnedCertificateURL) {
    if ([pinnedCertificateURL respondsToSelector:@selector(UTF8String)]) {
      _config->ssl_trust_certificate_path =
          std::string([(id)pinnedCertificateURL UTF8String]);
    } else {
      _config->ssl_trust_certificate_path =
          std::string(pinnedCertificateURL.path.UTF8String);
    }
  } else {
...
static void bindHandler(std::string const &, SyncConfig const &config,
  ...
  auto handle = [[RLMSyncSessionRefreshHandle alloc]
      initWithRealmURL:realmURL
                  user:user
...
static void errorHandler(std::shared_ptr<SyncSession> errored_session,
                         SyncError error) {
  ...
  // Note that certain types of errors are 'interactive'; users have several
  // options as to how to proceed after the error is reported.
  auto errorClass = errorKindForSyncError(error);
  switch (errorClass) {
  case RLMSyncSystemErrorKindClientReset: {
    custom = @{
      kRLMSyncPathOfRealmBackupCopyKey : recoveryPath,
      kRLMSyncErrorActionTokenKey : token
    };
    break;
  }
  ...
  NSError *nsError = make_sync_error(errorClass, @(error.message.c_str()),
                                     error.error_code.value(), custom);
...
- (instancetype)initWithUser:(RLMSyncUser *)user
  ...
  if (self = [super init]) {
    if (!isValidRealmURL(url)) {
      @throw RLMException(@"The provided URL (%@) was not a valid Realm URL.",
                          [url absoluteString]);
    }

    _config = std::make_unique<SyncConfig>(
        SyncConfig{[user _syncUser], [[url absoluteString] UTF8String]});
    _config->stop_policy = translateStopPolicy(stopPolicy);
    ...
    if (NSString *authorizationHeaderName =
            [RLMSyncManager sharedManager].authorizationHeaderName) {
      _config->authorization_header_name.emplace(
          authorizationHeaderName.UTF8String);
    }
    if (NSDictionary<NSString *, NSString *> *customRequestHeaders =
            [RLMSyncManager sharedManager].customRequestHeaders) {
      for (NSString *key in customRequestHeaders) {
        _config->custom_http_headers.emplace(
...
+ (RLMRealmConfiguration *)automaticConfiguration {
  if (RLMSyncUser.allUsers.count != 1)
    @throw RLMException(@"The automatic configuration requires there be "
                        @"exactly one logged-in sync user.");

  return [RLMSyncConfiguration
      automaticConfigurationForUser:RLMSyncUser.currentUser];
}

+ (RLMRealmConfiguration *)automaticConfigurationForUser:(RLMSyncUser *)user {
  RLMSyncConfiguration *syncConfig = [[RLMSyncConfiguration alloc]
       initWithUser:user
