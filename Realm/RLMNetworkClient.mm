...
typedef void (^RLMServerURLSessionCompletionBlock)(NSData *, NSURLResponse *,
                                                   NSError *);
...
+ (NSDictionary<NSString *, NSString *> *)
    httpHeadersForPayload:(NSDictionary *)json
...
+ (void)sendRequestToServer:(NSURL *)serverURL
  ...
  [self sendRequestToServer:serverURL
                       JSON:jsonDictionary
                    timeout:0
...
+ (void)sendRequestToServer:(NSURL *)serverURL
  ...
  if (![request.HTTPMethod isEqualToString:@"GET"]) {
    request.HTTPBody = [self httpBodyForPayload:jsonDictionary
                                          error:&localError];
    if (localError) {
  ...
  NSDictionary<NSString *, NSString *> *headers =
      [self httpHeadersForPayload:jsonDictionary options:options];
  for (NSString *key in headers) {
    [request addValue:headers[key] forHTTPHeaderField:key];
  }
  id delegate = [RLMSessionDelegate
      delegateWithCertificatePaths:options.pinnedCertificatePaths
...
+ (NSData *)httpBodyForPayload:(NSDictionary *)json error:(NSError **)error {
  ...
  NSAssert(localError,
           @"If there isn't a converted data object there must be an error.");
  if (error) {
    *error = localError;
  }
  return nil;
...
+ (NSDictionary<NSString *, NSString *> *)
  ...
  if (NSDictionary<NSString *, NSString *> *customHeaders =
...
- (void)URLSession:(__unused NSURLSession *)session
  ...
  // Reject the server auth and report an error if any errors occur along the
  // way
...
#if TARGET_OS_IPHONE
  id certificate = (__bridge_transfer id)SecCertificateCreateWithData(
      NULL, (__bridge CFDataRef)data);
  if (!certificate) {
    error = [NSError errorWithDomain:NSOSStatusErrorDomain
                                code:errSecUnknownFormat
                            userInfo:nil];
    return;
  }
  ...
  if (OSStatus status =
          SecItemImport((__bridge CFDataRef)data,
                        (__bridge CFStringRef)certPath.absoluteString, nullptr,
                        nullptr, 0, &params, nullptr, &items)) {
    error = [NSError errorWithDomain:NSOSStatusErrorDomain
                                code:status
                            userInfo:nil];
    return;
...
- (NSError *)validateResponse:(NSURLResponse *)response data:(NSData *)data {
  ...
  BOOL badResponse = (NSLocationInRange(httpResponse.statusCode,
                                        rangeForErrorType(ClientError)) ||
                      NSLocationInRange(httpResponse.statusCode,
                                        rangeForErrorType(ServerError)));
  if (badResponse) {
    if (RLMSyncErrorResponseModel *responseModel =
            [self responseModelFromData:data]) {
...
+ (NSURL *)urlForAuthServer:(NSURL *)authServerURL
                    payload:(__unused NSDictionary *)json {
...
+ (NSURL *)urlForAuthServer:(NSURL *)authServerURL
  ...
  return [authServerURL
      URLByAppendingPathComponent:@"auth/password/updateAccount"];
}
@end
...
+ (NSData *)httpBodyForPayload:(__unused NSDictionary *)json
...
+ (NSURL *)urlForAuthServer:(NSURL *)authServerURL
  ...
  return [authServerURL
      URLByAppendingPathComponent:
          [NSString stringWithFormat:@"permissions/offers/%@/accept",
                                     json[@"offerToken"]]];
}
@end
...
+ (NSURL *)urlForAuthServer:(NSURL *)authServerURL
  ...
  return [authServerURL
      URLByAppendingPathComponent:[NSString
                                      stringWithFormat:@"permissions/offers/%@",
                                                       json[@"offerToken"]]];
}
@end
