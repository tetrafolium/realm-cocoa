	...
	if (info.rlmObjectSchema.isSwiftClass) {
		...
		if (prop && prop.array && prop.swiftIvar) {
			...
			for (size_t i = 0; i < count; ++i) {
				...
				list._rlmArray = [[RLMManagedArray alloc]
				                  initWithList:realm::List(info.realm->_realm, *info.table(),
...
void RLMCollectionSetValueForKey(id<RLMFastEnumerable> collection,
...
RLMAddNotificationBlock(id objcCollection, Collection &collection,
	...
	auto cb = [=, &collection](realm::CollectionChangeSet const &changes,
			  ...
			  if (skip && *skip) {
				  *skip = false;
				  ...
				  block(objcCollection,
				        [[RLMCollectionChange alloc] initWithChanges:changes], nil);
			  }
		  };

	return [[RLMCancellationToken alloc]
	        initWithToken:collection.add_notification_callback(cb)
	        realm:(RLMRealm *)[objcCollection realm]];
}

// Explicitly instantiate the templated function for the two types we'll use it
// on
template RLMNotificationToken *RLMAddNotificationBlock<realm::List>(
	id, realm::List &, void (^)(id, RLMCollectionChange *, NSError *), bool);
template RLMNotificationToken *RLMAddNotificationBlock<realm::Results>(
	id, realm::Results &, void (^)(id, RLMCollectionChange *, NSError *), bool);
