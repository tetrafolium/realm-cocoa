...
typedef void (^RLMFetchedRealmCompletionBlock)(NSError *_Nullable,
                                               RLMRealm *_Nullable,
                                               BOOL *_Nonnull);
...
private:
  /**
     ...
     refreshing the Realm's access token before it expires. Tokens are indexed
  ...
  /**
     An optional callback invoked when the authentication server reports the
     user as being in an expired state.
   */
