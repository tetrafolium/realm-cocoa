...
void unregisterRefreshHandle(const std::weak_ptr<SyncUser> &user,
...
void reportInvalidAccessToken(const std::weak_ptr<SyncUser> &user,
                              NSError *error) {
	if (auto strong_user = user.lock()) {
		if (RLMUserErrorReportingBlock block =
			    context_for(strong_user).error_handler()) {
			RLMSyncUser *theUser =
				[[RLMSyncUser alloc] initWithSyncUser:std::move(strong_user)];
			[theUser logOut];
			block(theUser, error);
		}
	}
...
- (instancetype)initWithRealmURL:(NSURL *)realmURL
        ...
        completionBlock:
        (RLMSyncBasicErrorReportingBlock)completionBlock {
	if (self = [super init]) {
		...
		if (!self.authServerURL) {
			@throw RLMException(
				       @"User object isn't configured with an auth server URL.");
		}
...
- (void)scheduleRefreshTimer:(NSDate *)dateWhenTokenExpires {
	...
	NSDate *fireDate = [RLMSyncSessionRefreshHandle
	                    fireDateForTokenExpirationDate:dateWhenTokenExpires
	                    nowDate:[NSDate date]];
	if (!fireDate) {
	...
	dispatch_source_set_timer(
		self.timer,
		dispatch_time(DISPATCH_TIME_NOW, timeToExpiration * NSEC_PER_SEC),
...
/// Handler for network requests whose responses successfully parse into an auth
/// response model.
- (BOOL)_handleSuccessfulRequest:(RLMAuthResponseModel *)model
	...
	// Munge the path back onto the original URL, because the `sync` API expects
	// an entire URL.
	...
	if (!resolvedURLString) {
		@throw RLMException(
			       @"Resolved path returned from the server was invalid (%@).",
			       resolvedPath);
	}
	// Pass the token and resolved path to the underlying sync subsystem.
	session.refresh_access_token([model.accessToken.token UTF8String],
	                             { resolvedURLString.UTF8String });

	// Schedule a refresh. If we're successful we must already have `bind()`ed the
	// session initially, so we can null out the strong pointer.
	_strongSession = nullptr;
	NSDate *expires = [NSDate
	                   dateWithTimeIntervalSince1970:model.accessToken.tokenData.expires];
...
- (void)_handleFailedRequest:(NSError *)error {
	...
	if ([error.domain isEqualToString:NSURLErrorDomain]) {
		...
		case NSURLErrorCannotFindHost:
			// FIXME: 10 seconds is an arbitrarily chosen value, consider
			// rationalizing it.
	...
	if (!nextTryDate) {
		// This error isn't a network failure error. Just invalidate the refresh
		// handle and stop.
		if (_strongSession) {
	...
	// If we tried to initially bind the session and failed, we'll try again.
	// However, each subsequent attempt will use a weak pointer to avoid
	// prolonging the session's lifetime unnecessarily.
	_strongSession = nullptr;
