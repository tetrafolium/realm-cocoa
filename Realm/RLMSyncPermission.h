...
/**
   ...
   singleton object, the RLMClassPermission object for the desired class, or to a
   user-defined RLMArray<RLMPermission> property on a specific Object instance.
   The meaning of each of the properties of RLMPermission depend on what the
   permission is applied to, and so are left undocumented here. See
...
/**
   Returns the Permission object for the named Role on the object, creating it if
   needed.
...
/**
   ...
   A Role consists of a name for the role and a list of users which are members of
   the role. Roles are granted privileges on Realms, Classes and Objects, and in
   turn grant those privileges to all users which are members of the role.

   A role named "everyone" is automatically created in new Realms, and all new
   users which connect to the Realm are automatically added to it. Any other roles
   you wish to use are managed as normal Realm objects.
 */
...
/**
   ...
   An object of this type is automatically created in the Realm for you, and more
   objects cannot be created manually. Call `+[RLMRealmPermission objectInRealm:]`
   to obtain the instance for a specific Realm.
...
/**
   ...
   By default, all operations are permitted, and each privilege field indicates an
   operation which may be forbidden.
 */
...
typedef RLM_CLOSED_ENUM(NSUInteger, RLMSyncAccessLevel){
	...
	/**
	   ...
	   directly open the Realm is an error; in this case the Realm must be deleted
	   and re-opened.
	 */
...
/**
   ...
   should be modified (e.g. "/path/to/realm"). Pass "*" to apply to all Realms
   managed by the user.
   @param username    The username (often an email address) of the user who should
   be granted access to the Realm at `path`.
   @param accessLevel The access level to grant.
 */
- (instancetype)initWithRealmPath:(NSString *)path
