...
static std::vector<LinkPathPart>
parseKeypath(StringData keypath, Group const &group, Schema const &schema,
             const ObjectSchema *objectSchema) {
  auto check = [&](bool condition, const char *fmt, auto... args) {
    if (!condition) {
      throw std::invalid_argument(util::format(
          "Invalid LinkingObjects inclusion from key path '%1': %2.", keypath,
  ...
  while (begin != end) {
    ...
    check(prop->type == PropertyType::Object ||
              prop->type == PropertyType::LinkingObjects,
          "property '%1.%2' is of unsupported type '%3'", objectSchema->name,
    ...
    if (prop->type == PropertyType::Object) {
      check(begin != end,
            "key path must end in a LinkingObjects property and '%1.%2' is of "
            "type '%3'",
            objectSchema->name, key, string_for_property_type(prop->type));
      ret.emplace_back(prop->table_column);
    } else {
      ret.emplace_back(
          objectSchema->property_for_name(prop->link_origin_property_name)
              ->table_column,
...
- (instancetype)initWithOptions:(RLMSyncSubscriptionOptions *)options
                        results:(Results const &)results
  ...
  try {
    ...
    if (options.name) {
      opt.user_provided_name =
          std::string(RLMStringDataWithNSString(options.name));
    }
    ...
    if (options.includeLinkingObjectProperties) {
      ...
      opt.inclusions =
          IncludeDescriptor{*ObjectStore::table_for_object_type(
                                realm.group, results.get_object_type()),
                            keypaths};
    }
    _subscription = partial_sync::subscribe(
        options.limit ? results.limit(options.limit) : results, std::move(opt));
  ...
  _token = _subscription->add_notification_callback([weakSelf] {
    ...
    // Retrieve the current error and status. Update our properties only if the
    // values have changed, since clients use KVO to observe these properties.
    ...
    if (obj && obj->is_valid()) {
      _obj = std::move(*obj);
      _token = {};
      _token.result_sets_token = _obj.add_notification_callback(
          [weakSelf](CollectionChangeSet const &, std::exception_ptr) {
            @autoreleasepool {
...
- (void)updateFromRow {
  // We only want to call the setter if the value actually changed because of
  // KVO
#define REALM_SET_IF_CHANGED(prop, value)                                      \
  do {                                                                         \
    auto newValue = value;                                                     \
    if (prop != newValue) {                                                    \
      prop = newValue;                                                         \
  ...
  auto row = _obj.row();
  REALM_SET_IF_CHANGED(self.query, RLMStringDataToNSString(row.get_string(
                                       row.get_column_index("query"))));
  REALM_SET_IF_CHANGED(self.createdAt,
                       RLMTimestampToNSDate(row.get_timestamp(
                           row.get_column_index("created_at"))));
  REALM_SET_IF_CHANGED(self.updatedAt,
                       RLMTimestampToNSDate(row.get_timestamp(
                           row.get_column_index("updated_at"))));
  REALM_SET_IF_CHANGED(self.expiresAt,
                       RLMTimestampToNSDate(row.get_timestamp(
                           row.get_column_index("expires_at"))));
#undef REALM_SET_IF_CHANGED

  auto ttl =
      row.get<util::Optional<int64_t>>(row.get_column_index("time_to_live"));
...
- (NSString *)name {
  return _row.is_attached() ? RLMStringDataToNSString(_row.get_string(
                                  _row.get_column_index("name")))
                            : nil;
}

- (NSString *)query {
  return _row.is_attached() ? RLMStringDataToNSString(_row.get_string(
                                  _row.get_column_index("query")))
                            : nil;
}
...
- (RLMSyncSubscriptionState)state {
  ...
  return (RLMSyncSubscriptionState)_row.get_int(
      _row.get_column_index("status"));
}
...
- (NSError *)error {
  ...
  return
      [NSError errorWithDomain:RLMErrorDomain
                          code:RLMErrorFail
                      userInfo:@{
                        NSLocalizedDescriptionKey : RLMStringDataToNSString(err)
                      }];
}

- (NSDate *)createdAt {
  return _row.is_attached() ? RLMTimestampToNSDate(_row.get_timestamp(
                                  _row.get_column_index("created_at")))
                            : nil;
}

- (NSDate *)updatedAt {
  return _row.is_attached() ? RLMTimestampToNSDate(_row.get_timestamp(
                                  _row.get_column_index("updated_at")))
                            : nil;
}

- (NSDate *)expiresAt {
  return _row.is_attached() ? RLMTimestampToNSDate(_row.get_timestamp(
                                  _row.get_column_index("expires_at")))
                            : nil;
}
  ...
  return [NSString
      stringWithFormat:
          ...
          RLMStringDataToNSString(
              _row.get_string(_row.get_column_index("query"))),
          @(self.state), self.error, self.createdAt, self.updatedAt,
          self.expiresAt, @(self.timeToLive)];
}

- (void)unsubscribe {
  if (_row) {
    partial_sync::unsubscribe(
        Object(_realm->_realm, *_info->objectSchema, _row));
  }
}
...
RLMResultsSetInfo::RLMResultsSetInfo(__unsafe_unretained RLMRealm *const realm)
    : osObjectSchema(realm->_realm->read_group(),
                     partial_sync::result_sets_type_name),
      rlmObjectSchema([RLMObjectSchema
          objectSchemaForObjectStoreSchema:addPublicNames(osObjectSchema)]),
      info(realm, rlmObjectSchema, &osObjectSchema) {
  rlmObjectSchema.accessorClass = [RLMSyncSubscriptionObject class];
}

RLMClassInfo &
RLMResultsSetInfo::get(__unsafe_unretained RLMRealm *const realm) {
...
+ (instancetype)resultsWithRealm:(RLMRealm *)realm {
  auto table = ObjectStore::table_for_object_type(
      realm->_realm->read_group(), partial_sync::result_sets_type_name);
  if (!table) {
    @throw RLMException(@"-[RLMRealm subscriptions] can only be called on a "
                        @"Realm using query-based sync");
  }
  ...
  auto query = table->where().ends_with(
      table->get_column_index("matches_property"), "_matches");
...
- (RLMResults *)sortedResultsUsingDescriptors:
    (__unused NSArray<RLMSortDescriptor *> *)properties {
  @throw RLMException(
      @"Sorting subscription results is currently not implemented");
}

- (RLMResults *)distinctResultsUsingKeyPaths:
    (__unused NSArray<NSString *> *)keyPaths {
  @throw RLMException(
      @"Distincting subscription results is currently not implemented");
}
@end

@implementation RLMResults (SyncSubscription)
- (RLMSyncSubscription *)subscribe {
  return [[RLMSyncSubscription alloc] initWithOptions:nil
                                              results:_results
                                                realm:self.realm];
}
...
- (RLMSyncSubscription *)subscribeWithName:(NSString *)subscriptionName {
  ...
  return [[RLMSyncSubscription alloc] initWithOptions:options
                                              results:_results
                                                realm:self.realm];
}
...
- (nullable RLMSyncSubscription *)subscriptionWithName:(NSString *)name {
  ...
  if (!info.table()) {
    @throw RLMException(@"-[RLMRealm subcriptionWithName:] can only be called "
                        @"on a Realm using query-based sync");
  }
...
RLMSyncSubscription *RLMCastToSyncSubscription(id obj) { return obj; }
