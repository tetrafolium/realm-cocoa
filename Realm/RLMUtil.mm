...
static inline RLMArray *asRLMArray(__unsafe_unretained id const value) {
  return RLMDynamicCast<RLMArray>(value)
             ?: RLMDynamicCast<RLMListBase>(value)._rlmArray;
}

static inline bool
checkArrayType(__unsafe_unretained RLMArray *const array, RLMPropertyType type,
               bool optional,
  ...
  return array.type == type && array.optional == optional &&
         (type != RLMPropertyTypeObject ||
          [array.objectClassName isEqualToString:objectClassName]);
}
...
BOOL RLMValidateValue(__unsafe_unretained id const value, RLMPropertyType type,
                      bool optional, bool array,
                      __unsafe_unretained NSString *const objectClassName) {
...
void RLMThrowTypeError(__unsafe_unretained id const obj,
  ...
  @throw RLMException(
      ...
      prop.optional ? "?" : "", prop.array ? " array" : "",
...
void RLMValidateValueForProperty(
  ...
  if (prop.array) {
    ...
    if (!enumeration) {
      @throw RLMException(
          @"Invalid value (%@) for '%@%s' array property '%@.%@': value is not "
          @"enumerable.",
    ...
    if (RLMArray *array = asRLMArray(obj)) {
      if (!checkArrayType(array, prop.type, prop.optional,
...
BOOL RLMIsObjectValidForProperty(
  ...
  return RLMValidateValue(obj, property.type, property.optional, property.array,
...
NSError *RLMMakeError(RLMError code,
                      const realm::util::File::AccessError &exception) {
  return [NSError errorWithDomain:RLMErrorDomain
                             code:code
                         userInfo:@{
                           NSLocalizedDescriptionKey : @(exception.what()),
...
NSError *RLMMakeError(RLMError code,
                      const realm::RealmFileException &exception) {
  NSString *underlying = @(exception.underlying().c_str());
  return [NSError
      errorWithDomain:RLMErrorDomain
                 code:code
             userInfo:@{
               NSLocalizedDescriptionKey : @(exception.what()),
...
NSError *RLMMakeError(std::system_error const &exception) {
  ...
  BOOL isGenericCategoryError =
      (exception.code().category() == std::generic_category());
    ...
    *outError = error;
    ...
    if (error.userInfo[NSFilePathErrorKey]) {
      msg = [NSString stringWithFormat:@"%@: %@",
                                       error.userInfo[NSFilePathErrorKey],
