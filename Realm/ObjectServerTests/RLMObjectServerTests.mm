...
+ (instancetype)objectWithNumber:(NSInteger)number
                     firstString:(NSString *)first
...
/// Valid username/password credentials should be able to log in a user. Using
/// the same credentials should return the same user object.
- (void)testUsernamePasswordAuthentication {
  RLMSyncUser *firstUser = [self
      logInUserForCredentials:[RLMSyncTestCase
                                  basicCredentialsWithName:NSStringFromSelector(
                                                               _cmd)
                                                  register:YES]
                       server:[RLMSyncTestCase authServerURL]];
  RLMSyncUser *secondUser = [self
      logInUserForCredentials:[RLMSyncTestCase
                                  basicCredentialsWithName:NSStringFromSelector(
                                                               _cmd)
                                                  register:NO]
  ...
  XCTAssertEqualObjects(firstUser.authenticationServer,
                        [RLMSyncTestCase authServerURL]);
...
- (void)testAdminTokenAuthentication {
  RLMSyncCredentials *credentials =
      [RLMSyncCredentials credentialsWithAccessToken:self.adminToken
                                            identity:@"test"];
...
- (void)testCustomRefreshTokenAuthentication {
  RLMSyncCredentials *credentials =
      [RLMSyncCredentials credentialsWithCustomRefreshToken:@"token"
                                                   identity:@"custom_identity1"
                                                    isAdmin:NO];
  XCTAssertNotNil(credentials);
...
/// An invalid username/password credential should not be able to log in a user
/// and a corresponding error should be generated.
- (void)testInvalidPasswordAuthentication {
  ...
  RLMSyncCredentials *credentials =
      [RLMSyncCredentials credentialsWithUsername:NSStringFromSelector(_cmd)
...
- (void)testNonExistingUsernameAuthentication {
  RLMSyncCredentials *credentials =
      [RLMSyncTestCase basicCredentialsWithName:NSStringFromSelector(_cmd)
                                       register:NO];
...
- (void)testExistingUsernameRegistration {
  RLMSyncCredentials *credentials =
      [RLMObjectServerTests basicCredentialsWithName:NSStringFromSelector(_cmd)
                                            register:YES];
...
/// Errors reported in RLMSyncManager.errorHandler shouldn't contain sync error
/// domain errors as underlying error
- (void)testSyncErrorHandlerErrorDomain {
  RLMSyncUser *user = [self
      logInUserForCredentials:[RLMObjectServerTests
                                  basicCredentialsWithName:NSStringFromSelector(
                                                               _cmd)
                                                  register:YES]
                       server:[RLMObjectServerTests authServerURL]];
  ...
  NSURL *realmURL = [NSURL
      URLWithString:
          @"realm://127.0.0.1:9080/THE_PATH_USER_DONT_HAVE_ACCESS_TO/test"];
  ...
  __attribute__((objc_precise_lifetime)) RLMRealm *realm =
      [RLMRealm realmWithConfiguration:c error:&error];
  XCTAssertNil(error);
  XCTAssertTrue(realm.isEmpty);

  XCTestExpectation *expectation = [self expectationWithDescription:@""];
  [RLMSyncManager sharedManager].errorHandler =
      ^(__unused NSError *error, __unused RLMSyncSession *session) {
        XCTAssertTrue([error.domain isEqualToString:RLMSyncErrorDomain]);
        XCTAssertFalse([[error.userInfo[kRLMSyncUnderlyingErrorKey] domain]
            isEqualToString:RLMSyncErrorDomain]);
        [expectation fulfill];
      };
...
/// The pre-emptive token refresh subsystem should function, and properly
/// refresh the token.
- (void)testPreemptiveTokenRefresh {
  ...
  [RLMSyncSessionRefreshHandle
      calculateFireDateUsingTestLogic:YES
             blockOnRefreshCompletion:^(BOOL success) {
               XCTAssertTrue(success);
               if (refreshCount++ ==
                   3) { // arbitrary choice; refreshes every second
                 [ex fulfill];
               }
             }];
  // Open the Realm.
...
- (void)testLoginConnectionTimeoutFromManager {
  ...
  @autoreleasepool {
    RLMSyncCredentials *credentials =
        [RLMObjectServerTests basicCredentialsWithName:userName register:YES];
    RLMSyncUser *user = [self
        logInUserForCredentials:credentials
  ...
  RLMSyncCredentials *credentials =
      [RLMObjectServerTests basicCredentialsWithName:userName register:NO];
  XCTestExpectation *ex =
      [self expectationWithDescription:@"Login should time out"];
...
- (void)testBasicUserPersistence {
  ...
  RLMAssertThrowsWithReasonMatching([RLMSyncUser currentUser],
                                    @"currentUser cannot be called if more "
  ...
  RLMSyncUser *user = [self
      logInUserForCredentials:[RLMObjectServerTests
                                  basicCredentialsWithName:NSStringFromSelector(
                                                  ...
                                                  register:YES]
                       server:[RLMObjectServerTests authServerURL]];
  RLMSyncSession *badSession = [user
      sessionForURL:[NSURL
                        URLWithString:@"realm://127.0.0.1:9080/noSuchRealm"]];
...
- (void)testUserChangePassword {
  ...
  {
    RLMSyncCredentials *creds =
        [RLMSyncCredentials credentialsWithUsername:userName
                                           password:firstPassword
                                           register:YES];
    RLMSyncUser *user =
        [self logInUserForCredentials:creds
                               server:[RLMObjectServerTests authServerURL]];
    XCTestExpectation *ex =
        [self expectationWithDescription:@"change password callback invoked"];
  ...
  // Fail to log in with original password.
  {
    ...
    XCTestExpectation *ex =
        [self expectationWithDescription:@"login callback invoked"];
    [RLMSyncUser
...
/// A sync admin user should be able to successfully change another user's
/// password.
- (void)testOtherUserChangePassword {
  // Create admin user.
  NSURL *url = [RLMObjectServerTests authServerURL];
  RLMSyncUser *adminUser =
      [self createAdminUserForURL:url username:[[NSUUID UUID] UUIDString]];
  ...
  // Fail to change password from non-admin user.
  {
    ...
    RLMSyncCredentials *creds2 =
        [RLMSyncCredentials credentialsWithUsername:username2
                                           password:@"a"
                                           register:YES];
    RLMSyncUser *user2 = [self logInUserForCredentials:creds2 server:url];
    XCTestExpectation *ex =
        [self expectationWithDescription:@"change password callback invoked"];
    [user2 changePassword:@"foobar"
                forUserID:nonAdminUserID
  ...
  // Change password from admin user.
  {
    XCTestExpectation *ex =
        [self expectationWithDescription:@"change password callback invoked"];
    [adminUser changePassword:secondPassword
                    forUserID:nonAdminUserID
                   completion:^(NSError *error) {
                     XCTAssertNil(error);
                     [ex fulfill];
...
- (void)testRequestPasswordResetForRegisteredUser {
  NSString *userName =
      [NSStringFromSelector(_cmd) stringByAppendingString:@"@example.com"];
  RLMSyncCredentials *creds =
      [RLMSyncCredentials credentialsWithUsername:userName
                                         password:@"a"
                                         register:YES];
  [[self logInUserForCredentials:creds
                          server:[RLMObjectServerTests authServerURL]] logOut];

  XCTestExpectation *ex = [self expectationWithDescription:@"callback invoked"];
  [RLMSyncUser
      requestPasswordResetForAuthServer:[RLMObjectServerTests authServerURL]
                              userEmail:userName
                             completion:^(NSError *error) {
                               XCTAssertNil(error);
                               [ex fulfill];
  ...
  [RLMSyncUser
      completePasswordResetForAuthServer:[RLMObjectServerTests authServerURL]
                                   token:token
...
- (void)testRequestPasswordResetForNonexistentUser {
  ...
  [RLMSyncUser
      requestPasswordResetForAuthServer:[RLMObjectServerTests authServerURL]
                              userEmail:userName
                             completion:^(NSError *error) {
                               // Not an error even though the user doesn't
                               // exist
                               XCTAssertNil(error);
                               [ex fulfill];
                             }];
...
- (void)testRequestPasswordResetWithBadAuthURL {
  ...
  XCTestExpectation *ex = [self expectationWithDescription:@"callback invoked"];
  NSURL *badAuthUrl = [[RLMObjectServerTests authServerURL]
      URLByAppendingPathComponent:@"/bad"];
  [RLMSyncUser
      requestPasswordResetForAuthServer:badAuthUrl
                              userEmail:userName
                             completion:^(NSError *error) {
                               XCTAssertNotNil(error);
                               XCTAssertEqualObjects(
                                   error.userInfo[@"statusCode"], @404);
                               [ex fulfill];
                             }];
  [self waitForExpectationsWithTimeout:2.0 handler:nil];
}
...
- (void)testRequestConfirmEmailForRegisteredUser {
  ...
  XCTestExpectation *ex = [self expectationWithDescription:@"callback invoked"];
  [RLMSyncUser
      requestEmailConfirmationForAuthServer:[RLMObjectServerTests authServerURL]
                                  userEmail:userName
                                 completion:^(NSError *error) {
                                   XCTAssertNil(error);
                                   [ex fulfill];
  ...
  // This token should have been created when
  // requestEmailConfirmationForAuthServer was called
  NSString *token = [self emailForAddress:userName];
  XCTAssertNotNil(token);
  ...
  // Use the token
  ex = [self expectationWithDescription:@"callback invoked"];
  [RLMSyncUser confirmEmailForAuthServer:[RLMObjectServerTests authServerURL]
                                   token:token
                              completion:^(NSError *error) {
                                XCTAssertNil(error);
                                [ex fulfill];
...
- (void)testRetrieveUserInfo {
  ...
  RLMSyncCredentials *c1 =
      [RLMSyncCredentials credentialsWithUsername:nonAdminUsername
                                         password:pw
                                         register:YES];
  RLMSyncUser *nonAdminUser = [self logInUserForCredentials:c1 server:server];

  // Create an admin user.
  __unused RLMSyncUser *adminUser = [self createAdminUserForURL:server
                                                       username:adminUsername];

  // Create another admin user.
  RLMSyncUser *userDoingLookups =
      [self createAdminUserForURL:server username:[[NSUUID UUID] UUIDString]];

  // Get the non-admin user's info.
  XCTestExpectation *ex1 =
      [self expectationWithDescription:
                @"should be able to get info about non-admin user"];
  [userDoingLookups
      retrieveInfoForUser:nonAdminUsername
         identityProvider:RLMIdentityProviderUsernamePassword
               completion:^(RLMSyncUserInfo *info, NSError *err) {
                 ...
                 XCTAssertEqualObjects(acctInfo.providerUserIdentity,
                                       nonAdminUsername);
                 XCTAssertEqualObjects(acctInfo.provider,
  ...
  XCTestExpectation *ex2 =
      [self expectationWithDescription:
                @"should be able to get info about admin user"];
  [userDoingLookups
      retrieveInfoForUser:adminUsername
  ...
  XCTestExpectation *ex3 =
      [self expectationWithDescription:@"should fail for non-existent user"];
  [userDoingLookups
      retrieveInfoForUser:@"invalid_user@realm.example.org"
         identityProvider:RLMIdentityProviderUsernamePassword
               completion:^(RLMSyncUserInfo *info, NSError *err) {
                 XCTAssertNotNil(err);
  ...
  XCTestExpectation *ex4 =
      [self expectationWithDescription:
                @"should fail for user without admin privileges"];
  [nonAdminUser
      retrieveInfoForUser:adminUsername
         identityProvider:RLMIdentityProviderUsernamePassword
               completion:^(RLMSyncUserInfo *info, NSError *err) {
                 ...
                 // RLMSyncAuthErrorAccessDeniedOrInvalidPath?
                 XCTAssertEqual(err.code, RLMSyncAuthErrorUserDoesNotExist);
                 XCTAssertNil(info);
                 [ex4 fulfill];
               }];
...
- (void)testLoginQueueForSuccessfulLogin {
  ...
  RLMSyncCredentials *c1 =
      [RLMSyncCredentials credentialsWithUsername:[[NSUUID UUID] UUIDString]
                                         password:@"p"
                                         register:YES];
  XCTestExpectation *ex1 =
      [self expectationWithDescription:
                @"User logs in successfully on background queue"];
  [RLMSyncUser
      logInWithCredentials:c1
             authServerURL:[RLMObjectServerTests authServerURL]
                   timeout:30.0
             callbackQueue:queue
  ...
  RLMSyncCredentials *c2 =
      [RLMSyncCredentials credentialsWithUsername:[[NSUUID UUID] UUIDString]
                                         password:@"p"
                                         register:YES];
  XCTestExpectation *ex2 = [self
      expectationWithDescription:@"User logs in successfully on main queue"];
  [RLMSyncUser
      logInWithCredentials:c2
             authServerURL:[RLMObjectServerTests authServerURL]
                   timeout:30.0
             callbackQueue:dispatch_get_main_queue()
              onCompletion:^(RLMSyncUser *user, __unused NSError *error) {
...
- (void)testLoginQueueForFailedLogin {
  ...
  RLMSyncCredentials *c1 =
      [RLMSyncCredentials credentialsWithUsername:[[NSUUID UUID] UUIDString]
                                         ...
                                         register:NO];
  XCTestExpectation *ex1 =
      [self expectationWithDescription:@"Error returned on background queue"];
  [RLMSyncUser
      logInWithCredentials:c1
             authServerURL:[RLMObjectServerTests authServerURL]
                   timeout:30.0
  ...
  RLMSyncCredentials *c2 =
      [RLMSyncCredentials credentialsWithUsername:[[NSUUID UUID] UUIDString]
                                         ...
                                         register:NO];
  XCTestExpectation *ex2 =
      [self expectationWithDescription:@"Error returned on main queue"];
  [RLMSyncUser
      logInWithCredentials:c2
             authServerURL:[RLMObjectServerTests authServerURL]
                   timeout:30.0
...
- (void)testUserExpirationCallback {
  ...
  XCTestExpectation *ex =
      [self expectationWithDescription:@"callback should fire"];
  ...
  // Try to log in a Realm; this will cause our errorHandler block defined above
  // to be fired.
  __attribute__((objc_precise_lifetime)) RLMRealm *r =
      [self immediatelyOpenRealmForURL:REALM_URL() user:user];
...
/// It should be possible to successfully open a Realm configured for sync with
/// an access token.
- (void)testOpenRealmWithAdminToken {
  // FIXME (tests): opening a Realm with the access token, then opening a Realm
  // at the same virtual path with normal credentials, causes Realms to fail to
  // bind with a "bad virtual path" error.
  RLMSyncCredentials *credentials =
      [RLMSyncCredentials credentialsWithAccessToken:self.adminToken
                                            identity:@"test"];
  XCTAssertNotNil(credentials);
  RLMSyncUser *user =
      [self logInUserForCredentials:credentials
                             server:[RLMObjectServerTests authServerURL]];
  NSURL *url =
      [NSURL URLWithString:@"realm://127.0.0.1:9080/testSyncWithAdminToken"];
  RLMRealmConfiguration *c = [user configurationWithURL:url
                                    fullSynchronization:YES];
  NSError *error = nil;
...
/// If client B adds objects to a synced Realm, client A should see those
/// objects.
- (void)testAddObjects {
  NSURL *url = REALM_URL();
  RLMSyncUser *user = [self
  ...
  if (self.isParent) {
    ...
    // Add objects.
    [self addSyncObjectsToRealm:realm
                   descriptions:@[ @"child-1", @"child-2", @"child-3" ]];
    [self waitForUploadsForRealm:realm];
    CHECK_COUNT(3, SyncObject, realm);
  }
}

/// If client B deletes objects from a synced Realm, client A should see the
...
- (void)testDeleteObjects {
  ...
  if (self.isParent) {
    // Add objects.
    [self addSyncObjectsToRealm:realm
                   descriptions:@[ @"parent-1", @"parent-2", @"parent-3" ]];
    [self waitForUploadsForRealm:realm];
    CHECK_COUNT(3, SyncObject, realm);
    RLMRunChildAndWait();
...
/// If client B encrypts its synced Realm, client A should be able to access
/// that Realm with a different encryption key.
- (void)testEncryptedSyncedRealm {
  ...
  RLMRealm *realm = [self openRealmForURL:url
                                     user:user
                            encryptionKey:key
...
/// If an encrypted synced Realm is re-opened with the wrong key, throw an
/// exception.
- (void)testEncryptedSyncedRealmWrongKey {
  ...
  if (self.isParent) {
    ...
    RLMAssertThrowsWithError([RLMRealm realmWithConfiguration:c error:nil],
                             ...
                             RLMErrorFileAccess, @"invalid mnemonic");
...
- (void)testMultipleRealmsSessions {
  ...
  XCTAssertNotNil([user sessionForURL:urlA],
                  @"Expected to get a session for URL A");
  XCTAssertNotNil([user sessionForURL:urlB],
                  @"Expected to get a session for URL B");
  XCTAssertNotNil([user sessionForURL:urlC],
                  @"Expected to get a session for URL C");
  XCTAssertTrue([user sessionForURL:urlA].state == RLMSyncSessionStateActive,
                @"Expected active session for URL A");
  XCTAssertTrue([user sessionForURL:urlB].state == RLMSyncSessionStateActive,
                @"Expected active session for URL B");
  XCTAssertTrue([user sessionForURL:urlC].state == RLMSyncSessionStateActive,
...
- (void)testMultipleRealmsDeleteObjects {
  ...
  if (self.isParent) {
    ...
    [self addSyncObjectsToRealm:realmA
                   descriptions:@[
                     @"parent-A1", @"parent-A2", @"parent-A3", @"parent-A4"
                   ]];
    [self addSyncObjectsToRealm:realmB
                   descriptions:@[
...
/// When a session opened by a Realm goes out of scope, it should stay alive
...
- (void)testUploadChangesWhenRealmOutOfScope {
  ...
  if (self.isParent) {
    // Open the Realm in an autorelease pool so that it is destroyed as soon as
    // possible.
    @autoreleasepool {
      RLMRealm *realm = [self openRealmForURL:url user:user];
      [realm beginWriteTransaction];
      for (NSInteger i = 0; i < OBJECT_COUNT; i++) {
        [realm addObject:[[SyncObject alloc] initWithValue:@[
                 [NSString stringWithFormat:@"parent-%@", @(i + 1)]
               ]]];
      }
      [realm commitWriteTransaction];
    ...
    // - There is currently no API that allows asynchronous coordination for
    // waiting for an upload to begin.
    // - A delay longer than the specified one will not affect the outcome of
    // the test.
    sleep(2);
    RLMRunChildAndWait();
  } else {
...
/// A Realm that was opened before a user logged out should be able to resume
/// uploading if the user logs back in.
    ...
    user = [self logInUserForCredentials:
                     [RLMObjectServerTests
                         basicCredentialsWithName:NSStringFromSelector(_cmd)
                                  ...
                                  server:[RLMObjectServerTests authServerURL]];
    [self addSyncObjectsToRealm:realm
                   descriptions:@[ @"parent-2", @"parent-3" ]];
    [self waitForUploadsForRealm:realm];
    CHECK_COUNT(3, SyncObject, realm);
    RLMRunChildAndWait();
  } else {
...
/// A Realm that was opened before a user logged out should be able to resume
/// downloading if the user logs back in.
...
/// A Realm that was opened while a user was logged out should be able to start
/// uploading if the user logs back in.
...
/// After logging back in, a Realm whose path has been opened for the first time
/// should properly upload changes.
...
/// After logging back in, a Realm whose path has been opened for the first time
/// should properly download changes.
...
/// path as a previously-opened Realm.
...
- (void)testLogBackInReopenRealmDownload {
  ...
  if (self.isParent) {
    [self addSyncObjectsToRealm:realm descriptions:@[ @"parent-1" ]];
    [self waitForUploadsForRealm:realm];
    XCTAssert([SyncObject allObjectsInRealm:realm].count == 1,
              @"Expected 1 item");
    // Log out the user.
    [user logOut];
    // Log the user back in.
...
- (void)testSuspendAndResume {
  ...
  if (self.isParent) {
    ...
    [self addSyncObjectsToRealm:realmA
                   descriptions:@[ @"child-A2", @"child-A3" ]];
    [self waitForUploadsForRealm:realmA];
    [self addSyncObjectsToRealm:realmB
                   descriptions:@[ @"child-B2", @"child-B3" ]];
    [self waitForUploadsForRealm:realmB];
...
- (void)testClientReset {
  ...
  RLMSyncUser *user =
      [self logInUserForCredentials:[RLMObjectServerTests
                                        basicCredentialsWithName:sessionName
                                                        register:true]
                             server:[RLMObjectServerTests authServerURL]];
  // Open the Realm
  __attribute__((objc_precise_lifetime)) RLMRealm *realm =
  ...
  [RLMSyncManager sharedManager].errorHandler = ^void(NSError *error,
                                                      RLMSyncSession *session) {
    // Make sure we're actually looking at the right session.
    XCTAssertTrue([[session.realmURL absoluteString] rangeOfString:sessionName]
                      .location != NSNotFound);
    theError = error;
    [ex fulfill];
  };
  ...
  NSString *recoveryPath =
      @"io.realm.object-server-recovered-realms/recovered_realm";
...
- (void)testClientResetManualInitiation {
  ...
  @autoreleasepool {
    ...
    [RLMSyncManager sharedManager].errorHandler =
        ^void(NSError *error, RLMSyncSession *session) {
          // Make sure we're actually looking at the right session.
          XCTAssertTrue(
              [[session.realmURL absoluteString] rangeOfString:sessionName]
                  .location != NSNotFound);
          theError = error;
          [ex fulfill];
        };
  ...
  // At this point the Realm should be invalidated and client reset should be
  // possible.
  NSString *pathValue = [theError rlmSync_clientResetBackedUpRealmPath];
...
- (void)testStreamingDownloadNotifier {
  ...
  XCTAssert(transferred >= transferrable,
            @"Transferred (%@) needs to be greater than or equal to "
            @"transferrable (%@)",
            @(transferred), @(transferrable));
}
...
- (void)testDownloadCancelsOnAuthError {
  RLMSyncUser *user = [self
      logInUserForCredentials:[RLMObjectServerTests
                       ...
                       server:[RLMObjectServerTests authServerURL]];
  auto c = [user
      configurationWithURL:[NSURL
                               URLWithString:@"realm://127.0.0.1:9080/invalid"]
       fullSynchronization:true];
...
- (void)testOfflineClientReset {
  ...
  RLMSyncUser *user = [self
      logInUserForCredentials:[RLMObjectServerTests
                       ...
                       server:[RLMObjectServerTests authServerURL]];
  NSURL *sourceFileURL =
      [[NSBundle bundleForClass:[self class]] URLForResource:@"sync-1.x"
                                               withExtension:@"realm"];
  NSString *fileName = [NSString stringWithFormat:@"%@.realm", [NSUUID new]];
  NSURL *fileURL =
      [NSURL fileURLWithPath:[NSTemporaryDirectory()
                                 stringByAppendingPathComponent:fileName]];
  [NSFileManager.defaultManager copyItemAtURL:sourceFileURL
                                        toURL:fileURL
                                        error:&error];
  XCTAssertNil(error);
  if (error) {
  ...
  RLMRealmConfiguration *backupConfiguration =
      error.userInfo[RLMBackupRealmConfigurationErrorKey];
  XCTAssertNotNil(backupConfiguration);

  // Open the backup Realm with a schema subset since it was created using the
  // schema from .NET's unit tests. The Person class is declared in
  // SwiftObjectServerTests.swift.
  backupConfiguration.objectClasses = @[ NSClassFromString(@"Person") ];

  error = nil;
  RLMRealm *backupRealm = [RLMRealm realmWithConfiguration:backupConfiguration
                                                     error:&error];
...
- (void)waitForKeyPath:(NSString *)keyPath object:(id)object value:(id)value {
  [self
      waitForExpectations:@[ [[XCTKVOExpectation alloc] initWithKeyPath:keyPath
                                                                 object:object
                                                          expectedValue:value] ]
                  timeout:20.0];
}
...
- (void)testPartialSync {
  ...
  @autoreleasepool {
    RLMSyncCredentials *creds =
        [RLMObjectServerTests basicCredentialsWithName:name register:YES];
    ...
    [realm addObject:[PartialSyncObjectB objectWithNumber:0
                                              firstString:@""
                                             secondString:@""]];
    [realm addObject:[PartialSyncObjectB objectWithNumber:1
                                              firstString:@""
                                             secondString:@""]];
    [realm addObject:[PartialSyncObjectB objectWithNumber:2
                                              firstString:@""
                                             secondString:@""]];
    [realm addObject:[PartialSyncObjectB objectWithNumber:3
                                              firstString:@""
                                             secondString:@""]];
    [realm addObject:[PartialSyncObjectB objectWithNumber:4
                                              firstString:@""
                                             secondString:@""]];
    [realm addObject:[PartialSyncObjectB objectWithNumber:5
                                              firstString:@""
                                             secondString:@""]];
    [realm addObject:[PartialSyncObjectB objectWithNumber:6
                                              firstString:@""
                                             secondString:@""]];
    [realm addObject:[PartialSyncObjectB objectWithNumber:7
                                              firstString:@""
                                             secondString:@""]];
    [realm addObject:[PartialSyncObjectB objectWithNumber:8
                                              firstString:@""
                                             secondString:@""]];
    [realm addObject:[PartialSyncObjectB objectWithNumber:9
                                              firstString:@""
                                             secondString:@""]];
    [realm commitWriteTransaction];
    [self waitForUploadsForRealm:realm];
  ...
  @autoreleasepool {
    RLMSyncCredentials *creds =
        [RLMObjectServerTests basicCredentialsWithName:name register:NO];
    RLMSyncUser *user = [self logInUserForCredentials:creds server:server];
    RLMRealmConfiguration *configuration = [user configuration];
    RLMRealm *realm = [self openRealmWithConfiguration:configuration];

    // Perform some partial sync queries
    RLMResults *objects = [PartialSyncObjectA objectsInRealm:realm
                                                       where:@"number > 5"];
    RLMSyncSubscription *subscription = [objects subscribeWithName:@"query"];

    // Wait for the results to become available.
    [self waitForKeyPath:@"state"
                  object:subscription
    ...
    XCTAssertEqual([PartialSyncObjectA allObjectsInRealm:realm].count,
                   objects.count);
    XCTAssertEqual([PartialSyncObjectB allObjectsInRealm:realm].count, 0u);

    // Create a subscription with the same name but a different query. This
    // should trigger an error.
    RLMResults *objects2 = [PartialSyncObjectA objectsInRealm:realm
                                                        where:@"number < 5"];
    ...
    // Unsubscribe from the query, and ensure that it correctly transitions to
    // the invalidated state.
    [subscription unsubscribe];
    [self waitForKeyPath:@"state"
                  object:subscription
                   value:@(RLMSyncSubscriptionStateInvalidated)];
  }
}
...
- (void)testAllSubscriptionsReportsNewlyCreatedSubscription {
  ...
  RLMSyncSubscription *subscription = [[PartialSyncObjectA
      objectsInRealm:realm
               where:@"number > 5"] subscribeWithName:@"query"];
...
- (void)testAllSubscriptionsDoesNotReportLocalError {
  RLMRealm *realm = [self partialRealmWithName:_cmd];
  RLMSyncSubscription *subscription1 = [[PartialSyncObjectA
      objectsInRealm:realm
               where:@"number > 5"] subscribeWithName:@"query"];
  [self waitForKeyPath:@"state"
                object:subscription1
                 value:@(RLMSyncSubscriptionStateComplete)];
  RLMSyncSubscription *subscription2 = [[PartialSyncObjectA
      objectsInRealm:realm
               where:@"number > 6"] subscribeWithName:@"query"];
  [self waitForKeyPath:@"state"
                object:subscription2
                 value:@(RLMSyncSubscriptionStateError)];
  XCTAssertEqual(1U, realm.subscriptions.count);
...
- (void)testAllSubscriptionsReportsServerError {
  RLMRealm *realm = [self partialRealmWithName:_cmd];
  RLMSyncSubscription *subscription = [[PersonObject
      objectsInRealm:realm
               where:@"SUBQUERY(parents, $p1, $p1.age < 31 AND "
                     @"SUBQUERY($p1.parents, $p2, $p2.age > 35 AND $p2.name == "
                     @"'Michael').@count > 0).@count > 0"]
...
- (void)testUnsubscribeUsingOriginalSubscriptionObservingFetched {
  RLMRealm *realm = [self partialRealmWithName:_cmd];
  RLMSyncSubscription *original =
      [[PartialSyncObjectA allObjectsInRealm:realm] subscribeWithName:@"query"];
  [self waitForKeyPath:@"state"
                object:original
                 value:@(RLMSyncSubscriptionStateComplete)];
  XCTAssertEqual(1U, realm.subscriptions.count);
...
- (void)testSortAndFilterSubscriptions {
  ...
  [self waitForKeyPath:@"state"
                object:[[PartialSyncObjectA allObjectsInRealm:realm]
                           subscribeWithName:@"query 1"]
                 value:@(RLMSyncSubscriptionStateComplete)];
  [self waitForKeyPath:@"state"
                object:[[PartialSyncObjectA allObjectsInRealm:realm]
                           subscribeWithName:@"query 2"]
                 value:@(RLMSyncSubscriptionStateComplete)];
  [self waitForKeyPath:@"state"
                object:[[PartialSyncObjectB allObjectsInRealm:realm]
                           subscribeWithName:@"query 3"]
                 value:@(RLMSyncSubscriptionStateComplete)];
  RLMResults *unsupportedQuery = [PersonObject
      objectsInRealm:realm
               where:@"SUBQUERY(parents, $p1, $p1.age < 31 AND "
                     @"SUBQUERY($p1.parents, $p2, $p2.age > 35 AND $p2.name == "
                     @"'Michael').@count > 0).@count > 0"];
  [self waitForKeyPath:@"state"
                object:[unsupportedQuery subscribeWithName:@"query 4"]
                 value:@(RLMSyncSubscriptionStateError)];

  auto subscriptions = realm.subscriptions;
  XCTAssertEqual(4U, subscriptions.count);
  XCTAssertEqual(0U,
                 ([subscriptions objectsWhere:@"name = %@", @"query 0"].count));
  XCTAssertEqualObjects(
      @"query 1",
      ([subscriptions objectsWhere:@"name = %@", @"query 1"].firstObject.name));
  XCTAssertEqual(
      3U, ([subscriptions
               objectsWhere:@"status = %@", @(RLMSyncSubscriptionStateComplete)]
               .count));
  XCTAssertEqual(1U,
                 ([subscriptions objectsWhere:@"status = %@",
                                              @(RLMSyncSubscriptionStateError)]
                      .count));

  XCTAssertEqual(4U,
                 ([subscriptions objectsWhere:@"createdAt >= %@", now]).count);
  XCTAssertEqual(0U,
                 ([subscriptions objectsWhere:@"createdAt < %@", now]).count);
  ...
  XCTAssertThrows([subscriptions sortedResultsUsingKeyPath:@"name"
                                                 ascending:NO]);
...
- (void)testIncludeLinkingObjectsErrorHandling {
  ...
  RLMAssertThrowsWithReason(
      [objects subscribeWithOptions:opt],
      @"Invalid LinkingObjects inclusion from key path 'nonexistent': property "
      @"'PersonObject.nonexistent' does not exist.");

  opt.includeLinkingObjectProperties = @[ @"name" ];
  RLMAssertThrowsWithReason(
      [objects subscribeWithOptions:opt],
      @"Invalid LinkingObjects inclusion from key path 'name': property "
      @"'PersonObject.name' is of unsupported type 'string'.");

  opt.includeLinkingObjectProperties = @[ @"children.name" ];
  RLMAssertThrowsWithReason(
      [objects subscribeWithOptions:opt],
      @"Invalid LinkingObjects inclusion from key path 'children.name': "
      @"property 'PersonObject.name' is of unsupported type 'string'.");

  opt.includeLinkingObjectProperties = @[ @"children" ];
  RLMAssertThrowsWithReason(
      [objects subscribeWithOptions:opt],
      @"Invalid LinkingObjects inclusion from key path 'children': key path "
      @"must end in a LinkingObjects property and 'PersonObject.children' is "
      @"of type 'array'.");

  opt.includeLinkingObjectProperties = @[ @"children." ];
  RLMAssertThrowsWithReason([objects subscribeWithOptions:opt],
                            @"Invalid LinkingObjects inclusion from key path "
                            @"'children.': missing property name.");
...
- (void)attemptLoginWithUsername:(NSString *)userName
                        callback:(void (^)(RLMSyncUser *, NSError *))callback {
  NSURL *url = [RLMObjectServerTests secureAuthServerURL];
  RLMSyncCredentials *creds =
      [RLMObjectServerTests basicCredentialsWithName:userName register:YES];

  XCTestExpectation *expectation =
      [self expectationWithDescription:@"HTTP login"];
...
- (void)testHTTPSLoginFailsWithoutCertificate {
  [self attemptLoginWithUsername:NSStringFromSelector(_cmd)
                        callback:^(RLMSyncUser *user, NSError *error) {
                          XCTAssertNil(user);
                          XCTAssertNotNil(error);
                          XCTAssertEqualObjects(error.domain, NSURLErrorDomain);
...
static NSURL *certificateURL(NSString *filename) {
  return
      [NSURL fileURLWithPath:[[[@(__FILE__) stringByDeletingLastPathComponent]
...
- (void)testHTTPSLoginFailsWithIncorrectCertificate {
  RLMSyncManager.sharedManager.pinnedCertificatePaths =
      @{@"localhost" : certificateURL(@"not-localhost.cer")};
  [self attemptLoginWithUsername:NSStringFromSelector(_cmd)
                        callback:^(RLMSyncUser *user, NSError *error) {
                          XCTAssertNil(user);
                          XCTAssertNotNil(error);
...
- (void)testHTTPSLoginFailsWithInvalidPathToCertificate {
  RLMSyncManager.sharedManager.pinnedCertificatePaths =
      @{@"localhost" : certificateURL(@"nonexistent.pem")};
  [self
      attemptLoginWithUsername:NSStringFromSelector(_cmd)
                      callback:^(RLMSyncUser *user, NSError *error) {
                        XCTAssertNil(user);
                        XCTAssertNotNil(error);
...
- (void)testHTTPSLoginFailsWithDifferentValidCert {
  RLMSyncManager.sharedManager.pinnedCertificatePaths =
      @{@"localhost" : certificateURL(@"localhost-other.cer")};
  [self attemptLoginWithUsername:NSStringFromSelector(_cmd)
                        callback:^(RLMSyncUser *user, NSError *error) {
                          XCTAssertNil(user);
                          XCTAssertNotNil(error);
...
- (void)testHTTPSLoginFailsWithFileThatIsNotACert {
  RLMSyncManager.sharedManager.pinnedCertificatePaths =
      @{@"localhost" : certificateURL(@"../test-ros-server.js")};
  [self attemptLoginWithUsername:NSStringFromSelector(_cmd)
                        callback:^(RLMSyncUser *user, NSError *error) {
                          XCTAssertNil(user);
                          XCTAssertNotNil(error);
...
- (void)testHTTPSLoginDoesNotUseCertificateForDifferentDomain {
  RLMSyncManager.sharedManager.pinnedCertificatePaths =
      @{@"example.com" : certificateURL(@"localhost.cer")};
  [self attemptLoginWithUsername:NSStringFromSelector(_cmd)
                        callback:^(RLMSyncUser *user, NSError *error) {
                          XCTAssertNil(user);
                          XCTAssertNotNil(error);
...
- (void)testHTTPSLoginSucceedsWithValidSelfSignedCertificate {
  RLMSyncManager.sharedManager.pinnedCertificatePaths =
      @{@"localhost" : certificateURL(@"localhost.cer")};
...
- (void)testConfigurationFromUserAutomaticallyUsesCert {
  ...
  [self attemptLoginWithUsername:NSStringFromSelector(_cmd)
                        callback:^(RLMSyncUser *u, NSError *error) {
                          XCTAssertNotNil(u);
                          XCTAssertNil(error);
                          user = u;
                        }];
  ...
  XCTAssertEqualObjects(config.syncConfiguration.pinnedCertificateURL,
                        certificateURL(@"localhost.cer"));
...
- (void)verifyOpenFails:(RLMRealmConfiguration *)config {
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"wait for error"];
  RLMSyncManager.sharedManager.errorHandler =
...
- (void)testConfigurationFromInsecureUserAutomaticallyUsesCert {
  ...
  RLMRealmConfiguration *config =
      [user configurationWithURL:
                [NSURL URLWithString:@"realms://localhost:9443/~/default"]];
  XCTAssertEqualObjects(config.syncConfiguration.realmURL.scheme, @"realms");
  XCTAssertEqualObjects(config.syncConfiguration.pinnedCertificateURL,
                        certificateURL(@"localhost.cer"));
  ...
  RLMSyncUser *user = [self
      logInUserForCredentials:[RLMSyncTestCase
                                  basicCredentialsWithName:NSStringFromSelector(
                                                  ...
                                                  register:YES]
                       server:[RLMSyncTestCase authServerURL]];
  [self verifyOpenFails:
            [user configurationWithURL:
                      [NSURL
                          URLWithString:@"realms://localhost:9443/~/default"]]];
}
...
- (void)testLoginFailsWithoutCustomHeader {
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"register user"];
  [RLMSyncUser
      logInWithCredentials:[RLMSyncTestCase
                               basicCredentialsWithName:NSStringFromSelector(
                                                            _cmd)
                                               register:YES]
...
- (void)testLoginUsesCustomHeader {
  RLMSyncManager.sharedManager.customRequestHeaders =
      @{@"X-Allow-Connection" : @"true"};
  RLMSyncUser *user = [self
      logInUserForCredentials:[RLMSyncTestCase
                                  basicCredentialsWithName:NSStringFromSelector(
                                                               _cmd)
...
- (void)testModifyCustomHeadersAfterOpeningRealm {
  ...
  NSURL *url = [NSURL
      URLWithString:[NSString stringWithFormat:@"realm://127.0.0.1:9081/~/%@",
  ...
  XCTestExpectation *ex1 =
      [self expectationWithDescription:@"connection failure"];
  RLMSyncManager.sharedManager.errorHandler =
      ^(NSError *error, RLMSyncSession *) {
        XCTAssertNotNil(error);
        XCTAssertEqualObjects(@400, [error.userInfo[@"underlying_error"]
                                        userInfo][@"statusCode"]);
        [ex1 fulfill];
      };
  RLMRealm *realm = [RLMRealm realmWithConfiguration:c error:nil];
  ...
  [self waitForExpectationsWithTimeout:4.0 handler:nil];
  XCTAssertEqual(syncSession.connectionState,
  ...
  auto ex2 = [[XCTKVOExpectation alloc]
      initWithKeyPath:@"connectionState"
               object:syncSession
  ...
  XCTestExpectation *ex3 =
      [self expectationWithDescription:@"reconnection failure"];
  RLMSyncManager.sharedManager.errorHandler =
      ^(NSError *error, RLMSyncSession *) {
        XCTAssertNotNil(error);
        XCTAssertEqualObjects(@400, [error.userInfo[@"underlying_error"]
  ...
  auto ex4 = [[XCTKVOExpectation alloc]
      initWithKeyPath:@"connectionState"
               object:syncSession
