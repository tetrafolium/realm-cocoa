...
#define APPLY_PERMISSION(ma_permission, ma_user)                               \
  APPLY_PERMISSION_WITH_MESSAGE(ma_permission, ma_user, ma_user,               \
                                @"Setting a permission should work")

#define APPLY_PERMISSION_WITH_MESSAGE(ma_permission, ma_user, ma_target_user,  \
                                      ma_message)                              \
  do {                                                                         \
    APPLY_PERMISSION_UNCHECKED(ma_permission, ma_user, ma_message);            \
    CHECK_PERMISSION_PRESENT([self getPermissionResultsFor:ma_target_user],    \
                             ma_permission, ma_user);                          \
  } while (0)

#define APPLY_PERMISSION_UNCHECKED(ma_permission, ma_user, ma_message)         \
  do {                                                                         \
    XCTestExpectation *ex = [self expectationWithDescription:ma_message];      \
    [ma_user                                                                   \
        applyPermission:ma_permission                                          \
               callback:^(NSError * err) {                                     \
                 XCTAssertNil(                                                 \
                     err, @"Received an error when applying permission: %@",   \
                     err);                                                     \
                 [ex fulfill];                                                 \
               }];                                                             \
...
#define REVOKE_PERMISSION(ma_permission, ma_user)                              \
  do {                                                                         \
    XCTestExpectation *ex =                                                    \
        [self expectationWithDescription:@"revoke permission"];                \
    [ma_user                                                                   \
        applyPermission:ma_permission                                          \
               callback:^(NSError * err) {                                     \
                 XCTAssertNil(                                                 \
    ...
    [self waitForExpectations:@[ ex ] timeout:2.0];                            \
    CHECK_PERMISSION_ABSENT([self getPermissionResultsFor:ma_user],            \
                            ma_permission, ma_user);                           \
  } while (0)

#define CHECK_COUNT_PENDING_DOWNLOAD(expected_count, m_type, m_realm)          \
  CHECK_COUNT_PENDING_DOWNLOAD_CUSTOM_EXPECTATION(expected_count, m_type,      \
                                                  m_realm, nil)

/// This macro tries ten times to wait for downloads and then check for object
/// count. If the object count does not match, it waits 0.1 second before trying
/// again. It is most useful in cases where the test ROS might be expected to
/// take some non-negligible amount of time performing an operation whose
/// completion is required for the test on the client side to proceed.
#define CHECK_COUNT_PENDING_DOWNLOAD_CUSTOM_EXPECTATION(                       \
    expected_count, m_type, m_realm, m_exp)                                    \
  do {                                                                         \
    RLMSyncConfiguration *m_config = m_realm.configuration.syncConfiguration;  \
    XCTAssertNotNil(m_config,                                                  \
                    @"Realm passed to CHECK_COUNT_PENDING_DOWNLOAD() doesn't " \
                    @"have a sync config!");                                   \
    RLMSyncUser *m_user = m_config.user;                                       \
    NSURL *m_url = m_config.realmURL;                                          \
    for (int i = 0; i < 10; i++) {                                             \
      [self waitForDownloadsForUser:m_user                                     \
                                url:m_url                                      \
                        expectation:m_exp                                      \
                              error:nil];                                      \
      if (expected_count == [m_type allObjectsInRealm:m_realm].count) {        \
        break;                                                                 \
      }                                                                        \
...
static NSURL *makeTestURL(NSString *name, RLMSyncUser *owner) {
  ...
  return [[NSURL alloc]
      initWithString:[NSString stringWithFormat:@"realm://127.0.0.1:9080/%@/%@",
                                                userID, name]];
}
...
static NSURL *makeTestGlobalURL(NSString *name) {
  return [[NSURL alloc]
      initWithString:[NSString
                         stringWithFormat:@"realm://127.0.0.1:9080/%@", name]];
}
...
- (void)setUp {
  ...
  self.userA =
      [self logInUserForCredentials:[RLMSyncTestCase
                                        basicCredentialsWithName:userNameA
                                                        register:YES]
  ...
  self.userB =
      [self logInUserForCredentials:[RLMSyncTestCase
                                        basicCredentialsWithName:userNameB
                                                        register:YES]
                             server:[RLMSyncTestCase authServerURL]];
  ...
  self.userC =
      [self logInUserForCredentials:[RLMSyncTestCase
                                        basicCredentialsWithName:userNameC
                                                        register:YES]
                             server:[RLMSyncTestCase authServerURL]];

  RLMSyncManager.sharedManager.errorHandler =
      ^(NSError *error, __unused RLMSyncSession *session) {
        XCTFail(@"Error handler should not be called unless explicitly "
                @"expected. Error: %@",
                error);
      };
}
...
#define CHECK_PERMISSION_PRESENT(ma_results, ma_permission, ma_user)           \
  XCTAssertNotEqual(                                                           \
      [ma_results                                                              \
          indexOfObject:[ma_permission                                         \
                            tildeExpandedSyncPermissionForUser:ma_user]],      \
      NSNotFound)
...
- (NSArray<RLMSyncPermission *> *)getPermissionResultsFor:(RLMSyncUser *)user
  ...
  [user retrievePermissionsWithCallback:^(NSArray<RLMSyncPermission *> *r,
                                          NSError *error) {
    XCTAssertNil(error);
    XCTAssertNotNil(r);
    results = r;
  ...
  XCTAssertNotNil(results,
                  @"getPermissionResultsFor: failed for user %@. No results.",
                  user.identity);
  return results;
}
...
- (void)testErrorHandlingForInvalidUser {
  ...
  called = false;
  [self.userA acceptOfferForToken:@""
                         callback:^(NSURL *url, NSError *err) {
                           XCTAssertNil(url);
                           checkError(err);
                         }];
  XCTAssertTrue(called);
  ...
  called = false;
  [self.userA invalidateOfferForToken:@""
                             callback:^(NSError *err) {
                               checkError(err);
                             }];
  XCTAssertTrue(called);
...
/// If user A grants user B read access to a Realm, user B should be able to
/// read from it.
- (void)testReadAccess {
  ...
  [self addSyncObjectsToRealm:userARealm
  ...
  RLMSyncPermission *p =
      [[RLMSyncPermission alloc] initWithRealmPath:[userAURL path]
  ...
  RLMRealmConfiguration *userBConfig = [self.userB configurationWithURL:userBURL
                                                    fullSynchronization:YES];
  __block RLMRealm *userBRealm = nil;
  XCTestExpectation *asyncOpenEx =
      [self expectationWithDescription:@"Should asynchronously open a Realm"];
  ...
  // Run this portion of the test on a background queue, since the error handler
  // is dispatched onto the main queue.
  XCTestExpectation *deniedEx =
      [self expectationWithDescription:@"Expect a permission denied error."];
  RLMSyncManager.sharedManager.errorHandler =
      ^(NSError *err, __unused RLMSyncSession *session) {
        // Expect an error from the global error handler.
        XCTAssertNotNil(err);
        XCTAssertEqual(err.code, RLMSyncErrorPermissionDeniedError);
        [deniedEx fulfill];
      };
  [self addSyncObjectsToRealm:userBRealm
                 descriptions:@[ @"child-4", @"child-5", @"child-6" ]];
  [self waitForExpectations:@[ deniedEx ] timeout:20.0];

  // TODO: if we can get the session itself we can check to see if it's been
  ...
  RLMSyncPermission *p2 =
      [[RLMSyncPermission alloc] initWithRealmPath:[userBURL path]
                                          identity:self.userC.identity
                                       accessLevel:RLMSyncAccessLevelRead];
  XCTestExpectation *manageEx =
      [self expectationWithDescription:
                @"Managing a Realm you can't manage should fail."];
  [self.userB applyPermission:p2
...
/// If user A grants user B write access to a Realm, user B should be able to
/// write to it.
- (void)testWriteAccess {
  ...
  // Open the Realm for user B. Since user B has write privileges, they should
...
/// If user A grants user B manage access to a Realm, user B should be able to
/// set a permission for user C.
- (void)testManageAccess {
  ...
  // Open a Realm for user A.
  RLMRealm *userARealm = [self openRealmForURL:userAURLUnresolved
                                          user:self.userA];
  ...
  RLMSyncPermission *p =
      [[RLMSyncPermission alloc] initWithRealmPath:[userAURLUnresolved path]
                                          identity:self.userB.identity
                                       accessLevel:RLMSyncAccessLevelAdmin];
  // Set the permission.
  APPLY_PERMISSION(p, self.userA);

  // Open the Realm for user B. Since user B has admin privileges, they should
  // be able to open it 'normally'.
  RLMRealm *userBRealm = [self openRealmForURL:userAURLResolved
                                          user:self.userB];
  CHECK_COUNT_PENDING_DOWNLOAD(3, SyncObject, userBRealm);
  ...
  RLMSyncPermission *p2 =
      [[RLMSyncPermission alloc] initWithRealmPath:[userAURLResolved path]
                                          identity:self.userC.identity
                                       accessLevel:RLMSyncAccessLevelWrite];
  APPLY_PERMISSION_WITH_MESSAGE(
      p2, self.userB, self.userC,
      @"User B should be able to give C write permissions to A's Realm.");

  // User C should be able to write to the Realm.
  RLMRealm *userCRealm = [self openRealmForURL:userAURLResolved
                                          user:self.userC];
  CHECK_COUNT_PENDING_DOWNLOAD(5, SyncObject, userCRealm);
  [self addSyncObjectsToRealm:userCRealm
                 descriptions:@[ @"child-6", @"child-7", @"child-8" ]];
...
/// If user A grants user B write access to a Realm via username, user B should
...
- (void)testWriteAccessViaUsername {
  ...
  NSString *userAFullPath =
      [makeTildeSubstitutedURL(userAURL, self.userA) path];
  RLMSyncPermission *p =
      [[RLMSyncPermission alloc] initWithRealmPath:userAURL.path
  ...
  RLMSyncPermission *expected =
      [[RLMSyncPermission alloc] initWithRealmPath:userAFullPath
                                          identity:self.userB.identity
                                       accessLevel:RLMSyncAccessLevelWrite];
  CHECK_PERMISSION_PRESENT([self getPermissionResultsFor:self.userB], expected,
                           self.userB);
...
- (void)testWildcardWriteAccess {
  ...
  RLMSyncPermission *p =
      [[RLMSyncPermission alloc] initWithRealmPath:[ownerURL path]
                                          identity:@"*"
                                       accessLevel:RLMSyncAccessLevelWrite];
  ...
  [self addSyncObjectsToRealm:userCRealm
                 descriptions:@[
                   @"child-6", @"child-7", @"child-8", @"child-9"
                 ]];
  [self waitForUploadsForRealm:userCRealm];
...
- (void)testWildcardGlobalRealmReadAccess {
  RLMSyncUser *admin =
      [self createAdminUserForURL:[RLMSyncTestCase authServerURL]
  ...
  RLMSyncPermission *p =
      [[RLMSyncPermission alloc] initWithRealmPath:[globalRealmURL path]
                                          identity:@"*"
                                       accessLevel:RLMSyncAccessLevelRead];

  // Set the permission.
  APPLY_PERMISSION_WITH_MESSAGE(p, admin, self.userA,
                                @"Setting wildcard permission should work.");

  // Have the admin user write a few objects first.
  [self addSyncObjectsToRealm:adminUserRealm
                 descriptions:@[ @"child-1", @"child-2", @"child-3" ]];
  ...
  __block RLMRealm *userBRealm = nil;
  RLMRealmConfiguration *userBConfig =
      [self.userB configurationWithURL:globalRealmURL fullSynchronization:YES];
  XCTestExpectation *asyncOpenEx =
      [self expectationWithDescription:@"Should asynchronously open a Realm"];
  [RLMRealm asyncOpenWithConfiguration:userBConfig
                         callbackQueue:dispatch_get_main_queue()
  ...
  RLMRealmConfiguration *userCConfig =
      [self.userC configurationWithURL:globalRealmURL fullSynchronization:YES];
  XCTestExpectation *asyncOpenEx2 =
      [self expectationWithDescription:@"Should asynchronously open a Realm"];
...
- (void)testWildcardGlobalRealmWriteAccess {
  ...
  // Set the permission.
  APPLY_PERMISSION_WITH_MESSAGE(p, admin, self.userA,
                                @"Should grant access to all users");
...
/// Setting a permission should work, and then that permission should be able to
/// be retrieved.
- (void)testSettingPermission {
  // First, there should be no permissions.
  NSArray<RLMSyncPermission *> *results =
      [self getPermissionResultsFor:self.userA];
  ...
  // Give user B read permissions to that Realm.
  RLMSyncPermission *p = [[RLMSyncPermission alloc]
      initWithRealmPath:[makeTildeSubstitutedURL(url, self.userA) path]
               identity:self.userB.identity
            accessLevel:RLMSyncAccessLevelRead];
  ...
  // Now retrieve the permissions again and make sure the new permission is
  // properly set.
  results = [self
      getPermissionResultsFor:self.userB
                      message:@"One permission after setting the permission."];
...
- (void)testDeletingPermission {
  ...
  // properly set.
  NSArray<RLMSyncPermission *> *results =
      [self getPermissionResultsFor:self.userB
                            message:@"Setting new permission."];
  CHECK_PERMISSION_PRESENT(results, p, self.userA);

  // Delete the permission.
  RLMSyncPermission *p2 =
      [[RLMSyncPermission alloc] initWithRealmPath:url.path
                                          identity:self.userB.identity
                                       accessLevel:RLMSyncAccessLevelNone];
  REVOKE_PERMISSION(p2, self.userA);

  // Make sure the permission deletion is properly reflected.
  results = [self getPermissionResultsFor:self.userB
                                  message:@"Setting new permission."];
  CHECK_PERMISSION_COUNT(results, 0);
}

/// KVC getting and setting should work properly for
/// `NSArray<RLMSyncPermission>`.
- (void)testKVCWithPermissionsResults {
  NSURL *url1 = CUSTOM_REALM_URL(@"r1");
  NSURL *url2 = CUSTOM_REALM_URL(@"r2");
  __attribute__((objc_precise_lifetime)) RLMRealm *r1 =
      [self openRealmForURL:url1 user:self.userA];
  __attribute__((objc_precise_lifetime)) RLMRealm *r2 =
      [self openRealmForURL:url2 user:self.userA];
  ...
  id p1 = [[RLMSyncPermission alloc] initWithRealmPath:path1
                                           ...
                                           accessLevel:RLMSyncAccessLevelRead];
  APPLY_PERMISSION_WITH_MESSAGE(
      p1, self.userA, self.userA,
      @"Setting r1 permission for user B should work.");
  NSString *path2 = [makeTildeSubstitutedURL(url2, self.userA) path];
  id p2 = [[RLMSyncPermission alloc] initWithRealmPath:path2
                                              identity:uB
                                           accessLevel:RLMSyncAccessLevelRead];
  APPLY_PERMISSION_WITH_MESSAGE(
      p2, self.userA, self.userA,
      @"Setting r2 permission for user B should work.");
...
- (void)testFilteringPermissions {
  ...
  __attribute__((objc_precise_lifetime)) RLMRealm *r2 =
      [self openRealmForURL:url2 user:self.userA];
  __attribute__((objc_precise_lifetime)) RLMRealm *r3 =
      [self openRealmForURL:url3 user:self.userA];
  NSString *uB = self.userB.identity;
  ...
  id p3 = [[RLMSyncPermission alloc] initWithRealmPath:url3.path
                                           ...
                                           accessLevel:RLMSyncAccessLevelRead];
  APPLY_PERMISSION_WITH_MESSAGE(
      p3, self.userA, self.userA,
      @"Setting r3 permission for user B should work.");
  ...
  // Now make a filter.
  NSArray<RLMSyncPermission *> *filtered = [results
      filteredArrayUsingPredicate:[NSPredicate
                                      predicateWithFormat:@"path == %@",
                                                          finalPath]];
...
- (void)testSortingPermissionsOnUserID {
  ...
  id p1 = [[RLMSyncPermission alloc]
      initWithRealmPath:[makeTildeSubstitutedURL(url, self.userA) path]
               identity:self.userB.identity
            accessLevel:RLMSyncAccessLevelRead];
  APPLY_PERMISSION_WITH_MESSAGE(
      p1, self.userA, self.userA,
      @"Setting r permission for user B should work.");
  id p2 = [[RLMSyncPermission alloc]
      initWithRealmPath:[makeTildeSubstitutedURL(url, self.userA) path]
               identity:self.userC.identity
            accessLevel:RLMSyncAccessLevelRead];
  APPLY_PERMISSION_WITH_MESSAGE(
      p2, self.userA, self.userA,
      @"Setting r permission for user C should work.");

  // Now sort on user ID.
  NSArray<RLMSyncPermission *> *results =
      [self getPermissionResultsFor:self.userA];
  NSArray<RLMSyncPermission *> *sorted =
      [results sortedArrayUsingDescriptors:@[ [NSSortDescriptor
                                               sortDescriptorWithKey:@"identity"
                                                           ascending:YES] ]];
...
- (void)testPermissionResultsIndexOfObject {
  ...
  id p4 = [[RLMSyncPermission alloc]
      initWithRealmPath:[makeTildeSubstitutedURL(url, self.userB) path]
               identity:uB
            accessLevel:RLMSyncAccessLevelRead];
...
- (void)testSettingUnownedRealmPermission {
  ...
  XCTestExpectation *ex2 = [self
      expectationWithDescription:@"Setting an invalid permission should fail."];
  [self.userB
      applyPermission:p
             callback:^(NSError *error) {
               XCTAssertNotNil(error);
  ...
  // Now retrieve the permissions again and make sure the new permission was not
  // set.
  NSArray<RLMSyncPermission *> *results =
      [self getPermissionResultsFor:self.userB
...
- (NSString *)createOfferForRealmAtURL:(NSURL *)url
  ...
  XCTestExpectation *ex = [self
      expectationWithDescription:@"Should get a token when making an offer."];
...
- (void)testPermissionOffer {
  NSURL *url = REALM_URL();
  [self openRealmForURL:url user:self.userA];
  [self createOfferForRealmAtURL:url
                            user:self.userA
                     accessLevel:RLMSyncAccessLevelWrite
                      expiration:nil];
}
...
- (void)testPermissionOfferIsExpired {
  ...
  XCTestExpectation *ex =
      [self expectationWithDescription:
                @"Server should process the permission offer."];
  [self.userA
      createOfferForRealmAtURL:url
                   accessLevel:RLMSyncAccessLevelWrite
                    expiration:[NSDate dateWithTimeIntervalSinceNow:-30 * 24 *
                                                                    60 * 60]
                      callback:^(NSString *token, NSError *error) {
                        XCTAssertNotNil(error);
                        XCTAssertNil(token);
                        XCTAssertEqual(error.code,
                                       RLMSyncAuthErrorExpiredPermissionOffer);
...
/// Get a permission offer token, then permission offer response will be
/// processed, then open another user's Realm file
- (void)testPermissionOfferResponse {
  NSURL *url = REALM_URL();
  // Create the Realm
  [self openRealmForURL:url user:self.userA];

  NSString *token = [self createOfferForRealmAtURL:url
                                              user:self.userA
                                       accessLevel:RLMSyncAccessLevelWrite
                                        expiration:nil];
  ...
  XCTestExpectation *ex = [self
      expectationWithDescription:@"Server should process offer acceptance."];
  [self.userB acceptOfferForToken:token
                         callback:^(NSURL *returnedURL, NSError *error) {
                           XCTAssertNil(error);
  ...
  XCTAssertEqualObjects([realmURL path],
                        [makeTildeSubstitutedURL(url, self.userA) path]);
...
/// Failed to process a permission offer response object due to `token` is
/// invalid
- (void)testPermissionOfferResponseInvalidToken {
  ...
  XCTestExpectation *ex = [self
      expectationWithDescription:@"Server should process offer acceptance."];
  [self.userA acceptOfferForToken:badToken
                         callback:^(NSURL *returnedURL, NSError *err) {
  ...
  XCTAssertEqualObjects(error.userInfo[NSLocalizedDescriptionKey],
                        @"Your request parameters did not validate. token: "
                        @"Invalid parameter 'token'!;");
}

/// Failed to process a permission offer response object due to `token`
/// represents a Realm that does not exist
- (void)testPermissionOfferResponseTokenNotExist {
  ...
  XCTestExpectation *ex = [self
      expectationWithDescription:@"Server should process offer acceptance."];
  [self.userA acceptOfferForToken:fakeToken
                         callback:^(NSURL *returnedURL, NSError *err) {
                           XCTAssertNil(returnedURL);
                           XCTAssertNotNil(err);
                           error = err;
...
- (void)testInvalidatePermissionOffer {
  ...
  XCTestExpectation *ex2 =
      [self expectationWithDescription:@"Should invalidate a offer token."];
  [self.userA invalidateOfferForToken:token
                             callback:^(NSError *error) {
                               XCTAssertNil(error);
                               [ex2 fulfill];
                             }];
  ...
  XCTestExpectation *ex3 = [self
      expectationWithDescription:@"Server should reject invalidated offer"];
  [self.userB
      acceptOfferForToken:token
                 callback:^(NSURL *returnedURL, NSError *error) {
                   XCTAssertNil(returnedURL);
                   XCTAssertNotNil(error);
                   XCTAssertEqual(error.code,
...
- (void)testRetrievePermissionOffers {
  ...
  // Create two offers
  NSString *token1 = [self createOfferForRealmAtURL:url
                                               user:self.userA
                                        accessLevel:RLMSyncAccessLevelRead
                                         expiration:expiresAt];
  NSString *token2 = [self createOfferForRealmAtURL:url
                                               user:self.userA
                                        accessLevel:RLMSyncAccessLevelWrite
                                         expiration:nil];

  id ex1 = [self expectationWithDescription:@"Retrieve offers"];
  [self.userA retrievePermissionOffersWithCallback:^(
                  NSArray<RLMSyncPermissionOffer *> *offers, NSError *error) {
    XCTAssertNil(error);
    ...
    for (RLMSyncPermissionOffer *offer in offers) {
      ...
      XCTAssertGreaterThan(offer.createdAt.timeIntervalSince1970,
                           createdAt.timeIntervalSince1970);
      if (isFirst) {
        XCTAssertEqualObjects(offer.token, token1);
        // May be up to a half ms off due to rounding
        XCTAssertLessThan(
            fabs([offer.expiresAt timeIntervalSinceDate:expiresAt]), 0.001);
  ...
  XCTestExpectation *ex2 =
      [self expectationWithDescription:@"Should invalidate a offer token."];
  [self.userA invalidateOfferForToken:token1
                             callback:^(NSError *error) {
                               XCTAssertNil(error);
                               [ex2 fulfill];
                             }];
...
/// A Realm which is opened improperly should report an error allowing the app
/// to recover.
- (void)testDeleteRealmUponPermissionDenied {
  ...
  @autoreleasepool {
    ...
    XCTestExpectation *ex2 = [self
        expectationWithDescription:@"We should get a permission denied error."];
    RLMSyncManager.sharedManager.errorHandler =
        ^(NSError *err, RLMSyncSession *session) {
    ...
    __attribute__((objc_precise_lifetime)) RLMRealm *bad =
        [RLMRealm realmWithConfiguration:userBConfig error:nil];
    [self waitForExpectationsWithTimeout:10.0 handler:nil];
    onDiskPath = [RLMSyncTestCase onDiskPathForSyncedRealm:bad];
  }
  XCTAssertNotNil(onDiskPath);
  XCTAssertTrue(
      [[NSFileManager defaultManager] fileExistsAtPath:[onDiskPath path]]);
