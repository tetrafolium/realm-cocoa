...
// Workaround for
// <https://github.com/realm/realm-object-server-private/issues/950>.
...
if (!process.env.SYNC_WORKER_FEATURE_TOKEN) {
  try {
    ...
    console.error(
        'ROS feature token not found. Running Object Server tests requires setting the SYNC_WORKER_FEATURE_TOKEN environment variable.');
    process.exit(1);
  }
}
...
class Proxy {
  constructor(listenPort, targetPort) {
    this.proxy = httpProxy.createProxyServer(
        {target : `http://127.0.0.1:${targetPort}`, ws : true});
    this.proxy.on('error', e => { console.log('proxy error', e); });
    this.server = http.createServer((req, res) => { this.web(req, res); });
    this.server.on('upgrade',
                   (req, socket, head) => { this.ws(req, socket, head); });
  ...
  web(req, res) { this.proxy.web(req, res); }

  ws(req, socket, head) { this.proxy.ws(req, socket, head); }
}
...
class HeaderValidationProxy extends Proxy {
  ...
  validate(req) { return !!req.headers['x-allow-connection']; }
}

// A proxy which sits in front of ROS and takes a long time to establish
// connections
class SlowConnectingWebProxy extends Proxy {
  web(req, res) { setTimeout(() => this.proxy.web(req, res), 2000); }
}
...
server
    .start({
      // The desired logging threshold. Can be one of: all, trace, debug,
      // detail, info, warn, error, fatal, off)
      logLevel : 'off',
      ...
      authProviders : [
        ...
        new ROS.auth.PasswordAuthProvider({
          ...
          emailHandler :
              new PasswordEmailHandler(path.join(process.argv[2], 'email')),
        }),
      ],
      autoKeyGen : true,
    ...
    .catch(err => {console.error(
               `Error starting Realm Object Server: ${err.message}`)});
