...
- (instancetype)init {
  if (self = [super init]) {
    _serverDataRoot = [NSURL
        fileURLWithPath:[NSTemporaryDirectory()
                            stringByAppendingPathComponent:@"test-ros-data"]];
  }
  return self;
}
...
- (void)launch {
  ...
  [[NSTask launchedTaskWithLaunchPath:@"/usr/bin/pkill"
                            arguments:@[ @"-f", @"node.*test-ros-server.js" ]]
      waitUntilExit];
  NSError *error;
  [NSFileManager.defaultManager removeItemAtURL:self.serverDataRoot
                                          error:&error];
  ...
  _task.arguments = @[
    [directory stringByAppendingPathComponent:@"test-ros-server.js"],
  ...
  atexit([] {
    ...
    [NSFileManager.defaultManager removeItemAtURL:self->_serverDataRoot
                                            error:nil];
  });
}
...
- (NSString *)desiredObjectServerVersion {
  auto path =
      [[[[@(__FILE__) stringByDeletingLastPathComponent] // RLMSyncTestCase.mm
  ...
  auto file = [NSString stringWithContentsOfFile:path
  ...
  auto regex = [NSRegularExpression
      regularExpressionWithPattern:@"^REALM_OBJECT_SERVER_VERSION=(.*)$"
...
- (NSString *)currentObjectServerVersion {
  ...
  NSError *error;
  NSDictionary *json = [NSJSONSerialization JSONObjectWithData:file
                                                       options:0
                                                         error:&error];
  if (!json) {
...
- (void)downloadObjectServer {
  ...
  task.arguments = @[
    [[nodePath() stringByDeletingLastPathComponent]
        stringByAppendingPathComponent:@"npm"],
    @"--scripts-prepend-node-path=auto", @"--no-color", @"--no-progress",
    @"--no-save", @"--no-package-lock", @"install",
...
+ (RLMSyncCredentials *)basicCredentialsWithName:(NSString *)name
...
- (void)addSyncObjectsToRealm:(RLMRealm *)realm
...
- (RLMRealm *)openRealmForURL:(NSURL *)url
  ...
  RLMSyncManager.sharedManager.sessionCompletionNotifier = ^(NSError *error) {
    if (error) {
      XCTFail(@"Received an asynchronous error when trying to open Realm at "
              @"'%@' for user '%@': %@ (process: %@)",
  ...
  XCTAssert(dispatch_semaphore_wait(
            ...
            @"Timed out while trying to asynchronously open Realm for URL: %@",
            url);
...
- (RLMRealm *)openRealmWithConfiguration:(RLMRealmConfiguration *)configuration
  ...
  RLMSyncManager.sharedManager.sessionCompletionNotifier = ^(NSError *error) {
    if (error) {
      XCTFail(@"Received an asynchronous error when trying to open Realm at "
              @"'%@' for user '%@': %@ (process: %@)",
              syncConfig.realmURL, syncConfig.user.identity, error,
              self.isParent ? @"parent" : @"child");
    }
    dispatch_semaphore_signal(sema);
  };
...
- (RLMRealm *)immediatelyOpenRealmForURL:(NSURL *)url user:(RLMSyncUser *)user {
  return
      [self immediatelyOpenRealmForURL:url
                                  user:user
                         encryptionKey:nil
                            stopPolicy:RLMSyncStopPolicyAfterChangesUploaded];
}
...
- (RLMSyncUser *)logInUserForCredentials:(RLMSyncCredentials *)credentials
  ...
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"Should log in the user properly"];
  [RLMSyncUser
  ...
  XCTAssertTrue(theUser.state == RLMSyncUserStateActive,
                @"User should have been valid, but wasn't. (process: %@)",
                process);
  return theUser;
}

- (RLMSyncUser *)createAdminUserForURL:(NSURL *)url
                              username:(NSString *)username {
  return [self logInUserForCredentials:[RLMSyncCredentials
                                           credentialsWithDebugUserID:username
                                                              isAdmin:YES]
                                server:url];
}
...
- (NSString *)adminToken {
  ...
  NSDictionary *json = [NSJSONSerialization JSONObjectWithData:raw
                                                       options:0
                                                         error:nil];
...
- (NSString *)emailForAddress:(NSString *)email {
  NSURL *target = [[RealmObjectServer.sharedServer.serverDataRoot
      URLByAppendingPathComponent:@"/email"] URLByAppendingPathComponent:email];
  NSString *body = [NSString stringWithContentsOfURL:target
                                            encoding:NSUTF8StringEncoding
                                               error:nil];
  if (body) {
...
- (void)waitForUploadsForRealm:(RLMRealm *)realm error:(NSError **)error {
  ...
  BOOL queued = [session waitForUploadCompletionOnQueue:nil
...
- (void)manuallySetRefreshTokenForUser:(RLMSyncUser *)user
...
- (void)primeSyncManagerWithSemaphore:(dispatch_semaphore_t)semaphore {
  if (semaphore == nil) {
    [[RLMSyncManager sharedManager]
        setSessionCompletionNotifier:^(__unused NSError *error){
        }];
    return;
  }
  [[RLMSyncManager sharedManager] setSessionCompletionNotifier:^(
                                      NSError *error) {
    XCTAssertNil(error, @"Session completion block returned with an error: %@",
                 error);
...
- (void)setupSyncManager {
  ...
  if (self.isParent) {
    ...
    clientDataRoot =
        [NSURL fileURLWithPath:RLMDefaultDirectoryForBundleIdentifier(nil)];
  } else {
...
- (void)resetSyncManager {
  [RLMSyncManager.sharedManager._allUsers
      makeObjectsPerformSelector:@selector(logOut)];
  [RLMSyncManager resetForTesting];
