...
#define APPLY_PERMISSION(ma_permission, ma_user)                               \
  do {                                                                         \
    XCTestExpectation *ex =                                                    \
        [self expectationWithDescription:@"apply permission"];                 \
    [ma_user                                                                   \
...
- (NSError *)subscribeToRealm:(RLMRealm *)realm
                         type:(Class)cls
                        where:(NSString *)pred {
  RLMSyncSubscription *sub = [[cls objectsInRealm:realm where:pred] subscribe];
  id ex = [[XCTKVOExpectation alloc]
      initWithKeyPath:@"state"
               object:sub
        expectedValue:@(RLMSyncSubscriptionStateComplete)];
  [self waitForExpectations:@[ ex ] timeout:20.0];
  return sub.error;
}

- (NSURL *)createRealmWithName:(SEL)sel
                   permissions:(void (^)(RLMRealm *realm))block {
  ...
  // FIXME: we currently need to add a subscription to get the permissions types
  // sent to us
  [adminRealm refresh];
  CHECK_COUNT(0, SyncObject, adminRealm);
  [self subscribeToRealm:adminRealm
                    type:[SyncObject class]
                   where:@"TRUEPREDICATE"];
  CHECK_COUNT(3, SyncObject, adminRealm);

  // Set up permissions on the Realm
  [adminRealm transactionWithBlock:^{
    block(adminRealm);
  }];
  ...
  p = [[RLMSyncPermission alloc] initWithRealmPath:[url path]
...
static RLMPermissionRole *getRole(RLMRealm *realm, NSString *roleName) {
  return [RLMPermissionRole createOrUpdateInRealm:realm
                                        withValue:@{@"name" : roleName}];
}

static void addUserToRole(RLMRealm *realm, NSString *roleName, NSString *user) {
  [getRole(realm, roleName).users
      addObject:[RLMPermissionUser userInRealm:realm withIdentity:user]];
}

static void createPermissions(RLMArray<RLMPermission> *permissions) {
  auto permission = [RLMPermission permissionForRoleNamed:@"everyone"
                                                  inArray:permissions];
  ...
  permission = [RLMPermission permissionForRoleNamed:@"reader"
                                             inArray:permissions];
  permission.canRead = true;
  permission.canQuery = true;

  permission = [RLMPermission permissionForRoleNamed:@"writer"
                                             inArray:permissions];
  permission.canUpdate = true;
  permission.canCreate = true;
  permission.canDelete = true;

  permission = [RLMPermission permissionForRoleNamed:@"admin"
                                             inArray:permissions];
  permission.canSetPermissions = true;
}

#define CHECK_REALM_PRIVILEGE(realm, ...)                                      \
  do {                                                                         \
...
#define CHECK_CLASS_PRIVILEGE(realm, ...)                                      \
  do {                                                                         \
...
#define CHECK_OBJECT_PRIVILEGE(realm, ...)                                     \
  do {                                                                         \
    RLMObjectPrivileges expected{__VA_ARGS__};                                 \
    auto actual = [realm                                                       \
        privilegesForObject:[SyncObject allObjectsInRealm:realm].firstObject]; \
    XCTAssertEqual(expected.read, actual.read);                                \
...
- (void)testRealmReadAccess {
  NSURL *url =
      [self createRealmWithName:_cmd
                    permissions:^(RLMRealm *realm) {
                      createPermissions(
                          [RLMRealmPermission objectInRealm:realm].permissions);
  ...
  [self subscribeToRealm:userARealm
                    type:[SyncObject class]
                   where:@"TRUEPREDICATE"];
  ...
  [self subscribeToRealm:userBRealm
                    type:[SyncObject class]
                   where:@"TRUEPREDICATE"];
...
- (void)testRealmWriteAccess {
  ...
  CHECK_OBJECT_PRIVILEGE(userARealm, .read = true, .update = true, .del = true,
                         .setPermissions = true);
...
- (void)testRealmManagePermissions {
  // FIXME: this test is wrong; setPermission doesn't govern adding users to
  // roles
#if 0
...
- (void)testClassRead {
  NSURL *url =
      [self createRealmWithName:_cmd
                    permissions:^(RLMRealm *realm) {
                      createPermissions([RLMClassPermission
                                            objectInRealm:realm
                                                 forClass:SyncObject.class]
                                            .permissions);
                      addUserToRole(realm, @"reader", self.userA.identity);
                    }];
  ...
  [self subscribeToRealm:userARealm
                   ...
                   where:@"TRUEPREDICATE"];
  CHECK_REALM_PRIVILEGE(userARealm, .read = true, .update = true,
                        .setPermissions = true, .modifySchema = true);
  CHECK_CLASS_PRIVILEGE(userARealm, .read = true, .subscribe = true);
  CHECK_OBJECT_PRIVILEGE(userARealm, .read = true);
  CHECK_COUNT(3, SyncObject, userARealm);
  ...
  [self subscribeToRealm:userBRealm
                    type:[SyncObject class]
                   where:@"TRUEPREDICATE"];
  CHECK_REALM_PRIVILEGE(userBRealm, .read = true, .update = true,
                        .setPermissions = true, .modifySchema = true);
  CHECK_CLASS_PRIVILEGE(userBRealm, .read = false);
  CHECK_COUNT(0, SyncObject, userBRealm);
}
...
- (void)testClassSetPermissions {
  ...
  // Despite having write access userB should not be able to add "update" access
  // to "reader"
  RLMRealm *userBRealm = [self openRealmForURL:url user:self.userB];
  [self subscribeToRealm:userBRealm
                    type:[SyncObject class]
                   where:@"TRUEPREDICATE"];
  [userBRealm transactionWithBlock:^{
    auto permission = [RLMPermission permissionForRoleNamed:@"reader"
                                                    onClass:SyncObject.class
                                                      realm:userBRealm];
    permission.canCreate = true;
    permission.canUpdate = true;
  }];
  ...
  RLMRealm *userCRealm = [self openRealmForURL:url user:self.userC];
  [self subscribeToRealm:userCRealm
                    type:[SyncObject class]
                   where:@"TRUEPREDICATE"];
  CHECK_REALM_PRIVILEGE(userCRealm, .read = true, .update = true,
                        .setPermissions = true, .modifySchema = true);
...
- (void)testObjectRead {
  NSURL *url = [self
      createRealmWithName:_cmd
              permissions:^(RLMRealm *realm) {
                addUserToRole(realm, @"reader", self.userA.identity);
                auto obj1 = [ObjectWithPermissions createInRealm:realm
                                                       withValue:@[ @1 ]];
  ...
  RLMRealm *userARealm = [self openRealmForURL:url user:self.userA];
  [self subscribeToRealm:userARealm
                    type:[ObjectWithPermissions class]
                   where:@"TRUEPREDICATE"];
...
- (void)testObjectUpdate {
  ...
  [self subscribeToRealm:userARealm
                    type:[ObjectWithPermissions class]
                   where:@"TRUEPREDICATE"];
  ObjectWithPermissions *objA =
      [ObjectWithPermissions allObjectsInRealm:userARealm].firstObject;
  [userARealm transactionWithBlock:^{
    objA.value = 3;
  }];
  [self waitForSync:userARealm];
  ...
  [self subscribeToRealm:userBRealm
                    type:[ObjectWithPermissions class]
                   where:@"TRUEPREDICATE"];
  ObjectWithPermissions *objB =
      [ObjectWithPermissions allObjectsInRealm:userBRealm].firstObject;
  [userBRealm transactionWithBlock:^{
    objB.value = 3;
  }];
  [self waitForSync:userBRealm];
...
- (void)testObjectSetPermissions {
  NSURL *url = [self createRealmWithName:_cmd
                             permissions:^(RLMRealm *){
                             }];
  ...
  [self subscribeToRealm:userARealm
                   ...
                   where:@"TRUEPREDICATE"];
  [userARealm transactionWithBlock:^{
    auto obj = [ObjectWithPermissions createInRealm:userARealm
                                          withValue:@[ @1 ]];
    auto permissions = [RLMPermission permissionForRoleNamed:@"foo"
                                                    onObject:obj];
...
- (void)testRetrieveClassPermissionsForRenamedClass {
  [self createRealmWithName:_cmd
                permissions:^(RLMRealm *realm) {
                  XCTAssertNotNil([RLMClassPermission
                      objectInRealm:realm
                           forClass:RLMPermissionRole.class]);
                }];
}
