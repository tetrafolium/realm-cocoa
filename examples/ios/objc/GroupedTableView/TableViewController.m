...
- (void)viewDidLoad {
  ...
  for (NSString *section in self.sectionTitles) {
    RLMResults *unsortedObjects =
        [DemoObject objectsWhere:@"sectionTitle == %@", section];
    RLMResults *sortedObjects =
        [unsortedObjects sortedResultsUsingKeyPath:@"date" ascending:YES];
...
- (void)tableView:(UITableView *)tableView
  ...
  if (editingStyle == UITableViewCellEditingStyleDelete) {
    ...
    [realm
        deleteObject:[self objectsInSection:indexPath.section][indexPath.row]];
...
- (void)add {
  [[RLMRealm defaultRealm] transactionWithBlock:^{
    [DemoObject createInDefaultRealmWithValue:@[
      [self randomTitle], [NSDate date], [self randomSectionTitle]
    ]];
  }];
}
